...
def edo_clTwoDlineMakerSelectCmd():
    print "select ......"
    cmds.select(cl=1)
...
def edo_mirrorStroke():
    ...
    st=sels[0]+'_stroke'
    dt=sels[1]+'_stroke'
    cmds.setAttr(dt+'.minClip',cmds.getAttr(st+'.minClip'))
    cmds.setAttr(dt+'.maxClip',cmds.getAttr(st+'.maxClip'))
...
def edo_clTwoDlineMakerUI():
    ...
    cmds.textScrollList('twodcurvelist',e=1,sc='edo_clTwoDlineMakerSelectCmd()',ams=1)
    cmds.popupMenu(b=3,p='twodcurvelist')
    cmds.menuItem(l='find stroke',c='edo_selectStrokFromCurve()')
    cmds.textScrollList('twodstrokelist',e=1,sc='edo_clTwoDlineMakerSelectCmd()',ams=1)
