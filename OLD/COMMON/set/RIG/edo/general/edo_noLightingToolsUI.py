...
def edo_noLightingToolsUI():
    ui='//file-cluster/GDC/Resource/Support/Maya/extra/Rigging_Simulation/python/edo/general/edo_noLightingToolsUI.myuis'
...
def edo_setModelAllRender():
    ...
    for t in alltransforms:
        if t[len(':MODEL')*-1:]==':MODEL':
            amods.append(t)
        if t[len(':MSH_all')*-1:]==':MSH_all':
            amods.append(t)
    print amods
    for mod in amods:
        #mod=mods[0]
...
def edo_hideNoModelGrp():
    alltransforms=cmds.ls(type='transform')
    for t in alltransforms:
        if t[len(':DEFORMERS')*-1:]==':DEFORMERS':
            print 'hide ... '+t
            cmds.hide(t)
        if t[len(':DEFORMER')*-1:]==':DEFORMER':
            print 'hide ... '+t
            cmds.hide(t)
        if t[len(':Deformers')*-1:]==':Deformers':
            print 'hide ... '+t
        ...
        if t[len(':rig')*-1:]==':rig':
            print 'hide ... '+t
            cmds.hide(t)
        if t[len(':PROX')*-1:]==':PROX':
            print 'hide ... '+t
            cmds.hide(t)
        if t[len(':PROXY')*-1:]==':PROXY':
            print 'hide ... '+t
            cmds.hide(t)
        if t[len(':proxy')*-1:]==':proxy':
            print 'hide ... '+t
            cmds.hide(t)
        if t[len(':car_RIG')*-1:]==':car_RIG':
            print 'hide ... '+t
            cmds.hide(t)
...
def edo_setAllFacialCtrls():
    fcs=[]
    mas=[]
    allts=cmds.ls(type='transform')
    for t in allts:
        #t='aaa:Facial_CTRL_FRAME'
        if t[len(':Facial_CTRL_FRAME')*-1:]==':Facial_CTRL_FRAME':
            fcs.append(t)
        if t[len(':Master')*-1:]==':Master':
            mas.append(t)
    print fcs
    print mas
    if fcs:
