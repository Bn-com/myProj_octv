...
class AutoRigging():
    ...
    def runLayout(self):
        ...
        self.OrigenRB = rig.radioButton( label= u'2014升级版',sl = 1)
        ...
        #calimero
        print 'search for tool path'
        bodyrigpath=headfile.__file__.replace('headfile.py','RIG/').replace('\\','/')
        #print bodyrigpath
        rig.separator(w = 322,h=15,style='in')
        edo_calimeroFeetRiggingMainUIPath=bodyrigpath+'edo_calimeroFeetRiggingMainUI.py'
        print edo_calimeroFeetRiggingMainUIPath
        rig.button(l= u'calimeroFeetRigging',w=322,h=40,c = 'execfile(\''+edo_calimeroFeetRiggingMainUIPath+'\')')
        rig.separator(w = 322,h=15,style='in')
        edo_checkRiggingFileUIpath=bodyrigpath+'tools/edo_checkRiggingFileUI.py'
        print edo_checkRiggingFileUIpath
        rig.button(l= u'设置文件控制器规范检查',w=322,h=40,c = 'execfile(\''+edo_checkRiggingFileUIpath+'\')')
        #execfile('//file-cluster/GDC/Resource/Support/Maya/extra/Rigging_Simulation/edward/python/RIG/tools/edo_checkRiggingFileUI.py')
        rig.separator(w = 322,h=15,style='in')
        edo_motionBuilderControlGdcRigUIpath=bodyrigpath+'edo_motionBuilderControlGdcRigUI.py'
        print edo_motionBuilderControlGdcRigUIpath
        rig.button(l= u'联合HUMAN IK使用',w=322,h=40,c = 'execfile(\''+edo_motionBuilderControlGdcRigUIpath+'\');edo_motionBuilderControlGdcRigUI()')
        #execfile('//file-cluster/GDC/Resource/Support/Maya/extra/Rigging_Simulation/edward/python/RIG/edo_motionBuilderControlGdcRigUI.py');edo_motionBuilderControlGdcRigUI()
        return self.returnLayout
