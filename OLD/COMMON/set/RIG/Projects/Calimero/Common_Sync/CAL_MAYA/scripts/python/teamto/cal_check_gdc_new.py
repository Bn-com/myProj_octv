...
class check_toolbox_gdc():
    ...
    def check_texture_format(self, error_type):
        ...
        correctTypesByMode = {"_mo_": ["png", "tga"], "_rg_": ["png", "tga"], "_px_": ["tga", "png"], "_tx_rgb_": ["tga"], "_render_": ["png", "zip", "tga"]}
                    ...
                    if step in correctTypesByMode:
                        ...
                        if step == "_render_":
                            ...
                            for checkExt in correctTypesByMode[step]:
                                if not os.path.isfile(filePathName.replace("." + ext, '.' + checkExt)):
                                    message += filePathName.replace("." + ext, '.' + checkExt) + ", "
    ...
    def check_calimero_shading(self, scenePathName, modelType, error_type):
        ...
        for shader in shaders:
            ...
            elif modelType == "PROPS" and ('prp_' in scenePathName or 'veh_' in scenePathName):
                if colorIDExists:
                    colorR = cmds.getAttr(colorID + '.colorR')
                    colorG = cmds.getAttr(colorID + '.colorG')
                    colorB = cmds.getAttr(colorID + '.colorB')
...
def cal_check_shading_gdc(scenePathName, modelName, modelType, local, txStep=False):
    ...
    def test_steps(self, local):
        ...
        if self.scenePathName != "unknown" and not "_presentation_" in self.scenePathName:
            ...
            if "_mo_" in self.scenePathName or "_mo." in self.scenePathName or "_layout_" in self.scenePathName or "_layout." in self.scenePathName:
                ...
                self.step = "_tx_"
                check = cal_check_shading_gdc(self.scenePathName.lower(), self.modelName, self.modelType, local, txStep=True)
            elif "_render_" in self.scenePathName or "_render." in self.scenePathName:
                self.step = "_render_"
                check = cal_check_shading_gdc(self.scenePathName.lower(), self.modelName, self.modelType, local)
            else:
            ...
            if 'setAttr ".ftn"' in content:
                ...
                for line in f:
                    if 'setAttr ".ftn"' in line:
                        ...
                        newLine = lineSplit[0] + maFolder.replace('scenes/', 'sourceimages/') + name
                        if self.step == "_tx_":
