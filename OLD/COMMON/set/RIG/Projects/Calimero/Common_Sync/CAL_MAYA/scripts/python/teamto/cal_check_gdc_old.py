...
class check_toolbox_gdc():
    ...
    def check_texture_format(self, error_type):
        if displayPrints:
            print "* check_texture_format".ljust(self.defsMaxLenght)+" ("+error_type.center(9)+") : the texture must be a tga."

        correctTypesByMode = {"_mo_":["png", "tga"], "_rg_":["png", "tga"], "_px_":["tga", "png"], "_tx_rgb_":["tga"]}
    ...
    def check_valid_names(self,error_type):
        if displayPrints:
...
def cal_check_modeling_gdc(modelName, local):
...
def cal_check_setup_gdc(modelName, local):
...
def cal_check_shading_gdc(modelName, local, txStep=False):
...
class cal_scene_checker_gdc():
    def __init__(self, modelName, publish=False, override=False, local=False):
        ...
        if publish == True and local == True:
    ...
    def test_steps(self, local):
        ...
        if self.scenePathName != "unknown" and not "_presentation_" in self.scenePathName:
            ...
            if "_mo_" in self.scenePathName or "_mo." in self.scenePathName or "_layout_" in self.scenePathName or "_layout." in self.scenePathName:
                self.step = "_layout_"
                check = cal_check_modeling_gdc(self.modelName, local)
                ...
                self.step = "_anim_"
                check = cal_check_setup_gdc(self.modelName, local)
            elif "_tx_" in self.scenePathName or "_tx." in self.scenePathName:
                self.step = "_tx_"
                check = cal_check_shading_gdc(self.modelName, local, txStep=True)
            elif "_render_" in self.scenePathName or "_render." in self.scenePathName:
                self.step = "_render_"
                check = cal_check_shading_gdc(self.modelName, local)
            else:
    ...
    def publish_scene(self):
        ...
        publishPath = projectFolder + maFolder + self.modelName.lower() + self.step + "000.ma"

        print "Publishing scene", self.scenePathName, "to", publishPath

        if not os.path.isdir(projectFolder + maFolder):
            print "Creating path", projectFolder + maFolder
            os.makedirs(projectFolder + maFolder)
        ...
        if publishSucceeded:
            find = False;
            localTempFile = os.environ['TMP']+'/'+ self.modelName.lower() + self.step + "000.ma"
            fi = open(publishPath, "r")
            fo = open(localTempFile, "w")
            while True:
                oldLine = fi.readline()
                if not oldLine:
                    break
                newLine = re.compile(r'(setAttr \"\.ftn\" -type \"string\" \").*/([^/]+\")', re.IGNORECASE).sub(r'\g<1>%s\g<2>' % maFolder.replace('scenes/', 'sourceimages/'), oldLine)
                if oldLine != newLine:
                    find = True
                newLine = re.compile(r'\.map\"', re.IGNORECASE).sub('.png"', newLine)
                if self.step == "_tx_":
            ...
            fo.close()
            if find:
                shutil.copy2(localTempFile, publishPath)
        ...
        for f in files:
            if cmds.attributeQuery("fileTextureName", node=f, exists=True):
                ...
                copyPath = maFolder.replace('scenes/', 'sourceimages/') + os.path.basename(originalPath)
                if self.step == "_tx_":
                    copyPath = copyPath.replace('/publish/', '/work/')
                if copyPath:
                    if not os.path.exists(projectFolder + os.path.dirname(copyPath)):
                        os.makedirs(projectFolder + os.path.dirname(copyPath))
                    # zip by huangzhongwei@idmt.com.cn, 2013-05-07
                    zipPath = re.compile(r'\.map$', re.IGNORECASE).sub('.zip', copyPath)
                    if zipPath != copyPath:
                        pngSource = re.compile(r'\.map$', re.IGNORECASE).sub('.png', originalPath)
                        pngDest = re.compile(r'\.map$', re.IGNORECASE).sub('.png', projectFolder + copyPath)
                        print 'Copying', pngSource,  'to', pngDest
                        shutil.copy2(pngSource, pngDest)
                        print "Success"

                        print 'Ziping', originalPath,  'to', projectFolder + zipPath
                        z = zipfile.ZipFile(projectFolder + zipPath, "w", zipfile.ZIP_DEFLATED) 
                        z.write(originalPath, os.path.basename(originalPath))
                        z.close()
                        print "Success"
                    else:
                        print 'Copying', originalPath,  'to', projectFolder + copyPath
                        if os.path.exists(projectFolder + copyPath):
                            if self.override:
                                shutil.copy2(originalPath, projectFolder + copyPath)
                                print "Success"
                            else:
                                print "Error : a publication of this file already exists. If you want to publish, you need to set the override parameter to True"
                        else:
