...
class check_toolbox_gdc():
    ...
    def check_valid_nodes(self, error_type, nonValidTypes):
        ...
        for node in nodes:
            if cmds.referenceQuery(node, inr=1) == False and node.find("default") == -1 and node.find("Default") == -1 and node.find("initial") == -1:
                ...
                if type in nonValidTypes and not (type == "nucleus" and "pris_" in self.scenePathName.lower()):
        ...
        if len(filesNodeInfos):
            for path in filesNodeInfos:
                ...
                if path.find(" ") != -1 or pathOK != path or path.find(":") != -1:
                    ...
                    if cmds.getAttr(filesNodeInfos[path][0] + '.useFrameExtension'):
                        textureName = os.path.basename(path)
                        if not textureName.split('.')[-2].isdigit():
                            self.checkReport(error_type, 'Wrong name for textures attached to ' + filesNodeInfos[path][0] + ' file.', nodes=filesNodeInfos[path])
    ...
    def check_texture_format(self, error_type, maFolder=""):
        ...
        correctTypesByMode = {"_mo_": ["png", "tga"], "_rg_": ["png", "tga"], "_px_": ["tga", "png"], "_tx_rgb_": ["tga"], "_render_": ["map", "tga"]}
    ...
    def check_calimero_shading(self, scenePathName, modelType, error_type):
        ...
        for colorID in colorIDs:
            ...
            if not (colorR == 1.0 and colorG == 0.0 and colorB == 0.0) and not (colorR == 0.0 and colorG == 1.0 and colorB == 0.0) and not (colorR == 0.0 and colorG == 0.0 and colorB == 1.0) and not (colorR == 0.0 and colorG == 0.0 and colorB == 0.0) and not 'glass' in colorID.lower():
                message = colorID + " should be red, green, blue or black."
                        ...
                        if colorR == 0.0 and colorG == 1.0 and colorB == 0.0:
                            ...
                            if connectedEngines:
                                ...
                                for mesh in meshes:
                                    ...
                                    if parents :
                                        parent = parents[0]
                                        if parent.lower().count('vegetation') or parent.lower().count('planter'):
                                            break
                                else:
                                    message += colorID + ' have to be blue.\n'
