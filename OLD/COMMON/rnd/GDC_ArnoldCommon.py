...
#sys.path.append('E:/GIT/GDC_Repository/COMMON/pre')
#import GDC_ProjectInfo
#reload(GDC_ProjectInfo)
import GDC_ArnoldCommon
reload(GDC_ArnoldCommon)
class GDC_ArnoldCommon(object):
    def __init__(self):
        pass
...
#  Data   : 2014
#  update   : 2015-08
# 包括（AOV创建面板，常用工具，IDP工具）

   #----------------------------------------------------------------------------------------------------------#      #----------------------------------------------------------#
    def RenderArnoldUI(self):
    # 窗口
        if mc.window('GDCRenderArnold', exists=True):
            mc.deleteUI('GDCRenderArnold')
        mc.window('GDCRenderArnold', title=u'Arnold 渲染面板',
        ...
        mc.button(label=u'创建', bgc=[
                  0, 0, 0.0], width=110, height=30, command='GDC_ArnoldCommon.GDC_ArnoldCommon().ArnoldAOVCreat(AOVtype="AO")')
        mc.button(
            label=u'删除', bgc=[0, 0, 0.0], width=110, height=30, command='GDC_ArnoldCommon.GDC_ArnoldCommon().ArnoldAOVAODelete()')
        collectionnormal = mc.radioCollection()
        # normal
        normalset = mc.checkBox('hhchecknormal',label='Normal', visible=1, v=1,
                             bgc=[0.13, 0.15, 0.25], height=30, width=130)
        mc.button(label=u'创建', bgc=[
                  0, 0, 0.0], width=110, height=30, command='GDC_ArnoldCommon.GDC_ArnoldCommon().ArnoldAOVCreat(AOVtype="Normal")')
        mc.button(
            label=u'删除', bgc=[0, 0, 0.0], width=110, height=30, command='GDC_ArnoldCommon.GDC_ArnoldCommon().ArnoldAOVDelete(AOVtype="Normal")')
        collectionfre = mc.radioCollection()
        # fre
        freset = mc.checkBox('hhcheckfre',label='Fre', visible=1, v=1,
                             bgc=[0.13, 0.15, 0.25], height=30, width=130)
        mc.button(label=u'创建', bgc=[
                  0, 0, 0.0], width=110, height=30, command='GDC_ArnoldCommon.GDC_ArnoldCommon().ArnoldAOVCreat(AOVtype="Fre")')
        mc.button(
            label=u'删除', bgc=[0, 0, 0.0], width=110, height=30, command='GDC_ArnoldCommon.GDC_ArnoldCommon().ArnoldAOVDelete(AOVtype="Fre")')
...
#        mc.button(label=u'创建', bgc=[
#                  0, 0, 0.0], width=110, height=30, command='import GDC_ArnoldCommon\nreload(GDC_ArnoldCommon)\nGDC_ArnoldCommon.GDC_ArnoldCommon()..ArnoldAOVKeyLight()')
#        mc.button(
#            label=u'删除', bgc=[0, 0, 0.0], width=110, height=30, command='import GDC_ArnoldCommon\nreload(GDC_ArnoldCommon)\nGDC_ArnoldCommon.GDC_ArnoldCommon()..ArnoldAOVKeyDelete()')
#        collectionzdp = mc.radioCollection()
        #SSS
        SSSset = mc.checkBox('hhcheckSSS',label='SSS', visible=1,
                             v=1, bgc=[0.13, 0.15, 0.25], height=30, width=130)
        mc.button(label=u'创建', bgc=[
                  0, 0, 0.0], width=110, height=30, command='GDC_ArnoldCommon.GDC_ArnoldCommon().ArnoldAOVCreat(AOVtype="sss")')
        mc.button(
            label=u'删除', bgc=[0, 0, 0.0], width=110, height=30, command='GDC_ArnoldCommon.GDC_ArnoldCommon().ArnoldAOVZdepDelete(AOVtype="sss")')
        ...
        mc.button(label=u'创建', bgc=[
                  0, 0, 0.0], width=110, height=30, command='GDC_ArnoldCommon.GDC_ArnoldCommon().ArnoldAOVCreat(AOVtype="Shadow")')
        mc.button(
            label=u'删除', bgc=[0, 0, 0.0], width=110, height=30, command='GDC_ArnoldCommon.GDC_ArnoldCommon().ArnoldAOVDelete(AOVtype="Shadow")')
        collectionzdp = mc.radioCollection()
        # zdepth
        zdepthset = mc.checkBox('hhcheckzdep',label='Zdepth', visible=1, v=0,
                             bgc=[0.13, 0.15, 0.25], height=30, width=130)
        mc.button(label=u'创建', bgc=[
                  0, 0, 0.0], width=110, height=30, command='GDC_ArnoldCommon.GDC_ArnoldCommon().ArnoldAOVCreat(AOVtype="Zdp")')
        mc.button(
            label=u'删除', bgc=[0, 0, 0.0], width=110, height=30, command='GDC_ArnoldCommon.GDC_ArnoldCommon().ArnoldAOVDelete(AOVtype="Zdp")')
        # motionvector
        zdepthset = mc.checkBox('hhcheckmov',label='motionvector', visible=1, v=0,
                             bgc=[0.13, 0.15, 0.25], height=30, width=130)
        mc.button(label=u'创建', bgc=[
                  0, 0, 0.0], width=110, height=30, command='GDC_ArnoldCommon.GDC_ArnoldCommon().ArnoldAOVCreat(AOVtype="motionvector")')
        mc.button(
            label=u'删除', bgc=[0, 0, 0.0], width=110, height=30, command='GDC_ArnoldCommon.GDC_ArnoldCommon().ArnoldAOVZdepDelete(AOVtype="motionvector")')
        ...
        mc.button(label=u'创建渲染层', width=130, height=30,
                  bgc=[0.13, 0.15, 0.25], command='GDC_ArnoldCommon.GDC_ArnoldCommon().ArnoldRendererLayerCreat()')
        mc.button(label=u'删除所有AOV', width=110,
                  height=30, bgc=[0, 0, 0.0], command='GDC_ArnoldCommon.GDC_ArnoldCommon().ArnoldALLDelete(nodetype="aiAOV")')
        mc.button(label=u'删除所有渲染层', width=110,
                  height=30, bgc=[0, 0, 0.0], command='GDC_ArnoldCommon.GDC_ArnoldCommon().ArnoldALLDelete(nodetype="renderLayer")')
        mc.setParent('..')
        mc.setParent('..')        
        ...
        mc.button(label=u'角色ID创建（PS:需要在参考状态）', width=180, height=30,
                  bgc=[0.13, 0.15, 0.25], command='GDC_ArnoldCommon.GDC_ArnoldCommon().ProjectArnoldSetting(0)\nGDC_ArnoldCommon.GDCRenderArnold().csl_IDRenderLayerCreatAll(type="chr",ref=1)')
        mc.button(label=u'场景ID创建（PS:需要在参考状态）', width=180, height=30,
                  bgc=[0, 0, 0.0], command='GDC_ArnoldCommon.GDC_ArnoldCommon().ProjectArnoldSetting(0)\nGDC_ArnoldCommon.GDCRenderArnold().csl_IDRenderLayerCreatAll(type="set",ref=0)')
        ...
        mc.button(label=u'Arnold渲染设置', width=180, height=30,
                  bgc=[0.13, 0.6, 0.25], command='GDC_ArnoldCommon.GDC_ArnoldCommon().ProjectArnoldSetting(0)')
        mc.button(label=u'color', width=180, height=30,
                  bgc=[0, 0, 0.0], command='GDC_ArnoldCommon.GDC_ArnoldCommon().csl_ArnoldRenderLayerCreat(layername="color")')
        mc.button(label=u'motionblur', width=180, height=30,
                  bgc=[0.13, 0.15, 0.25], command='GDC_ArnoldCommon.GDC_ArnoldCommon().csl_ArnoldRenderLayerCreat(layername="motionblur")')
        mc.button(label=u'id', width=180, height=30,
                  bgc=[0, 0, 0.0], command='GDC_ArnoldCommon.GDC_ArnoldCommon().csl_ArnoldRenderLayerCreat(layername="id")')
        mc.button(label=u'随机id', width=180, height=30,
                  bgc=[0, 0, 0.0], command='GDC_ArnoldCommon.GDC_ArnoldCommon().csl_ArnoldRenderLayerCreat(layername="id")')
        mc.setParent('..')
        mc.setParent('..')
        ...
        mc.button(label=u'输出角色id01',width=90,height=30,bgc=[0.13, 0.15, 0.25],command='GDC_ArnoldCommon.GDC_ArnoldCommon().csl_RGBInfoExport(id=\'id01\')')
        mc.button(label=u'输出角色id02',width=90,height=30,bgc=[0.13, 0.15, 0.25],command='GDC_ArnoldCommon.GDC_ArnoldCommon().csl_RGBInfoExport(id=\'id02\')')
        mc.button(label=u'输出角色id03',width=90,height=30,bgc=[0.13, 0.15, 0.25],command='GDC_ArnoldCommon.GDC_ArnoldCommon().csl_RGBInfoExport(id=\'id03\')')
        mc.button(label=u'输出角色id04',width=90,height=30,bgc=[0.13, 0.15, 0.25],command='GDC_ArnoldCommon.GDC_ArnoldCommon().csl_RGBInfoExport(id=\'id04\')')
        mc.button(label=u'输出场景id11',width=90,height=30,bgc=[0.0, 0.0, 0.0],command='GDC_ArnoldCommon.GDC_ArnoldCommon().csl_RGBInfoExport(id=\'id11\')')
        mc.button(label=u'输出场景id12',width=90,height=30,bgc=[0.0, 0.0, 0.0],command='GDC_ArnoldCommon.GDC_ArnoldCommon().csl_RGBInfoExport(id=\'id12\')')
        mc.button(label=u'输出场景id13',width=90,height=30,bgc=[0.0, 0.0, 0.0],command='GDC_ArnoldCommon.GDC_ArnoldCommon().csl_RGBInfoExport(id=\'id13\')')
        mc.button(label=u'输出场景id14',width=90,height=30,bgc=[0.0, 0.0, 0.0],command='GDC_ArnoldCommon.GDC_ArnoldCommon().csl_RGBInfoExport(id=\'id14\')')
        mc.button(label=u'ID材质输出前检测',width=90,height=30,bgc=[0.1, 0.6, 0.25],command='GDC_ArnoldCommon.GDC_ArnoldCommon().csl_RGBInfoExportTest()')
        mc.setParent('..')
        mc.setParent('..')        
        #idp材质工具    
        mc.frameLayout(label=u'IDP材质工具', bgc=[0, 0, 0.0], borderStyle='in', cll=1,cl=1)
        mc.rowColumnLayout(numberOfColumns=4) 
        mc.button(label=u'R',width=90,height=30,bgc=[1,0,0],command='GDC_ArnoldCommon.GDC_ArnoldCommon().ProjectArnoldSetting(0)\nGDC_ArnoldCommon.GDCRenderArnold().ArnoldIDCreat(idpShader="ArnoldIdpR")')
        mc.button(label=u'G',width=90,height=30,bgc=[0,1,0],command='GDC_ArnoldCommon.GDC_ArnoldCommon().ProjectArnoldSetting(0)\nGDC_ArnoldCommon.GDCRenderArnold().ArnoldIDCreat(idpShader="ArnoldIdpG")')
        mc.button(label=u'B',width=90,height=30,bgc=[0,0,1],command='GDC_ArnoldCommon.GDC_ArnoldCommon().ProjectArnoldSetting(0)\nGDC_ArnoldCommon.GDCRenderArnold().ArnoldIDCreat(idpShader="ArnoldIdpB")')
        mc.button(label=u'M',width=90,height=30,bgc=[0,0,0],command='GDC_ArnoldCommon.GDC_ArnoldCommon().ProjectArnoldSetting(0)\nGDC_ArnoldCommon.GDCRenderArnold().ArnoldIDCreat(idpShader="ArnoldIdpM")')
        mc.button(label=u'Y',width=90,height=30,bgc=[1,1,0],command='GDC_ArnoldCommon.GDC_ArnoldCommon().ProjectArnoldSetting(0)\nGDC_ArnoldCommon.GDCRenderArnold().ArnoldIDCreat(idpShader="ArnoldIdpY")')
        mc.button(label=u'C',width=90,height=30,bgc=[0,1,1],command='GDC_ArnoldCommon.GDC_ArnoldCommon().ProjectArnoldSetting(0)\nGDC_ArnoldCommon.GDCRenderArnold().ArnoldIDCreat(idpShader="ArnoldIdpC")')
        mc.button(label=u'K',width=90,height=30,bgc=[1,0,1],command='GDC_ArnoldCommon.GDC_ArnoldCommon().ProjectArnoldSetting(0)\nGDC_ArnoldCommon.GDCRenderArnold().ArnoldIDCreat(idpShader="ArnoldIdpK")')
        mc.setParent('..')
        mc.setParent('..')        
        ...
        mc.rowColumnLayout(numberOfColumns=4) 
        mc.button(label=u'R',width=90,height=30,bgc=[1,0,0],command='GDC_ArnoldCommon.GDC_ArnoldCommon().P5_ArnoldDisMatCreat(idpShader="ArnoldIdpR01")')
        mc.button(label=u'G',width=90,height=30,bgc=[0,1,0],command='GDC_ArnoldCommon.GDC_ArnoldCommon().P5_ArnoldDisMatCreat(idpShader="ArnoldIdpG")')
        mc.button(label=u'B',width=90,height=30,bgc=[0,0,1],command='GDC_ArnoldCommon.GDC_ArnoldCommon().P5_ArnoldDisMatCreat(idpShader="ArnoldIdpB")')
        mc.button(label=u'M',width=90,height=30,bgc=[0,0,0],command='GDC_ArnoldCommon.GDC_ArnoldCommon().P5_ArnoldDisMatCreat(idpShader="ArnoldIdpM")')
        mc.button(label=u'Y',width=90,height=30,bgc=[1,1,0],command='GDC_ArnoldCommon.GDC_ArnoldCommon().P5_ArnoldDisMatCreat(idpShader="ArnoldIdpY")')
        mc.button(label=u'C',width=90,height=30,bgc=[0,1,1],command='GDC_ArnoldCommon.GDC_ArnoldCommon().P5_ArnoldDisMatCreat(idpShader="ArnoldIdpC")')
        mc.button(label=u'K',width=90,height=30,bgc=[1,0,1],command='GDC_ArnoldCommon.GDC_ArnoldCommon().P5_ArnoldDisMatCreat(idpShader="ArnoldIdpK")')
        mc.button(label=u'Lam',width=90,height=30,bgc=[0.5,0.5,0.5],command='GDC_ArnoldCommon.GDC_ArnoldCommon().P5_ArnoldDisMatCreat(idpShader="ArnoldLambert")')
        mc.button(label=u'Occ',width=90,height=30,bgc=[1,1,1],command='GDC_ArnoldCommon.GDC_ArnoldCommon().P5_ArnoldDisMatCreat(idpShader="ArnoldOcc")')
        mc.button(label=u'Normal',width=90,height=30,bgc=[0.4,0.55,0.9],command='GDC_ArnoldCommon.GDC_ArnoldCommon().P5_ArnoldDisMatCreat(idpShader="ArnoldNormal")')
        mc.setParent('..')        
        mc.tabLayout(tabs, edit=True, tabLabel=(
            (child1, 'Arnold AOV 及常用工具'), (child2, u'IDP工具')))
        mc.showWindow('GDCRenderArnold')    
    ...
    #LEV 是指渲染
   #----------------------------------------------------------------------------------------------------------#      #----------------------------------------------------------#
    def ProjectArnoldSetting(self,LEV=1):
        sys.path.append('E:/GIT/GDC_Repository/COMMON/pre')
        import GDC_ProjectInfo
        reload(GDC_ProjectInfo)
        shotinfo=GDC_ProjectInfo.GDC_ProjectInfo().checkShotInfo()
        if shotinfo[0] in ['csl'] and LEV==1:
            self.ArnoldRendererSettings('tif',1,0,5,0,0,0,3,3)
        if shotinfo[0] in ['csl'] and LEV==0:
            self.ArnoldRendererSettings('tif',1,0,3,0,0,0,0,3)
        return 0
    #----------------------------------------------------------#
    ...
    #  Author : 韩虹
    #  Data   : 2015
    # formatype,渲染格式；formset,压缩方式，如lzw；format，位数，如int8;AA,渲染精度
    def ArnoldRendererSettings(self,formatype='tif',formset=1,format=1,AA=3,dif=0,Glossy=0,Ref=0,sss=0,ray=3):
    ...
    # 下来所需的节点提前创建
        ...
        #设置渲染精度
        mc.setAttr('defaultArnoldRenderOptions.AASamples', int(AA))
        mc.setAttr('defaultArnoldRenderOptions.GIDiffuseSamples', int(dif))
        mc.setAttr('defaultArnoldRenderOptions.GIGlossySamples', int(Glossy))
        mc.setAttr('defaultArnoldRenderOptions.GIRefractionSamples', int(Ref))
        mc.setAttr('defaultArnoldRenderOptions.sssBssrdfSamples', int(sss))
        mc.setAttr('defaultArnoldRenderOptions.lock_sampling_noise', 1)
        mc.setAttr('defaultArnoldRenderOptions.GITotalDepth', int(ray))
        #格式
        #输出格式为tif,压缩模式，由formset确定（0,非压缩，1，lzw压缩）,format由format确定（0，8位；1，16位；2，32位）
        mc.setAttr('defaultArnoldDriver.aiTranslator',formatype,type='string')
        if formatype=='tif' :
            mc.setAttr('defaultArnoldDriver.tiffCompression',int(formset))
            mc.setAttr('defaultArnoldDriver.tiffFormat',int(format))
            mc.setAttr('defaultArnoldDriver.mergeAOVs', 0)
        #输出格式为exr,压缩模式，由formset确定
        if formatype=='exr' :
            mc.setAttr('defaultArnoldDriver.exrCompression',int(formset))
            #添加Exr合并
            mc.setAttr ('defaultArnoldDriver.mergeAOVs', 1)
        #输出格式为png,format由format确定（0,8位；1，16位）
        if formatype=='png':
            mc.setAttr('defaultArnoldDriver.pngFormat',int(format))
            mc.setAttr('defaultArnoldDriver.mergeAOVs', 0)
        return 0
    #----------------------------------------------------------#
    ...
    def csl_ArnoldRenderLayerCreat(self,layername='motionblur'):
        ...
        from idmt.maya.pycommon import GDC_ArnoldCommon
        reload(GDC_ArnoldCommon)
        self.ArnoldRendererSettings('04')
        mc.select(objselect)

      #创建渲染层
        GDC_RenderLayer.GDC_RenderLayer().createlayer(0,name=layername)
...
#        keypass=mc.checkBox('hhcheckkey',q=1,v=1)
        ...
        if occpass == True :
        	from idmt.maya.pycommon import GDC_ArnoldCommon
        	reload(GDC_ArnoldCommon)
        	GDCRenderArnoldLayer.GDCRenderArnold().ArnoldAOVCreat(AOVtype="AO")
        #normal
        if normalpass == True :
        	from idmt.maya.pycommon import GDC_ArnoldCommon
        	reload(GDC_ArnoldCommon)
        	GDCRenderArnoldLayer.GDCRenderArnold().ArnoldAOVCreat(AOVtype="Normal")
        #fre
        if frepass == True :
        	from idmt.maya.pycommon import GDC_ArnoldCommon
        	reload(GDC_ArnoldCommon)
        	GDCRenderArnoldLayer.GDCRenderArnold().ArnoldAOVCreat(AOVtype="Fre")
        #keyl
#        if keypass == True :
#        	from idmt.maya.pycommon import GDC_ArnoldCommon
#        	reload(GDC_ArnoldCommon)
#        	GDCRenderArnoldLayer.GDCRenderArnold().ArnoldAOVKeyLight()
        if SSSpass == True :
        	from idmt.maya.pycommon import GDC_ArnoldCommon
        	reload(GDC_ArnoldCommon)
        	GDCRenderArnoldLayer.GDCRenderArnold().ArnoldAOVCreat(AOVtype="SSS")
        if shadowpass == True :
        	from idmt.maya.pycommon import GDC_ArnoldCommon
        	reload(GDC_ArnoldCommon)
        	GDCRenderArnoldLayer.GDCRenderArnold().ArnoldAOVCreat(AOVtype="Shadow")
        #zdp
        if zdppass == True :
        	from idmt.maya.pycommon import GDC_ArnoldCommon
        	reload(GDC_ArnoldCommon)
        	GDCRenderArnoldLayer.GDCRenderArnold().ArnoldAOVCreat(AOVtype='Zdp') 
        #zdp
        if movpass == True :
        	from idmt.maya.pycommon import GDC_ArnoldCommon
        	reload(GDC_ArnoldCommon)
        	GDCRenderArnoldLayer.GDCRenderArnold().ArnoldAOVCreat(AOVtype='motionvector') 
...
#            mc.connectAttr((idpShader+'.outColor'), ( SG + '.surfaceShader'), f=True) 
    ...
    def ArnoldIDCreat(self,idpShader):
        ...
        #from idmt.maya.pycommon import GDC_ArnoldCommon
        #reload(GDC_ArnoldCommon)
        #GDCRenderArnoldLayer.GDCRenderArnold().ArnoldRendererSettings('02')
        if mc.objExists(idpShader)==0:
            mc.shadingNode('aiUtility', asShader=True,n=idpShader)
        idpSG=idpShader+'SG'
...
# 创建相应ID       
    ...
    def csl_RGBInfoImport(self,LayerName = 'id01', assetName = '' ,server = 0):
        # 路径
        if server:
            serverPath = self.checkLayerInfoServerPath('RGB', LayerName)
            filePath = serverPath + assetName + '\\' 
        else:
            localPath = self.checkLayerInfoLocalPath('RGB', LayerName)
        ...
        for fileName in alllFiles:
            if fileName[-4:] != '.txt':
                continue
            result.append(self.checkFileRead(filePath + fileName))
        return result
...
#删除id信息
    ...
    def ydRLayerRGBInfoExport(self, LayerName='FullBody', exportType='all', update = 0,openIt = False):
        ...
        self.checkFileWrite(filePath, outPutInfo)
        # update
        if update:
        # 本地及服务器端路径
            # 此处路径问题
            ...
            mel.eval(updateCMD)
            # 注册数据库
            projName = self.checkProjectNameSimple2Full(shotInfo[0])
    ...
    def checkProjectNameSimple2Full(self, simple):
        full = ''
        if simple in ['cl','do4','hf','zo','zm',u'do5']:
            if simple == 'zm':
