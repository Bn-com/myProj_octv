...
import re
import sys
sys.path.append('E:\\GIT\GDC_Repository')
class GDCRenderLayer(object):
    def __init__(self):
        pass
    #----------------------------------------------------------------------------------------------------------#      #----------------------------------------------------------#
# 【通用】【创建层面板】
...
#  Data   : 2015
    ...
    # 窗口
        ...
        b1=mc.textField('RenderLayerName',width=200,c='self.')
        t=mc.text(label='',height=10)
        b2=mc.button(label=u'建层',command='from idmt.maya.py_common import GDC_ArnoldCommon\nreload(GDC_ArnoldCommon)\nGDC_ArnoldCommon.GDCRenderArnold().P5_ArnoldDisMatCreat(idpShader="ArnoldIdpK")') )
