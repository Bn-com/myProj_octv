...
class sk_cacheFinalLayout_Base(object):
    def __init__(self):
        # namespace清理
        pass
    ...
    def checkFinalLayoutPerform(self , server = 1 , viewCheck = 0 , cachePre = -50 ):
        ...
        #---------------------------#
        unDisplayLayerObjs = sk_cacheFinalLayout.sk_cacheFinalLayout().sk_FL_RefHideObjsRecord(server)
        ...
        #---------------------------#
        # 处理大组
        noNeedRefNodeInfo = sk_referenceConfig.sk_referenceConfig().skFLNoNeedRefNodeInfo()
        ...
        # 输出需要的角色和道具参考信息
        if server:
            assetNeedOutputInfo = sk_referenceConfig.sk_referenceConfig().skFLAssetNeedInfo(refInfos,noNeedRefNodeInfo)
        ...
        if shotInfos[3] == 'an':
            ...
            sk_cacheFinalLayout.sk_cacheFinalLayout().checkAnimCurveInfoExport(animObjs, 1)
            ...
            # 强行备份材质
            if cacheObjs:
                MatLists = sk_cacheFinalLayout.sk_cacheFinalLayout().checkCacheRecordMaterial(cacheObjs,1)
            ...
            # 强行还原材质
            if cacheObjs:
                sk_cacheFinalLayout.sk_cacheFinalLayout().checkCacheReturnMaterial(MatLists)
