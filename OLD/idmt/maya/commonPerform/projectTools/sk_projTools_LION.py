...
class sk_projTools_LION(object):
    def __init__(self):
        self.errorSet = 'ErrorTemp_Set'

    # 切换参考
    def stoneReferenceSwitch(self):
        objs = mc.ls(sl=1,l=1)
        errorInfo = u'---请选取有效物体切换---'
        if not objs:
            print errorInfo
            mc.error()
        refList = [['c001001Nick','c001002NickStone']]
        refList +=[['c002001Chip','c002002ChipSton']]
        refList +=[['p007001NickPedestal','p007002NickPedestalStone']]
        refList +=[['p007003ChipPedestal','p007004ChipPedestalStone']]
        refPathList = []
        for obj in objs:
            refNode = mc.referenceQuery(obj,referenceNode = 1)
            refPath = mc.referenceQuery(refNode,filename = 1)
            if refNode in refPathList:
                continue
            refPathList.append(refPath)
        if not refPathList:
            print errorInfo
            mc.error()
        for refPath in refPathList:
            refNode = mc.referenceQuery(refPath,referenceNode=1)
            assetID = refPath.split('/')[-1].split('_')[1]
            anotherID = ''
            for refCheck in refList:
                if assetID not in refCheck:
                    continue
                idNum = refCheck.index(assetID)
                newNum = 0
                if idNum == 0:
                    newNum = 1
                if idNum == 1:
                    newNum = 0
                anotherID = refCheck[newNum]
            if not anotherID:
                continue
            newPath = refPath.replace(assetID,anotherID)
    ...
    def sk_sceneUICheckTools(self):
        ...
        mc.window("sk_sceneUICheckTools", title="Check Tools", widthHeight=(320, 380), menuBar=0)
        # 主界面
        mc.columnLayout()
        ...
        # 全自动
        mc.button(w=80 , h=350 , bgc=[0.1, 0.1, 0.1], label=(unicode('【全自动】\n【Check】', 'utf8')), c='sk_projTools_LION.sk_projTools_LION().checkDetailsWarning(UIShow = 1)')
        mc.columnLayout()
        # 分割按钮
        # 第1排
        mc.rowLayout(numberOfColumns=2, columnWidth2=(125, 125))
        mc.button(w=125 , h=30 , bgc=[0, 0.4, 0.1], label=(unicode('【Check】【参考】          ', 'utf8')), c='reload(sk_projTools_LION);sk_projTools_LION.sk_projTools_LION().checkDetailsWarning(\"refCheck\")')
        ...
        # 第2排
        mc.rowLayout(numberOfColumns=2, columnWidth2=(125, 125))
        mc.button(w=125 , h=30 , bgc=[0, 0.4, 0.1], label=(unicode('【Check】【namespace】', 'utf8')),c='reload(sk_projTools_LION);sk_projTools_LION.sk_projTools_LION().checkDetailsWarning(\"nsCheck\")')
        mc.button(w=125 , h=30 , bgc=[0, 0.4, 0.7], label=(unicode('<<namespace工具>>', 'utf8')),c = 'mel.eval(\"common_namespaceTools\")')
        mc.setParent("..")
        # 第3排
        mc.rowLayout(numberOfColumns=2, columnWidth2=(125, 125))
        mc.button(w=125 , h=30 , bgc=[0, 0.4, 0.1], label=(unicode('【Check】【命名】          ', 'utf8')),c='reload(sk_projTools_LION);sk_projTools_LION.sk_projTools_LION().checkDetailsWarning(\"MSHCheck\")')
        ...
        # 第4排
        mc.rowLayout(numberOfColumns=2, columnWidth2=(125, 125))
        mc.button(w=125 , h=30 , bgc=[0, 0.4, 0.1], label=(unicode('【Check】【面数】          ', 'utf8')),c='reload(sk_projTools_LION);sk_projTools_LION.sk_projTools_LION().checkDetailsWarning(\"faceCheck\")')
        ...
        # 第5排
        mc.rowLayout(numberOfColumns=2, columnWidth2=(125, 125))
        mc.button(w=125 , h=30 , bgc=[0, 0.4, 0.1], label=(unicode('【Check】【instance】    ', 'utf8')),c='reload(sk_projTools_LION);sk_projTools_LION.sk_projTools_LION().checkDetailsWarning(\"insCheck\")')
        ...
        # 第6排
        mc.rowLayout(numberOfColumns=2, columnWidth2=(125, 125))
        mc.button(w=125 , h=30 , bgc=[0, 0.4, 0.1], label=(unicode('【Check】【smooth】     ', 'utf8')),c='reload(sk_projTools_LION);sk_projTools_LION.sk_projTools_LION().checkDetailsWarning(\"smoothCheck\")')
        ...
        # 第7排
        mc.rowLayout(numberOfColumns=2, columnWidth2=(125, 125))
        mc.button(w=125 , h=30 , bgc=[0, 0.4, 0.1], label=(unicode('【Check】【标记】         ', 'utf8')),c='reload(sk_projTools_LION);sk_projTools_LION.sk_projTools_LION().checkDetailsWarning(\"signCheck\")')
        mc.button(w=125 , h=30 , bgc=[0, 0.4, 0.7], label=(unicode('<<自动清理空组>>', 'utf8')),c = 'mel.eval(\"deleteEmptyGroups()\")')
        mc.setParent("..")
        # 第8排
        mc.rowLayout(numberOfColumns=2, columnWidth2=(125, 125))
        mc.button(w=125 , h=30 , bgc=[0, 0.4, 0.1], label=(unicode('【Check】【物体重名】  ', 'utf8')),c='reload(sk_projTools_LION);sk_projTools_LION.sk_projTools_LION().checkDetailsWarning(\"sameTransformCheck\")')
        ...
        # 第9排
        mc.rowLayout(numberOfColumns=2, columnWidth2=(125, 125))
        mc.button(w=125 , h=30 , bgc=[0, 0.4, 0.1], label=(unicode('【Check】【shape重名】', 'utf8')),c='reload(sk_projTools_LION);sk_projTools_LION.sk_projTools_LION().checkDetailsWarning(\"sameShapeCheck\")')
        ...
        # 第10排
        mc.rowLayout(numberOfColumns=2, columnWidth2=(125, 125))
        mc.button(w=125 , h=30 , bgc=[0, 0.4, 0.1], label=(unicode('【Check】【Mesh同名】', 'utf8')),c='reload(sk_projTools_LION);sk_projTools_LION.sk_projTools_LION().checkDetailsWarning(\"sameShapeNodeCheck\")')
        mc.button(w=125 , h=30 , bgc=[0, 0.4, 0.1], label=(unicode('【Check】【proxy位移】', 'utf8')),c='reload(sk_projTools_LION);sk_projTools_LION.sk_projTools_LION().checkDetailsWarning(\"proxyInfo\")')
        mc.setParent("..")

        # 第11排
        mc.rowLayout(numberOfColumns=2, columnWidth2=(125, 125))
        mc.button(w=125 , h=30 , bgc=[0, 0.4, 0.3], label=(unicode('【Check】【smoothSet】', 'utf8')),c='reload(sk_projTools_LION);sk_projTools_LION.sk_projTools_LION().checkDetailsWarning("smoothSet")')
        mc.button(w=125 , h=30 , bgc=[0, 0.4, 0.3], label=(unicode('【Check】【renderState】', 'utf8')),c='reload(sk_projTools_LION);sk_projTools_LION.sk_projTools_LION().checkDetailsWarning("renderState")')
        mc.setParent("..")
    ...
    # 前期检测工具
    #--------------------------------------------------------#
    def checkDetailsWarning(self, checkType = '',UIShow = 0,printMode = 0,errorMode = 0):
        infoWrong = []
        errorPrint = 0
        ...
        if stepSimp in ['rg']:
            ...
            reload(sk_backCmd)
            sk_backCmd.sk_backCmd().checkRgGrpsRebuild('rg',1)
        ...
        if len(shotInfo) > 3:
            ...
            if not model:
                ...
                if len(model) > 1 and not setState:
                    ...
                    # MODEL组必须第二层级
                    if stepSimp in ['rg']:
                        if len(model[0].split('|')) !=4:
                            infoWrong.append(u'【 错 误 】\t\tMODEL组不在第三层级!')
                            errorPrint += 1
                    else:
            ...
            # 检测丢失材质的物体
            if checkType == '' or checkType == 'rgShader':
                # 对rg检测
                errorSet = 'Error_rgShader'
                if stepSimp in ['rg']:
                    errorNames = sk_checkTools.sk_checkTools().checkRgLostShader(refMode=1)
                    if errorNames:
                        for name in errorNames:
                            infoWrong.append(u'【rg丢材质】\t\t%s' % (str(name)))
                            errorPrint += 1
                        mc.sets(errorNames , e=1 , addElement=errorSet)
                    else:
                        mc.delete(errorSet)
    ...
    def checkShotDetails(self, backMode=1, returnMode=0 , printErrorMode = 1 ,preCheck = 0,anMode = 1):
        ...
        refErrors=[]
        if refInfo:
            for i in range(len(refInfo[0])):
                refPath=refInfo[1][0][i]
                if 'ms_anim' not in refPath:
                    refErrors.append(refPath)
        if refErrors:
            mc.warning(u'=====请检查以下非ms_anim参考=====')
            for ref in refErrors:
                print(u'======【%s】======='%ref)
            mc.error(u'=====参考错误，请检查以上非ms_anim参考=====')
    ...
    def animCheckTools(self, selectIndex=1):
        ...
        # 显示层检测
        mc.rowLayout()
        mc.button(w = widthValue , h = 30 ,bgc = [0,0.1,0.45],label = u'显示层检测  | Display Layer Check 'c = 'from commonCore.core_mayaCommon import sk_animFileCheck\nreload(sk_animFileCheck)\nsk_animFileCheck.sk_animFileCheck().checkDisplayLayers(norenderConfig = 0,donotKeys = [\'norender\',\'nodisplay\'])')
        mc.setParent("..")
        '''
