...
class sk_projTools_mtd(object):
    ...
    def sk_sceneUICheckTools(self):
        ...
        # 全自动
        mc.button(w=100 , h=350 , bgc=[0.1, 0.1, 0.1], label=(unicode('【全自动】【Check】', 'utf8')), c='from idmt.maya.commonPerform.projectTools import sk_projTools_mtd;reload(sk_projTools_mtd);sk_projTools_mtd.sk_projTools_mtd().checkDetailsWarning(UIShow= 1)')
        mc.columnLayout()
        # 分割按钮
        # 第1排
    ...
    def checkDetailsWarning(self, checkType = '',UIShow = 0,printMode = 1,errorMode = 0):
        ...
        if shotInfo[0] in ['mk']:
            ...
            reload(sk_projTools_mk)
            sk_projTools_mk.sk_projTools_mk().checkDetailsWarning(UIShow=1)
            return
        ...
        if 'cTest' in shotInfo[1]:
            return
        if 'pTest' in shotInfo[1]:
            return
        if 'sTest' in shotInfo[1]:
            return
    ...
    def checkShotDetails(self, backMode=1, returnMode=0 , printErrorMode = 1 ,preCheck = 0,printMode = 0,anMode = 1):
        ...
        stepFormat = sk_infoConfig.sk_infoConfig().checkProjectFileFormat(shotInfo[0])
        formatNow = '.%s'%(mc.file(exn=1, q=1).split('.')[-1])
        if formatNow != stepFormat:
            errorInfoList.append(u'[Error Format|文件名错误]This Step Format Should Be [%s]'%(stepFormat))

        print u'=====================File Format Check Done====================='
    ...
    def sk_projSmallTools(self,showDict = {'mo':0,'rg':0,'an':0,'fx':0,'lr':0}):
        ...
        for stepKey in stepKeys:
            ...
            if stepKey in ['lr']:
                ...
                mc.button(l=u'孔明灯加灯',w=130,bgc = [0.15,0.45,0.65],c = 'reload(sk_projTools_mtd);sk_projTools_mtd.sk_projTools_mtd().lanternsAddLights()')
                mc.button(l=u'星星加灯',w=130,bgc = [0.15,0.45,0.65],c = 'reload(sk_projTools_mtd);sk_projTools_mtd.sk_projTools_mtd().AddStarLights()')
                mc.setParent( '..' )
                # row2
                mc.rowLayout(numberOfColumns = 2,columnWidth2=(130 , 130))
    ...
    #mtd 馒头 额外处理
    def mtdExportExtra(self,server = 1):
        self.paperMatherExport(server)

    def mtdImportExtra(self,server = 1):
        self.paperMatherImport(server)

    def paperMatherExport(self,server):
        pmRefNode = mc.ls('mtd_c002001PaperMama*',type = 'reference')
        print '------mtdE001'
        if not pmRefNode:
            return
        print '------mtdE002'
        pmRefNode = pmRefNode[0]
        shotType = sk_infoConfig.sk_infoConfig().checkShotType()
        shotInfo = sk_infoConfig.sk_infoConfig().checkShotInfo()
        fileFomat = '.ma'
        ...
        print '------mtdE003'
        try:
            pmRefPath = mc.referenceQuery(pmRefNode,filename=True )
        except:
            return
        print '------mtdE005'
        mc.file(pmRefPath,importReference = 1, f = 1)
        # 整理
        STROKEGrp = mc.ls('mtd_c002001PaperMama*:STROKE',type = 'transform',l=1)[0]
        STROKEGrp = mc.parent(STROKEGrp,world = 1)
        DEFORMERSGrp = mc.ls('mtd_c002001PaperMama*:DEFORMERS',type = 'transform',l=1)[0]
        DEFORMERSGrp = mc.parent(DEFORMERSGrp,world = 1)
        RIGGrp = mc.ls('mtd_c002001PaperMama*:RIG',type = 'transform',l=1)[0]
        RIGGrp = mc.parent(RIGGrp,world = 1)
        for checkObj in mc.listRelatives(STROKEGrp,c=1,f=1,type = 'transform'):
            checkAttr = checkObj+'.v'
            cons = mc.listConnections(checkAttr,s=1,d=0,plugs=1)
            lockState = mc.getAttr(checkAttr,l=1)
            if cons or lockState:
                continue
            mc.setAttr(checkObj+'.v',1)
        MODELGrp = mc.ls('mtd_c002001PaperMama*:CHR',type = 'transform',l=1)[0]
        mc.delete(MODELGrp)
        print '------mtdE006'
        # 导出
        exportFileName = shotID + '_pm' + fileFomat
        localPath = sk_infoConfig.sk_infoConfig().checkCacheLocalPath(shotType)
        mc.select(STROKEGrp)
        mc.file((localPath + exportFileName), force=1, options="v=0", type=fileTypeFull, preserveReferences=1, exportSelected=1)
        print '------mtdE007'
        print '-----------PaperMatherExtraPath'
        if server:
            serverPath = sk_infoConfig.sk_infoConfig().checkCacheServerPath(shotType)
            import os
            if os.path.exists(serverPath+exportFileName):
                cmd = 'zwSysFile(\"del\",\"' + serverPath+exportFileName + '\",\"\",1)'
                mel.eval(cmd)
            cmd = 'zwSysFile("copy","' + (localPath+exportFileName) + '","' + (serverPath+exportFileName) + '",1)'
            mel.eval(cmd)
            print serverPath
        else:
            print localPath
    ...
    def paperMatherImport(self,server):
        shotType = sk_infoConfig.sk_infoConfig().checkShotType()
        shotInfo = sk_infoConfig.sk_infoConfig().checkShotInfo()
        fileFomat = '.ma'
        shotID = shotInfo[0] + '_' + shotInfo[1] + '_' + shotInfo[2]
        ...
        exportFileName = shotID + '_pm' + fileFomat
        if server:
            pathPre = sk_infoConfig.sk_infoConfig().checkCacheServerPath(shotType)
        else:
            pathPre = sk_infoConfig.sk_infoConfig().checkCacheLocalPath(shotType)
        needFile = pathPre + exportFileName
        import  os
        if os.path.exists(needFile):
            # paperM Root
            checkGrps = mc.listRelatives('CHR_GRP',c=1,type='transform')
            needRoot = ''
            for checkGrp in checkGrps:
                if 'mtd_c002001PaperMama' in checkGrp:
                    needRoot = checkGrp
                if needRoot:
                    break
            mc.file(needFile,i=1)
            STROKEGrp = mc.ls('mtd_c002001PaperMama*:STROKE',type = 'transform',l=1)[0]
            STROKEGrp = mc.parent(STROKEGrp, needRoot)
            DEFORMERSGrp = mc.ls('mtd_c002001PaperMama*:DEFORMERS',type = 'transform',l=1)[0]
            DEFORMERSGrp = mc.parent(DEFORMERSGrp, needRoot)
            RIGGrp = mc.ls('mtd_c002001PaperMama*:RIG',type = 'transform',l=1)[0]
            RIGGrp = mc.parent(RIGGrp, needRoot)

    #-------------------------------#
    # 灯笼加灯
    def lanternsAddLights(self):
        # 获取1
        checkGrps = mc.ls('mtd_p001001*:*',type = 'transform',l=1) + mc.ls('mtd_p001002*:*',type = 'transform',l=1)
        attrKey = '.deng'
        consPre = 'foodCons_'
        needObjs = []
        for checkGrp in checkGrps:
            if not mc.ls(checkGrp+attrKey):
                continue
        ...
        if not needObjs:
            return
        # 准备
        oldConsNodes = mc.ls(type = 'pointOnPolyConstraint')
        if oldConsNodes:
            for checkNode in oldConsNodes:
                inr = mc.referenceQuery(checkNode,inr = 1)
                if not inr:
                    mc.delete(oldConsNodes)
        lightRoot = 'lanternLight_Grp'
        if mc.ls(lightRoot,type = 'transform'):
            mc.delete(lightRoot)
        mc.group(n=lightRoot,em=1)
        # 处理每个组
        pLClr = [0.696,0.880,0]
        pLPos = [-0.252,-9.503,-0.411]
    ...
    #-------------------------------#
    # 星星加灯
    def AddStarLights(self):
        checkGrps = mc.listRelatives('mtd_demo_0150_xing_ef_c001:xingFX',c=1)
        #attrKey = '.deng'
        #consPre = 'foodCons_'
        needObjs = []
        for checkGrp in checkGrps:
