...
class sk_projTools_yzc(object):
    ...
    def sk_sceneUICheckTools(self):
        ...
        # 全自动
        mc.button(w=100 , h=350 , bgc=[0.1, 0.1, 0.1], label=(unicode('【全自动】【Check】', 'utf8')), c='from idmt.maya.commonPerform.projectTools import sk_projTools_yzc;reload(sk_projTools_yzc);sk_projTools_yzc.sk_projTools_yzc().checkDetailsWarning(UIShow= 1)')
        mc.columnLayout()
        # 分割按钮
        # 第1排
    ...
    def sk_projSmallTools(self,showDict = {'mo':0,'rg':0,'an':0,'fx':0,'lr':0}):
        ...
        for stepKey in stepKeys:
            ...
            projBaseKey = 'from idmt.maya.commonPerform.projectTools import sk_projTools_yzc;reload(sk_projTools_yzc);sk_projTools_yzc.sk_projTools_yzc()'
            renderBaseKey = 'from idmt.maya.commonPerform.renderLayers import sk_renderLayer_yzc;reload(sk_renderLayer_yzc);sk_renderLayer_yzc.sk_renderLayer_yzc()'
            if stepKey in ['mo']:
                rowNum = 4
                ...
                mc.button(l=u'动态海草选取',w=84,bgc = [0.15,0.55,0.65],h=bH,c = 'reload(sk_projTools_yzc);sk_projTools_yzc.sk_projTools_yzc().animatedPlaneSelect()')
    ...
    # test ref fix
    def testPathFix(self):
        shotInfos = sk_infoConfig.sk_infoConfig().checkShotInfo()
        needState = 0
        if shotInfos[0].lower() in ['yzc'] and shotInfos[1].lower() in ['test']:
            needState = 1
        if not needState:
            return
        ...
        for checkRef in refNodes:
            ...
            if not refPath:
                continue
            newPath = refPath
            oldList = ['//file-cluster/GDC','Projects/yongzhicheng/Daily_N/Test']
            newList = ['L:','Projects/YongZhiCheng/Project/scenes/Animation/Test']
            for num in range(len(oldList)):
                oldKey = oldList[num]
                newKey = newList[num]
                newPath = newPath.replace(oldKey,newKey)
            if not os.path.exists(newPath):
                continue
            if refPath != newPath:
                mc.file(newPath, loadReference = checkRef)
        print('--------Ref Fix Done--------')