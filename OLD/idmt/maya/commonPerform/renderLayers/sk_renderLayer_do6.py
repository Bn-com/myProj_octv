...
class sk_renderLayer_do6(object):
    ...
    def chrEnvCreateState(self):
        ...
        setsNsList = ['s606','s613','s614','s634',
                      's617','s625','s626','s627']
        needState = 0
        for checkNs in nsList:
            ...
            if len(checkKey) <= 7:
                continue
            if checkKey[:4] in setsNsList:
                needState = 1
        return needState

    #--------------#
    # 是否使用chrenv
    def greenEyesCreateState(self):
        nsList = mc.namespaceInfo(listOnlyNamespaces=1)
        nsList.remove('UI')
        nsList.remove('shared')
        setsNsList = ['c012','c034','c035']
        needState = 0
        for checkNs in nsList:
            if '_' not in checkNs:
                continue
    ...
    def rlCHR_HOLO(self,printMode = 0):
       # 物体
        ...
        from idmt.maya.commonCore.core_renderLayer import sk_renderLayerCore
        reload(sk_renderLayerCore)
        sk_renderLayerCore.sk_renderLayerCore().masterLayerLambertShader(shaderType='lambert',skipSGList = skipSGList)
        # Screen
        layerName = 'Screen'
    ...
    #---------------#
    # 屏幕外挂处理
    def rlCHR_screenAddTools(self,printMode = 0):
        if printMode:print '\n' + time.strftime('%Y-%m-%d %H:%M:%S') + '\n'
        layerName = 'screenAdd'
        # 素模
        from idmt.maya.commonCore.core_renderLayer import sk_renderLayerCore
        ...
        chrMeshObjs = objDict['chrMeshObjs']
        prpMeshObjs = objDict['prpMeshObjs']
        for checkObj in (chrMeshObjs+prpMeshObjs):
            checkMesh = mc.listRelatives(checkObj,s=1,type='mesh',f=1,ni=1)
            if not checkMesh:
                continue
            mc.setAttr(checkMesh[0] + '.aiMatte',1)
        #-----总表信息
        import xlrd
        screenRoot = 'L:/Projects/DiveollyDive6/Project/data/lighting/Screen'
        screenExcel = '%s/screenData.xls'%screenRoot
        screenData = xlrd.open_workbook(screenExcel)
        screenInfos = screenData.sheets()[0]
        shotDict = {}
        for num in range(len(screenInfos.col_values(0))):
            if num in [0]:
                continue
            sc = screenInfos.col_values(0)[num]
            if not sc:
                continue
            sh = screenInfos.col_values(1)[num]
            if not sh:
                continue
            shotID = '%s_%s'%(sc,sh)
            shotDict[shotID] = screenInfos.col_values(3)[num]
        # 导入需要的文件
        shotInfos = sk_infoConfig.sk_infoConfig().checkShotInfo()
        shotIDNow = '%s_%s'%(shotInfos[1],shotInfos[2])
        if shotIDNow not in shotDict.keys():
            print '\n------[Error]\n------[%s] Not in Screen.xls \n'%shotIDNow
            self.emptyLayer = layerName
            return
        needScreenType = shotDict[shotIDNow]
        screenFile = '%s/%s.mb'%(screenRoot,needScreenType)
        print '------screenFile'
        print screenFile
        mc.file(screenFile,i=1,namespace = 'foodScreen')
        # 环境变量化路径
        from idmt.maya.commonCore.core_mayaCommon import sk_sceneTools
        reload(sk_sceneTools)
        sk_sceneTools.sk_sceneTools().sk_setPathEnv(justType=['aiImage','file'])
        # 物体
        screenGrp = mc.ls('foodScreen*:MODEL',type = 'transform')
        screenMaster = mc.ls('foodScreen*:Master',type = 'transform')[0]
        if 's0_' in needScreenType:
            screenMovCtl = mc.ls('foodScreen*:Move_ctrl_grp',type = 'transform')[0]
        else:
            screenMovCtl = mc.ls('foodScreen*:Move_Ctrl',type = 'transform')[0]
        if 's0_' in needScreenType:
            screenChrCtl = mc.ls('foodScreen*:Move_ctrl',type = 'transform')[0]
        else:
            screenChrCtl = mc.ls('foodScreen*:Character',type = 'transform')[0]
        # 匹配
        checkObjs = mc.ls('*_c602*Bag*:MSH_c_hi_screen_3_',type='transform',l=1)   \
                 + mc.ls('*_p028*Bag*:MSH_c_hi_screen_3_',type='transform',l=1)
        screenObjs = []
        for checkObj in checkObjs:
            if '|OTC_GRP' in checkObj:
                continue
            screenObjs.append(checkObj)
        if not screenObjs:
            print '------No Screen Objs!!!'
            self.emptyLayer = layerName
            return
        screenObj = screenObjs[0]
        # 修正
        fixSz = 0
        if needScreenType in ['s2','s3','s4','s5']:
            mc.setAttr(screenMovCtl+'.ty',-0.06)
            mc.setAttr(screenMovCtl+'.tz',-3.63)
            mc.setAttr(screenMovCtl+'.ry',180.000)
            mc.setAttr(screenMovCtl+'.rz',180.000)
            mc.xform(screenMovCtl,rp = [0,0,-4.1500])
            mc.xform(screenMovCtl,sp = [0,0,-4.1500])
            scaleValue = 1
            fixSz = 1
        else:
            mc.setAttr(screenMovCtl+'.ty',-0.06)
            mc.setAttr(screenMovCtl+'.tz',-3.152)
            mc.setAttr(screenMovCtl+'.ry',180.000)
            mc.setAttr(screenMovCtl+'.rz',180.000)
            mc.xform(screenMovCtl,rp = [0,0,-3.6900])
            mc.xform(screenMovCtl,sp = [0,0,-3.6900])
            scaleValue = 1.04
        # 约束
        screenP = screenObj + '.vtx[224]'
        movLoc = mc.spaceLocator(n = 'screenMovLoc')[0]
        mc.select(screenP)
        mc.select(movLoc,add = 1)
        mel.eval('doCreatePointOnPolyConstraintArgList 2 {   "0" ,"0" ,"0" ,"1" ,"" ,"1" ,"0" ,"0" ,"0" ,"0" };')
        mc.parentConstraint(movLoc,screenMaster)
        # 锁
        for checkAttr in ['.tx','.ty','.tz','.rx','.ry','.rz','.sx','.sy','.sz']:
            fixAttr = screenChrCtl + checkAttr
            if checkAttr in ['.sx','.sy','.sz']:
                cons = mc.listConnections(fixAttr,s=1,d=0,plugs=1)
                if cons:
                    mc.disconnectAttr(cons[0],fixAttr)
                mc.setAttr(fixAttr,l=0)
                mc.setAttr(fixAttr,scaleValue)
            if checkAttr not in ['.tz','.sx','.sy','.sz']:
                mc.setAttr(screenMovCtl + checkAttr,l=1)
            mc.setAttr(fixAttr,l=1)
            mc.setAttr(screenMaster + checkAttr,l=1)
        mc.connectAttr(screenMovCtl+'.sx',screenMovCtl+'.sy')
        mc.connectAttr(screenMovCtl+'.v',screenGrp[0]+'.v')
        # 特殊处理
        if fixSz:
            mc.expression(s='%s.sz = 0.942 * %s.sx'%(screenMovCtl,screenMovCtl))
        else:
            mc.connectAttr(screenMovCtl+'.sx',screenMovCtl+'.sz')
        mc.setAttr(screenObj+'.v',0)
        # 初始化
        self.baseSettings(format = 'png')
        # 灯光
        lightFile = '%s/baseL.mb'%screenRoot
        mc.file(lightFile,i=1,namespace = 'screenL')
        lightGrp = mc.ls('*:baseScreenLight')
        # 分层
        rlObjs = chrMeshObjs + prpMeshObjs + screenGrp + lightGrp + [screenMovCtl]
        if not rlObjs:
            print (u'===============!!!No Objects 【RL】【%s】!!!===============' % (u'%s_层' % layerName))
            self.emptyLayer = layerName
            return
        ...
        attrDict = {'.AASamples':4,'.GIDiffuseSamples':0,'.GIGlossySamples':0,
                    '.GIRefractionSamples':2,'.GISssSamples':0,'.GIVolumeSamples':0,
                    '.GIDiffuseDepth':2,'.GIGlossyDepth':0,'.GIReflectionDepth':2,'.GIRefractionDepth':0}
        for checkAttr in attrDict.keys():
            nodeAttr = baseNode + checkAttr
            mc.editRenderLayerAdjustment(nodeAttr)
            mc.setAttr(nodeAttr,attrDict[checkAttr])
    ...
    #----------------#
    # green eyes
    def rlGreenEyes(self,printMode = 0):
        if printMode:print '\n' + time.strftime('%Y-%m-%d %H:%M:%S') + '\n'
        layerName = 'greenEyes'
        # 初始化
        self.baseSettings()
        ...
        eyeObjs = []
        for num in [2,3,4,5]:
            tempObjs = mc.ls('*_c012*:MSH_l_eye%s_'%str(num),l=1) + mc.ls('*_c012*:MSH_r_eye%s_'%str(num),l=1) \
                    + mc.ls('*_c034*:MSH_l_eye%s_'%str(num),l=1) + mc.ls('*_c034*:MSH_r_eye%s_'%str(num),l=1) \
                    + mc.ls('*_c035*:MSH_l_eye%s_'%str(num),l=1) + mc.ls('*_c035*:MSH_r_eye%s_'%str(num),l=1) \
                    + mc.ls('*_c036*:MSH_l_eye%s_'%str(num),l=1) + mc.ls('*_c036*:MSH_r_eye%s_'%str(num),l=1)
            eyeObjs += tempObjs
        matteObjs = mc.ls('*_c012*:MSH_c_hi_body6_',l=1) + mc.ls('*_c012*:MSH_c_hi_body7_',l=1) \
                + mc.ls('*_c034*:MSH_c_hi_body6_',l=1) + mc.ls('*_c034*:MSH_c_hi_body7_',l=1) \
                + mc.ls('*_c035*:MSH_c_hi_body6_',l=1) + mc.ls('*_c035*:MSH_c_hi_body7_',l=1) \
                + mc.ls('*_c036*:MSH_c_hi_body6_',l=1) + mc.ls('*_c036*:MSH_c_hi_body7_',l=1)
        # 灯光
        screenRoot = 'L:/Projects/DiveollyDive6/Project/data/lighting/Screen'
        lightFile = '%s/baseL.mb'%screenRoot
        mc.file(lightFile,i=1,namespace = 'screenL')
        lightGrp = mc.ls('*:baseScreenLight')
        # 分层
        rlObjs = eyeObjs + matteObjs + lightGrp
        if not eyeObjs or not (eyeObjs + matteObjs ):
            print (u'===============!!!No Objects 【RL】【%s】!!!===============' % (u'%s_层' % layerName))
            self.emptyLayer = layerName
            return
        if mc.ls(layerName):
        ...
        mc.createRenderLayer(rlObjs , name=layerName , noRecurse=1 , makeCurrent=1)
        # matte
        for checkObj in matteObjs:
            checkMesh = mc.listRelatives(checkObj,s=1,ni=1,type='mesh',f=1)
            mc.setAttr(checkMesh[0]+'.aiMatte',1)
        # sample values
        baseNode = 'defaultArnoldRenderOptions'
        attrDict = {'.AASamples':4,'.GIDiffuseSamples':2,'.GIGlossySamples':0,
                    '.GIRefractionSamples':2,'.GISssSamples':0,'.GIVolumeSamples':0,
                    '.GIDiffuseDepth':1,'.GIGlossyDepth':1,'.GIReflectionDepth':2,'.GIRefractionDepth':2}
        for checkAttr in attrDict.keys():
            nodeAttr = baseNode + checkAttr
            mc.editRenderLayerAdjustment(nodeAttr)
            mc.setAttr(nodeAttr,attrDict[checkAttr])
