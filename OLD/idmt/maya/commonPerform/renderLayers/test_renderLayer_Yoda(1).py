...
class sk_renderLayer_Yoda(object):
    ...
    def ydRLSpeficCreate(self, renderLayer, saveMode='', selectMode=0, arnoldType=0 ,createMode = 1 , ui = 0):
        ...
        if layerType in ['BSR','FBR','SPR']:
            if mc.ls(BGType + '_' + layerType):
                mc.delete(BGType + '_' + layerType)
            self.ydRLRGBCreate(BGType, selectObjType=selectMode)
    ...
    def mentalRayProductionLevel(self,createMode = 1):
        ...
        mc.setAttr('miDefaultOptions.sampleLock', 1)
    ...
    # ZIP code
    #----------------------------------------------------------#
    def ydRLFramebuffer(self, fileType = 'iff',datatype=5 ,notmaster = 1):    
        mc.setAttr('defaultRenderGlobals.animation', 1)
        mc.setAttr('defaultRenderGlobals.putFrameBeforeExt', 1)
        mc.setAttr('defaultRenderGlobals.periodInExt', 1)
        mc.setAttr('defaultRenderGlobals.extensionPadding', 4)
    ...
    def ydRLTransShaderConnection(self, transpancyNode, transShader, transShaderAttr):
        if transpancyNode[:6] == '[food]':
            ...
            if mc.nodeType(transpancyNode.split('.')[0]) in['layeredShader','layeredTexture', 'surfaceShader', 'ramp', 'file', 'reverse','check','bulge']:
                ...
                if mc.nodeType(transpancyNode.split('.')[0]) in ['ramp', 'file' ,'bulge','checker']:
                    try:
    ...
    def ydRLSave(self, mode , shotType = 3 , RGB = ''):
        ...
        if shotType == 2:
            ...
            camName = 'CAM:*_' + shotInfo[1] + '_' + shotInfo[2] + '_baked'
        if shotType == 3:
    ...
    def ydRLFaceObjs(self):
        ...
        planeObjs = mc.ls('*:*glasses_*',type = 'transform',l = 1) + planeObjs
    ...
    #-------------------------------------------------#
    # 核心创建层函数
    def ydRLRGBCreate(self, layerType , shaderForece = 0):
