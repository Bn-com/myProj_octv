...
class sk_renderLayer_Yo(object):
    def __init__(self):
        pass
...
#                pass                                   
        ...
        version = mel.eval("about -apiVersion")/100
        if int(version)>2014:
            self.maya2016MRSeting()
        else:
            ...
            mc.setAttr('miDefaultOptions.miRenderUsing', 2)
#maya2016 渲染设置
    def maya2016MRSeting(self):
        mc.setAttr("miDefaultOptions.miRenderUsing",2)
        mc.setAttr ("miDefaultOptions.maxSamples",2)
        mc.setAttr ("miDefaultOptions.contrastA",0.1)
        mc.setAttr ("miDefaultOptions.lightImportanceSamplingQuality",1)
        mc.setAttr ("miDefaultOptions.filter",2)
        mc.setAttr ("miDefaultOptions.giGpuDepth",3)
        mc.setAttr ("miDefaultOptions.maxReflectionRays",1)
        mc.setAttr ("miDefaultOptions.jitter",1)
        mc.setAttr ("mentalrayGlobals.imageFormat",7)
        mc.setAttr ("mentalrayGlobals.imfkey",type="iff")
    ...
    def ydRLObjsInfo(self,layerType,renderLayer):
        ...
        if MaskObjs and renderLayer not in ['BSR','FBR','SPR']:
            for obj in MaskObjs:
                if mc.objExists(obj):
                    objMask.append(obj)
        
        if MaskObjs and renderLayer in ['BSR','FBR','SPR']:
            for ob in MaskObjs:
                obn=[]
                if ob:
                    for obj in ob:
        ...
        if tempMeshes:
            for mesh in tempMeshes:
                ...
                if '.f[' not in mesh:
                    ...
                    obj = mesh.split('.f[')[0]                 
                if outObjs:
                    if mc.ls(obj) and obj not in outObjs:
                        checkMesh = mesh
                else:
                    if obj in inObjs :
                        checkMesh = mesh
    ...
    def ydRLSave(self, mode , shotType = 3 , RGB = ''):
        ...
        # 处理非参考同名相机
        if mc.ls(camName) and mode!='MV':
            # 检测是否参考
            camName=mc.ls(camName,l=1)[0]
        ...
        if mode=='MV':
            ...
            if cam:
                for ca in cam:
                    if 'persp' not in ca and 'side' not in ca and 'top' not in ca and 'front' not in ca :
                        try:
                            mc.delete(ca)
                        except:
                            pass  
...
# typeMode 2--do_xxx_xxx  |  3--do_xxx_xxx_xxx
    ...
    def ydRLGBowObjs(self):
        ...
        bowObjs = []
        for obj in allObjs:
            keyWords = ['drape','brow','brow_r_','brow_l_','browr_','browl_','eyepatch_','eyeborw']
            for key in keyWords:
    ...
    def ydRLCOCreate(self, layerType, selectObjType=0 ,createMode = 1,createName = ''):
        ...
        if layerObjs:
            ...
            objsN=[]
            objsO=rlObjs + rlLights
            for obj in objsO:
                if mc.objExists(obj):
                    objsN.append(obj)            
            mc.createRenderLayer(objsN, name=layerName, noRecurse=1, makeCurrent=1)
                        ...
                        try:
                            ...
                            for mesh in meshes:
                                try:
                                    ...
                                    print(mesh)
                                    mc.error(u'===有物体无法赋予材质===')
                    targetSGNode = transpancySGNodes[i]
                    # SG连接着色
                    if SGOType:
                        shaderInfo = mc.listConnections((targetSGNode + '.surfaceShader'),s = 1,plugs = 1)
                        if shaderInfo:
                            shaderInfo = shaderInfo[0]
                            mc.editRenderLayerAdjustment(targetSGNode + '.surfaceShader')
                            mc.disconnectAttr(shaderInfo, (targetSGNode + '.surfaceShader'))
                            mc.connectAttr((shaderTrsNode + '.outColor'), (targetSGNode + '.surfaceShader'), f=1)
