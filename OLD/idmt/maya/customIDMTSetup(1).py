...
def BeforeCreateReference(retCode, file, clientData):
    ...
    try:
        ...
        if 0 == 1:
            pass
        #elif re.compile(r'[/\\](DiveollyDive4|VickyTheViking)[/\\].*\.ma$', re.IGNORECASE).search(pathOld) != None:
        ...
        #        print "%s -> %s" % (pathOld, pathNew)
        #        file.setRawFullName(pathNew)
        elif re.compile(r"^[/\\][/\\]file-cluster[/\\]GDC([/\\]SW_S3_Pipeline[/\\])", re.IGNORECASE).search(pathOld) != None:
            pathNew = re.compile(r"^[/\\][/\\]file-cluster[/\\]GDC([/\\]SW_S3_Pipeline[/\\])", re.IGNORECASE).sub(r"Z:\g<1>", pathOld)
            print "%s -> %s" % (pathOld, pathNew)
            file.setRawFullName(pathNew)
            ...
            if 0 == 1:
                pass
            #elif re.compile(r'[/\\]HeroFactory[/\\]', re.IGNORECASE).search(pathOld) != None:
...
def customIDMTSetup():
    ...
    if maya.cmds.about(apiVersion = True) >= 201700:
        maya.cmds.optionVar(intValue = ('FileDialogStyle', 1))
