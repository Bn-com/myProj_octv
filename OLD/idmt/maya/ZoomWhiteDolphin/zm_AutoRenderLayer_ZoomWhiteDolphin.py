...
class zmRLConfig(object):
    ...
    def ReadEXcle(self):
        ...
        shotInfo = sk_infoConfig.sk_infoConfig().checkShotInfo()  # 读文件名，获取项目及镜头号

        projFullName = 'ZoomWhiteDolphin'

        serverPath = '//file-cluster/GDC/Projects/' + projFullName + '/Reference/Product manager/Render ref/BKL_RN/'
        serverExcelPath = serverPath + 'ZM' + str(shotInfo[1]) + '_BKL_RN.xls'
        ...
        import xlrd
        reload(xlrd)
        shotAllData = xlrd.open_workbook(serverExcelPath).sheets()[0]  # shotAllData = xlrd.open_workbook(serverExcelPath).sheets()[0] 0是第一页表格
        ...
        for i in range(len(shotData)):
            ...
            if str(shotData[i]).split('.')[0]:
                shotData[i] = str(shotData[i]).split('.')[0]

            while(shotData[i][-1] in [' ', ';']):
                shotData[i] = shotData[i][:-1]
        return shotData
        # shotData = shotAllData.row_values(int(shotInfo[1]) + 1)  # 读行数，具体的是镜头号加多少，视表格内容定

    # 自动分层
    def zmRLCOautoCreate(self, path):
        print ('=================================================================')
        print '====================!!!Start AutoRenderLayer!!!===================='

        mc.file(path, force=1, loadReferenceDepth="all", open=1)
        # Step 1：背景与遮罩层
        self.zmRLSpeficCreate("FG_CO")
        self.zmRLSpeficCreate("BG_CO")
        # save
        from idmt.maya.py_common import sk_infoConfig
        reload(sk_infoConfig)
    ...
    def zmRLCAUSTICautoCreate(self, path):
        print ('=================================================================')
        print '====================!!!Start AutoRenderLayer!!!===================='

        mc.file(path, force=1, loadReferenceDepth="all", open=1)
        # Step 1：背景与遮罩层
        self.zmRLSpeficCreate("BG_CAUSTICS")
        self.zmRLSpeficCreate("CHR_CAUSTICS")
        # save
        from idmt.maya.py_common import sk_infoConfig
        reload(sk_infoConfig)
        shotInfo = sk_infoConfig.sk_infoConfig().checkShotInfo()
        pathLocal = sk_infoConfig.sk_infoConfig().checkRenderLayerLocalPath()
        fileName = pathLocal + shotInfo[0] + '_' + shotInfo[1] + '_' + shotInfo[2]
        fileType = '_l2caustics_lr_c001.mb'
        fileName = fileName + fileType
        mc.file(rename=fileName)
        mc.file(save=1)
    ...
    def zmRLALLZDPautoCreate(self, path):
        print ('=================================================================')
        print '====================!!!Start AutoRenderLayer!!!===================='

        mc.file(path, force=1, loadReferenceDepth="all", open=1)
        # Step 1：背景与遮罩层
        self.zmRLSpeficCreate("ALL_ZDEPTH")
        # save
        from idmt.maya.py_common import sk_infoConfig
        reload(sk_infoConfig)
        shotInfo = sk_infoConfig.sk_infoConfig().checkShotInfo()
    ...
    def zmRLBGZDPautoCreate(self, path):
        print ('=================================================================')
        print '====================!!!Start AutoRenderLayer!!!===================='

        mc.file(path, force=1, loadReferenceDepth="all", open=1)
        # Step 1：背景与遮罩层
        self.zmRLSpeficCreate("BG_ZDEPTH")
        # save
        from idmt.maya.py_common import sk_infoConfig
        reload(sk_infoConfig)
        shotInfo = sk_infoConfig.sk_infoConfig().checkShotInfo()
    ...
    def zmRLALLMask01autoCreate(self, path):
        print ('=================================================================')
        print '====================!!!Start AutoRenderLayer!!!===================='

        mc.file(path, force=1, loadReferenceDepth="all", open=1)
        # Step 1：背景与遮罩层
        self.zmRLSpeficCreate("ALL_MASK01")
        # save
        from idmt.maya.py_common import sk_infoConfig
        reload(sk_infoConfig)
        shotInfo = sk_infoConfig.sk_infoConfig().checkShotInfo()
        pathLocal = sk_infoConfig.sk_infoConfig().checkRenderLayerLocalPath()
        fileName = pathLocal + shotInfo[0] + '_' + shotInfo[1] + '_' + shotInfo[2]
        fileType = '_l1AllMask01_lr_c001.mb'
        fileName = fileName + fileType
        mc.file(rename=fileName)
        mc.file(save=1)
    ...
    def zmRLALLMask02autoCreate(self, path):
        print ('=================================================================')
        print '====================!!!Start AutoRenderLayer!!!===================='

        mc.file(path, force=1, loadReferenceDepth="all", open=1)
        # Step 1：背景与遮罩层
        self.zmRLSpeficCreate("ALL_MASK02")
        # save
        from idmt.maya.py_common import sk_infoConfig
        reload(sk_infoConfig)
        shotInfo = sk_infoConfig.sk_infoConfig().checkShotInfo()
        pathLocal = sk_infoConfig.sk_infoConfig().checkRenderLayerLocalPath()
        fileName = pathLocal + shotInfo[0] + '_' + shotInfo[1] + '_' + shotInfo[2]
        fileType = '_l1AllMask02_lr_c001.mb'
        fileName = fileName + fileType
        mc.file(rename=fileName)
        mc.file(save=1)
    ...
    def zmRLBGMask02autoCreate(self, path):
        print ('=================================================================')
        print '====================!!!Start AutoRenderLayer!!!===================='

        mc.file(path, force=1, loadReferenceDepth="all", open=1)
        # Step 1：背景与遮罩层
        self.zmRLSpeficCreate("BG_MASK")
        # save
        from idmt.maya.py_common import sk_infoConfig
        reload(sk_infoConfig)
        shotInfo = sk_infoConfig.sk_infoConfig().checkShotInfo()
        pathLocal = sk_infoConfig.sk_infoConfig().checkRenderLayerLocalPath()
        fileName = pathLocal + shotInfo[0] + '_' + shotInfo[1] + '_' + shotInfo[2]
        fileType = '_l1BGMask02_lr_c001.mb'
        fileName = fileName + fileType
        mc.file(rename=fileName)
        mc.file(save=1)
    ...
    def zmRLBGMask03autoCreate(self, path):
        print ('=================================================================')
        print '====================!!!Start AutoRenderLayer!!!===================='

        mc.file(path, force=1, loadReferenceDepth="all", open=1)
        # Step 1：背景与遮罩层
        self.zmRLSpeficCreate("BG_MASK03")
        # save
        from idmt.maya.py_common import sk_infoConfig
        reload(sk_infoConfig)
        shotInfo = sk_infoConfig.sk_infoConfig().checkShotInfo()
        pathLocal = sk_infoConfig.sk_infoConfig().checkRenderLayerLocalPath()
        fileName = pathLocal + shotInfo[0] + '_' + shotInfo[1] + '_' + shotInfo[2]
        fileType = '_l1BGMask03_lr_c001.mb'
        fileName = fileName + fileType
        mc.file(rename=fileName)
        mc.file(save=1)
    ...
    def zmRLSEACPautoCreate(self, path):
        print ('=================================================================')
        print '====================!!!Start AutoRenderLayer!!!===================='

        mc.file(path, force=1, loadReferenceDepth="all", open=1)
        # Step 1：背景与遮罩层
        self.zmRLSpeficCreate("SEA_CP")
        # save
        from idmt.maya.py_common import sk_infoConfig
        reload(sk_infoConfig)
        shotInfo = sk_infoConfig.sk_infoConfig().checkShotInfo()
    ...
    def zmRLSEACOAautoCreate(self, path):
        print ('=================================================================')
        print '====================!!!Start AutoRenderLayer!!!===================='

        mc.file(path, force=1, loadReferenceDepth="all", open=1)
        # Step 1：背景与遮罩层
        self.zmRLSpeficCreate("SEA_WO")
        # save
        from idmt.maya.py_common import sk_infoConfig
        reload(sk_infoConfig)
        shotInfo = sk_infoConfig.sk_infoConfig().checkShotInfo()
    ...
    def zmRLSEACOuautoCreate(self, path):
        print ('=================================================================')
        print '====================!!!Start AutoRenderLayer!!!===================='

        mc.file(path, force=1, loadReferenceDepth="all", open=1)
        # Step 1：背景与遮罩层
        self.zmRLSpeficCreate("SEA_TO")
        # save
        from idmt.maya.py_common import sk_infoConfig
        reload(sk_infoConfig)
        shotInfo = sk_infoConfig.sk_infoConfig().checkShotInfo()
    ...
    def zmRLFGLGTautoCreate(self, path):
        print ('=================================================================')
        print '====================!!!Start AutoRenderLayer!!!===================='

        mc.file(path, force=1, loadReferenceDepth="all", open=1)
        # Step 1：背景与遮罩层
        self.zmRLSpeficCreate("FG_LGT")
        # save
        from idmt.maya.py_common import sk_infoConfig
        reload(sk_infoConfig)
        shotInfo = sk_infoConfig.sk_infoConfig().checkShotInfo()
    ...
    def zmRLLGLGTautoCreate(self, path):
        print ('=================================================================')
        print '====================!!!Start AutoRenderLayer!!!===================='

        mc.file(path, force=1, loadReferenceDepth="all", open=1)
        # Step 1：背景与遮罩层
        self.zmRLSpeficCreate("ALL_SHADOW")
        # save
        from idmt.maya.py_common import sk_infoConfig
        reload(sk_infoConfig)
        shotInfo = sk_infoConfig.sk_infoConfig().checkShotInfo()
    ...
    def zmRLSEAshadowautoCreate(self, path):
        print ('=================================================================')
        print '====================!!!Start AutoRenderLayer!!!===================='

        mc.file(path, force=1, loadReferenceDepth="all", open=1)
        # Step 1：背景与遮罩层
        self.zmRLSpeficCreate("SEA_SHDW")
        # save
        from idmt.maya.py_common import sk_infoConfig
        reload(sk_infoConfig)
        shotInfo = sk_infoConfig.sk_infoConfig().checkShotInfo()
        pathLocal = sk_infoConfig.sk_infoConfig().checkRenderLayerLocalPath()
        fileName = pathLocal + shotInfo[0] + '_' + shotInfo[1] + '_' + shotInfo[2]
        fileType = '_l1SEAshadow_lr_c001.mb'
        fileName = fileName + fileType
        mc.file(rename=fileName)
        mc.file(save=1)
    ...
    def zmRLALLnormalautoCreate(self, path):
        print ('=================================================================')
        print '====================!!!Start AutoRenderLayer!!!===================='

        mc.file(path, force=1, loadReferenceDepth="all", open=1)
        # Step 1：背景与遮罩层
        self.zmRLSpeficCreate("ALL_NM")
        # save
        from idmt.maya.py_common import sk_infoConfig
        reload(sk_infoConfig)
        shotInfo = sk_infoConfig.sk_infoConfig().checkShotInfo()
    ...
    def zmRLReflectionautoCreate(self, path):
        print ('=================================================================')
        print '====================!!!Start AutoRenderLayer!!!===================='

        mc.file(path, force=1, loadReferenceDepth="all", open=1)
        # Step 1：反射层
        self.zmRLSpeficCreate("ALL_Reflect")
        # save
        from idmt.maya.py_common import sk_infoConfig
        reload(sk_infoConfig)
        shotInfo = sk_infoConfig.sk_infoConfig().checkShotInfo()
        pathLocal = sk_infoConfig.sk_infoConfig().checkRenderLayerLocalPath()
        fileName = pathLocal + shotInfo[0] + '_' + shotInfo[1] + '_' + shotInfo[2]
        fileType = '_l1reflect_lr_c001.mb'
        fileName = fileName + fileType
        mc.file(rename=fileName)
        mc.file(save=1)
    ...
    def zmRLCHRautoCreate(self, path):
        print ('=================================================================')
        print '====================!!!Start AutoRenderLayer!!!===================='

        mc.file(path, force=1, loadReferenceDepth="all", open=1)
        # Step 1：背景与遮罩层
        self.zmRLSpeficCreate("CHR_LGTSYS")
        # save
        from idmt.maya.py_common import sk_infoConfig
        reload(sk_infoConfig)
        shotInfo = sk_infoConfig.sk_infoConfig().checkShotInfo()
    ...
    def zmRLCHRlineautoCreate(self, path):
        print ('=================================================================')
        print '====================!!!Start AutoRenderLayer!!!===================='

        mc.file(path, force=1, loadReferenceDepth="all", open=1)
        # Step 1：背景与遮罩层
        self.zmRLSpeficCreate("CHR_LINE")
        # save
        from idmt.maya.py_common import sk_infoConfig
        reload(sk_infoConfig)
        shotInfo = sk_infoConfig.sk_infoConfig().checkShotInfo()
        pathLocal = sk_infoConfig.sk_infoConfig().checkRenderLayerLocalPath()
        fileName = pathLocal + shotInfo[0] + '_' + shotInfo[1] + '_' + shotInfo[2]
        fileType = '_l1CHRline_lr_c001.mb'
        fileName = fileName + fileType
        mc.file(rename=fileName)
        mc.file(save=1)
    ...
    def zmRLEXTRALCreate(self, path):
        print ('=================================================================')
        print '====================!!!Start AutoRenderLayer!!!===================='

        mc.file(path, force=1, loadReferenceDepth="all", open=1)
        # Step 1：背景与遮罩层
        self.zmRLSpeficCreate("ALL_Volume")
        # save
        from idmt.maya.py_common import sk_infoConfig
        reload(sk_infoConfig)
        shotInfo = sk_infoConfig.sk_infoConfig().checkShotInfo()
        pathLocal = sk_infoConfig.sk_infoConfig().checkRenderLayerLocalPath()
        fileName = pathLocal + shotInfo[0] + '_' + shotInfo[1] + '_' + shotInfo[2]
        fileType = '_l1EXTRALIGHT_lr_c001.mb'
        fileName = fileName + fileType
        mc.file(rename=fileName)
        mc.file(save=1)
    ...
    def zmRLBGcausticswallsautoCreate(self, path):
        print ('=================================================================')
        print '====================!!!Start AutoRenderLayer!!!===================='

        mc.file(path, force=1, loadReferenceDepth="all", open=1)
        # Step 1：背景与遮罩层
        self.zmRLSpeficCreate("BG_CAUSTICSWALLS")
        # save
        from idmt.maya.py_common import sk_infoConfig
        reload(sk_infoConfig)
        shotInfo = sk_infoConfig.sk_infoConfig().checkShotInfo()
        pathLocal = sk_infoConfig.sk_infoConfig().checkRenderLayerLocalPath()
        fileName = pathLocal + shotInfo[0] + '_' + shotInfo[1] + '_' + shotInfo[2]
        fileType = '_l1bgCausticswalls_lr_c001.mb'
        fileName = fileName + fileType
        mc.file(rename=fileName)
        mc.file(save=1)
    ...
    def zmRLBGvolumelightautoCreate(self, path):
        print ('=================================================================')
        print '====================!!!Start AutoRenderLayer!!!===================='

        mc.file(path, force=1, loadReferenceDepth="all", open=1)
        # Step 1：背景与遮罩层
        self.zmRLSpeficCreate("BG_VOLUMELIGHTS")
        # save
        from idmt.maya.py_common import sk_infoConfig
        reload(sk_infoConfig)
        shotInfo = sk_infoConfig.sk_infoConfig().checkShotInfo()
    ...
    def zmIDPCHRCreate(self, layerType, selectObjType=0):
        ...
        if layerType=='CHR':
            ...
            if CHRGP:
                ...
                print u'确认有“c016001Biff”壮汉这个角色，点击YES OR NO继续分层'
    ...
    def zmRLLIGHTCreate(self, layerType, selectObjType=0):
        ...
        if lightGrps:
            if layerType == 'LGTFG':
                ...
                if len(lightgup) >= 2:
                    print u'FG_LGT层可能存在多盏灯光，请自行隐藏其余灯光，点击YES OR NO继续分层'
   ...
   # LG_LGT层
    ...
    def zmRLLIGHTNewCreate(self, layerType, selectObjType=0):
        ...
        shot_info = self.ReadEXcle()
        shot_scene = shot_info[4]
                    ...
                    if len(needLightGrps) >= 2:
                        for light in needLightGrps:
                            if mc.getAttr(light.split('|')[4] + ".v") == True:
                                lightgup.append(light)
                    if len(lightgup) >= 2:
                        print u'SEA_SHDW层可能存在多盏灯光，请自行隐藏其余灯光，点击YES OR NO继续分层'
    ...
    def zmRLMskCreate(self, layerType, selectObjType=0):
        ...
        if mc.ls("*:*SEA", type='objectSet'):
            ...
            print u'确定场景中有SET组SEA吗，不会你优化场景给删掉了吧'
        ...
        if mc.ls("*:*ROCK", type='objectSet'):
            ...
            print u'确定场景中有SET组ROCK吗，不会你优化场景给删掉了吧'
        ...
        if mc.ls("*:*VEGETATION", type='objectSet'):
            ...
            print u'确定场景中有SET组VEGETATION吗，不会你优化场景给删掉了吧'
        ...
        if objsAll:
            ...
            if mc.ls('*:*SEA', type='objectSet'):
                if mc.ls('*:*MSH_c_hi_temp_sea_MSK_', type='transform'):
                    ...
                    print u'请确定场景中有SET组SEA中命名有其中“MSH_c_hi_temp_sea_MSK_”字符段'
                ...
                if needSEA==[]:             
                    ...
                    if mc.ls('*:*SEA', type='objectSet'):
                        if mc.ls('*:*MSH_c_hi_temp_sea_CO_', type='transform'):
                            ...
                            print u'请确定场景中有SET组SEA中命名有其中“MSH_c_hi_temp_sea_CO_”字符段'
                    else:
                        print u'请确定场景中有SET组SEA,即使创建成功渲染层材质也是不正确的'
    ...
    def zmRLSEACObCreate(self, layerType, selectObjType=0):
        ...
        if len(refernames)==0:
            print u'请确保文件中有场景参考，可不必勾选但不能移除'
  
        if len(refernames)>=2:
            print u'文件中可能存在两个以上场景参考，请移除多余场景参考，只保留需要的一个'
        ...
        if layerName01 =='WAVESCO':
            ...
            if mc.ls('*:*MSH_temp_CO_', type='transform'):
                ...
                if objSGs:
                    ...
                    print u'请确定有海浪这个道具，并且道具中存在“MSH_temp_CO_”字符段'
        ...
        if layerName02 == 'WAVESMSK':
            ...
            if mc.ls('*:*MSH_temp_MSK_', type='transform'):
                ...
                if objSGs:
                    ...
                    print u'请确定有海浪这个道具，并且道具中存在“MSH_temp_MSK_”字符段'
        ...
        if layerName03 == 'WAVESNOISE':
            ...
            if mc.ls('*:*MSH_temp_NOISE_', type='transform'):
                ...
                if objSGs:
                    ...
                    print u'请确定有海浪这个道具，并且道具中存在“MSH_temp_NOISE_”字符段'
            if objSG:
                for obj in rlObjs:
                    mc.select(obj)
                    mc.sets(e=1, forceElement=objSG) 
    ...
    def zmRLLIGHTBabyCreate(self, layerType, selectObjType=0):
        ...
        if len(lightnames)==0:
            print u'（1）请确保文件中有场景参考，可不必勾选但不能移除；（2）如若排除掉（1）的情况依然提示对话框，请和S联系，让他更新角色灯光'
            
        if len(lightnames)>=2:
            print u'文件中可能存在两个以上场景参考，请确定文件以哪个场景为主，并移除其他场景参考保留主场景参考'
            ...
            if lights:
                ...
                if needLightGrps_A:
                    ...
                    if len(Cams)==0 or len(Cams)>1:
                        print u'请确保CAM_GRP组下有且仅有一个相机'
                    if len(needLightGrps_A)==0 or len(needLightGrps_A)>1:
                        print u'请确保有且仅有一盏灯光命名有字段“DAY_CHR_A1”'
    ...
    def zmMotuvilageAutolayer(self, path):
        print (u'===============!!!strat 【%s】!!!===============' % (u's003001MaotuVillage分层开始'))

        self.zmRLCOautoCreate(path)
        self.zmRLALLZDPautoCreate(path)
        self.zmRLALLMask01autoCreate(path)
        self.zmRLBGMask02autoCreate(path)
        self.zmRLFGLGTautoCreate(path)
        self.zmRLSEACPautoCreate(path)
        self.zmRLCAUSTICautoCreate(path)
        self.zmRLCHRautoCreate(path)
        self.zmRLCHRlineautoCreate(path)

        print (u'===============!!!Done 【%s】!!!===============' % (u's003001MaotuVillage分层结束'))
        print '\n'

    def zmBoatRicoAutolayer(self, path):
        print (u'===============!!!strat 【%s】!!!===============' % (u's017001BoatRico分层开始'))

        self.zmRLCOautoCreate(path)
        self.zmRLALLZDPautoCreate(path)
        self.zmRLALLMask01autoCreate(path)
        self.zmRLFGLGTautoCreate(path)
        self.zmRLALLnormalautoCreate(path)
        self.zmRLCHRautoCreate(path)
        self.zmRLCHRlineautoCreate(path)

        print (u'===============!!!Done 【%s】!!!===============' % (u's017001BoatRico场景分层结束'))
        print '\n'

    def zmDeepLagoonAutolayer(self, path):
        print (u'===============!!!strat 【%s】!!!===============' % (u's016001DeepLagoon分层开始'))

        self.zmRLCOautoCreate(path)
        self.zmRLALLZDPautoCreate(path)
        self.zmRLALLMask01autoCreate(path)
        self.zmRLALLMask02autoCreate(path)
        self.zmRLFGLGTautoCreate(path)
        self.zmRLBGMask02autoCreate(path)
        self.zmRLSEAshadowautoCreate(path)
        self.zmRLSEACPautoCreate(path)
        self.zmRLCAUSTICautoCreate(path)
        self.zmRLCHRautoCreate(path)
        self.zmRLCHRlineautoCreate(path)

        print (u'===============!!!Done 【%s】!!!===============' % (u's016001DeepLagoon场景分层结束'))
        print '\n'

    def zmDFanahoBeachAutolayer(self, path):
        print (u'===============!!!strat 【%s】!!!===============' % (u's018001FanahoBeach分层开始'))
        ...
        print (u'===============!!!Done 【%s】!!!===============' % (u's018001FanahoBeach场景分层结束'))
        print '\n'

    def zmGalionBeachAutolayer(self, path):
        print (u'===============!!!strat 【%s】!!!===============' % (u's001001GalionBeach分层开始'))
        ...
        self.zmRLSEACPautoCreate(path)
        self.zmRLALLnormalautoCreate(path)
        self.zmRLBGMask03autoCreate(path)
        self.zmRLCAUSTICautoCreate(path)
        self.zmRLCHRautoCreate(path)
        self.zmRLCHRlineautoCreate(path)

        print (u'===============!!!Done 【%s】!!!===============' % (u's001001GalionBeach场景分层结束'))
        print '\n'

    def zmGalionIntAutolayer(self, path):
        print (u'===============!!!strat 【%s】!!!===============' % (u's002001GalionInt分层开始'))
        ...
        self.zmRLALLZDPautoCreate(path)
        self.zmRLALLMask01autoCreate(path)
        self.zmRLLGLGTautoCreate(path)
        self.zmRLALLnormalautoCreate(path)
        self.zmRLBGMask03autoCreate(path)
        self.zmRLCHRautoCreate(path)
        self.zmRLCHRlineautoCreate(path)

        print (u'===============!!!Done 【%s】!!!===============' % (u's002001GalionInt场景分层结束'))
        print '\n'

    def zmOpenSeaAutolayer(self, path):
        print (u'===============!!!strat 【%s】!!!===============' % (u's019001OpenSea分层开始'))
        from idmt.maya.commonCore.core_mayaCommon import sk_sceneTools
        reload(sk_sceneTools)
        ...
        self.zmRLFGLGTautoCreate(path)
        self.zmRLSEAshadowautoCreate(path)
        self.zmRLSEACOAautoCreate(path)
        self.zmRLCHRautoCreate(path)
        self.zmRLCHRlineautoCreate(path)

        print (u'===============!!!Done 【%s】!!!===============' % (u's019001OpenSea场景分层结束'))
        print '\n'

    def zmSeabedAAutolayer(self, path):
        print (u'===============!!!strat 【%s】!!!===============' % (u's013001SeabedA分层开始'))

        self.zmRLCOautoCreate(path)
        self.zmRLALLZDPautoCreate(path)
        self.zmRLALLMask01autoCreate(path)
        self.zmRLFGLGTautoCreate(path)
        self.zmRLSEACOuautoCreate(path)
        self.zmRLCAUSTICautoCreate(path)
        self.zmRLCHRautoCreate(path)
        self.zmRLCHRlineautoCreate(path)

        print (u'===============!!!Done 【%s】!!!===============' % (u's013001SeabedA场景分层结束'))
        print '\n'

    def zmSurfingBeachAutolayer(self, path):
        print (u'===============!!!strat 【%s】!!!===============' % (u's008001SurfingBeach分层开始'))
        ...
        self.zmRLCHRautoCreate(path)
        self.zmRLCHRlineautoCreate(path)
        self.zmRLEXTRALCreate(path)
        print (u'===============!!!Done 【%s】!!!===============' % (u's008001SurfingBeach场景分层结束'))
        print '\n'

    def zmTapuniRocksAutolayer(self, path):
        print (u'===============!!!strat 【%s】!!!===============' % (u's021001TapuniRocks分层开始'))
        ...
        print (u'===============!!!Done 【%s】!!!===============' % (u's021001TapuniRocks场景分层结束'))
        print '\n'

    def zmMotuAutolayer(self, path):
        print (u'===============!!!strat 【%s】!!!===============' % (u's023001Motu分层开始'))
        ...
        print (u'===============!!!Done 【%s】!!!===============' % (u's023001Motu场景分层结束'))
        print '\n'

    def zmRamanaHutAutolayer(self, path):
        print (u'===============!!!strat 【%s】!!!===============' % (u's004001RamanaHut分层开始'))
        ...
        self.zmRLSEAshadowautoCreate(path)
        self.zmRLSEACPautoCreate(path)
        self.zmRLReflectionautoCreate(path)
        self.zmRLCAUSTICautoCreate(path)
        self.zmRLCHRautoCreate(path)
        self.zmRLCHRlineautoCreate(path)

        print (u'===============!!!Done 【%s】!!!===============' % (u's004001RamanaHut场景分层结束'))
        print '\n'

    def zmMafatuCavesAutolayer(self, path):
        print (u'===============!!!strat 【%s】!!!===============' % (u's020001MafatuCaves分层开始'))
        ...
        print (u'===============!!!Done 【%s】!!!===============' % (u's020001MafatuCaves场景分层结束'))
        print '\n'

    def zmSeabedGalleonAutolayer(self, path):
        print (u'===============!!!strat 【%s】!!!===============' % (u's022001SeabedGalleon分层开始'))

        self.zmRLCOautoCreate(path)
        self.zmRLALLZDPautoCreate(path)
        self.zmRLBGZDPautoCreate(path)
        self.zmRLALLMask01autoCreate(path)
        self.zmRLFGLGTautoCreate(path)
        self.zmRLCAUSTICautoCreate(path)
        self.zmRLSEACOuautoCreate(path)
        ...
        print (u'===============!!!Done 【%s】!!!===============' % (u's022001SeabedGalleon场景分层结束'))
        print '\n'

    def zmSeabedGalleonBrokenAutolayer(self, path):
        print (u'===============!!!strat 【%s】!!!===============' % (u's022002SeabedGalleonBroken分层开始'))
        ...
        print (u'===============!!!Done 【%s】!!!===============' % (u's022002SeabedGalleonBroken场景分层结束'))
        print '\n'

    def zmManahaReefAutolayer(self, path):
        print (u'===============!!!strat 【%s】!!!===============' % (u's015001ManahaReef分层开始'))
        ...
        print (u'===============!!!Done 【%s】!!!===============' % (u's015001ManahaReef场景分层结束'))
        print '\n'

    def zmIcebergAutolayer(self, path):
        print (u'===============!!!strat 【%s】!!!===============' % (u's026001Iceberg分层开始'))
        ...
        print (u'===============!!!Done 【%s】!!!===============' % (u's026001Iceberg场景分层结束'))
        print '\n'

    def zmIVanKrookBoatAutolayer(self, path):
        print (u'===============!!!strat 【%s】!!!===============' % (u's015001ManahaReef分层开始'))
    ...
    def zmVanKrookBoatIntExtAutolayer(self, path):
        print (u'===============!!!strat 【%s】!!!===============' % (u's010002VanKrookBoatIntExt分层开始'))
        ...
        print (u'===============!!!Done 【%s】!!!===============' % (u's010002VanKrookBoatIntExt场景分层结束'))
        print '\n'


    def zmMaotuGamesCompetitionAutolayer(self, path):
        print (u'===============!!!strat 【%s】!!!===============' % (u's003029MaotuGamesCompetition分层开始'))
        ...
        print (u'===============!!!Done 【%s】!!!===============' % (u's003029MaotuGamesCompetition场景分层结束'))
        print '\n'
        
    def zmForbiddenCaveBlockedAutolayer(self, path):
        print (u'===============!!!strat 【%s】!!!===============' % (u's039001ForbiddenCave分层开始'))
        ...
        self.zmRLCAUSTICautoCreate(path)
        self.zmRLReflectionautoCreate(path)
        self.zmRLBGcausticswallsautoCreate(path)   
        self.zmRLBGvolumelightautoCreate(path)
        self.zmRLCHRautoCreate(path)
        self.zmRLCHRlineautoCreate(path)

        print (u'===============!!!Done 【%s】!!!===============' % (u's039001ForbiddenCave场景分层结束'))
        print '\n'

    def zmJungleAutolayer(self, path):
        print (u'===============!!!strat 【%s】!!!===============' % (u's038001Jungle分层开始'))
        ...
        print (u'===============!!!Done 【%s】!!!===============' % (u's038001Jungle场景分层结束'))
        print '\n'


    def zmMotuBuildingRuinedAutolayer(self, path):
        print (u'===============!!!strat 【%s】!!!===============' % (u's037002MotuBuildingRuined分层开始'))
        ...
        print (u'===============!!!Done 【%s】!!!===============' % (u's037002MotuBuildingRuined场景分层结束'))
        print '\n'

    def zmMotuBuildingAutolayer(self, path):
        print (u'===============!!!strat 【%s】!!!===============' % (u's037001MotuBuilding分层开始'))
        ...
        print (u'===============!!!Done 【%s】!!!===============' % (u's037001MotuBuilding场景分层结束'))
        print '\n'

    def zmPomareBeachAutolayer(self, path):
        print (u'===============!!!strat 【%s】!!!===============' % (u's027001PomareBeach场景分层开始'))
        ...
        print (u'===============!!!Done 【%s】!!!===============' % (u's027001PomareBeach场景分层结束'))
        print '\n'

    def zmPomareBeachHoleAutolayer(self, path):
        print (u'===============!!!strat 【%s】!!!===============' % (u's027003PomareBeachHole场景分层开始'))
        ...
        print (u'===============!!!Done 【%s】!!!===============' % (u's027003PomareBeachHole场景分层结束'))
        print '\n'
        
    def zmMotuDinnerDevastatedwAutolayer(self, path):
        print (u'===============!!!strat 【%s】!!!===============' % (u's023003MotuDinnerDevastatedw场景分层开始'))
        ...
        print (u'===============!!!Done 【%s】!!!===============' % (u's023003MotuDinnerDevastatedw场景分层结束'))
        print '\n'

    def zmMotuDinnerAutolayer(self, path):
        print (u'===============!!!strat 【%s】!!!===============' % (u's023002MotuDinner场景分层开始'))
        ...
        print (u'===============!!!Done 【%s】!!!===============' % (u's023002MotuDinner场景分层结束'))
        print '\n'        
 
    def zmSeabedTugboatAutolayer(self, path):
        print (u'===============!!!strat 【%s】!!!===============' % (u's013002SeabedTugboat场景分层开始'))
        ...
        print (u'===============!!!Done 【%s】!!!===============' % (u's013002SeabedTugboat场景分层结束'))
        print '\n'  

 
    def zmMangroveAutolayer(self, path):
        print (u'===============!!!strat 【%s】!!!===============' % (u's005001Mangrove场景分层开始'))
        ...
        print (u'===============!!!Done 【%s】!!!===============' % (u's005001Mangrove场景分层结束'))
        print '\n'

    def zmMaotuVillageCoconutAutolayer(self, path):
        print (u'===============!!!strat 【%s】!!!===============' % (u's003028MaotuVillageCoconut场景分层开始'))
        ...
        print (u'===============!!!Done 【%s】!!!===============' % (u's003028MaotuVillageCoconut场景分层结束'))
        print '\n'
        
    def zmChiefHouseIntExtAutolayer(self, path):
        print (u'===============!!!strat 【%s】!!!===============' % (u's003025ChiefHouseIntExt分层开始'))
        ...
        print (u'===============!!!Done 【%s】!!!===============' % (u's003025ChiefHouseIntExt场景分层结束'))
        print '\n'

    def zmForbiddenCaveAutolayer(self, path):
        print (u'===============!!!strat 【%s】!!!===============' % (u's039001ForbiddenCave分层开始'))
    ...
    def zmGalionBeachRedSeaAutolayer(self, path):
        print (u'===============!!!strat 【%s】!!!===============' % (u's001004GalionBeachRedSea场景分层开始'))
        ...
        print (u'===============!!!Done 【%s】!!!===============' % (u's001004GalionBeachRedSea场景分层结束'))
        print '\n'
    ...
    def zmMaotuRedSeaAutolayer(self, path):
        print (u'===============!!!strat 【%s】!!!===============' % (u's003032MaotuRedSea场景分层开始'))
        ...
        print (u'===============!!!Done 【%s】!!!===============' % (u's003032MaotuRedSea场景分层结束'))
        print '\n' 

    def zmPomareMonsterAutolayer(self, path):
        print (u'===============!!!strat 【%s】!!!===============' % (u's027005PomareMonster场景分层开始'))
        ...
        print (u'===============!!!Done 【%s】!!!===============' % (u's027005PomareMonster场景分层结束'))
        print '\n' 

    def zmGalionBeachPartyAutolayer(self, path):
        print (u'===============!!!strat 【%s】!!!===============' % (u's001003GalionBeachParty场景分层开始'))
        ...
        print (u'===============!!!Done 【%s】!!!===============' % (u's001003GalionBeachParty场景分层结束'))
        print '\n' 
        
                                                                                                                
    def zmAutoLayercrete(self, path):
        shot_info = self.ReadEXcle()
        shot_scene = shot_info[4]
        
        if shot_scene == 's003001MaotuVillage':
            self.zmMotuvilageAutolayer(path)
        if shot_scene == 's017001BoatRico':
            self.zmBoatRicoAutolayer(path)
        if shot_scene == 's016001DeepLagoon':
            self.zmDeepLagoonAutolayer(path)
        if shot_scene == 's018001FanahoBeach':
            self.zmDFanahoBeachAutolayer(path)
        if shot_scene == 's001001GalionBeach':
            self.zmGalionBeachAutolayer(path)
        if shot_scene == 's002001GalionInt':
            self.zmGalionIntAutolayer(path)
        if shot_scene == 's019001OpenSea':
            self.zmOpenSeaAutolayer(path)
        if shot_scene == 's013001SeabedA':
            self.zmSeabedAAutolayer(path)
        if shot_scene == 's008001SurfingBeach':
            self.zmSurfingBeachAutolayer(path)
        if shot_scene == 's021001TapuniRocks':
            self.zmTapuniRocksAutolayer(path)
        if shot_scene == 's023001Motu':
            self.zmMotuAutolayer(path)
        if shot_scene == 's004001RamanaHut':
            self.zmRamanaHutAutolayer(path)
        if shot_scene == 's020001MafatuCaves':
            self.zmMafatuCavesAutolayer(path)
        if shot_scene == 's022001SeabedGalleon':
            self.zmSeabedGalleonAutolayer(path)
        if shot_scene == 's022002SeabedGalleonBroken':
            self.zmSeabedGalleonBrokenAutolayer(path)
        if shot_scene == 's015001ManahaReef':
            self.zmManahaReefAutolayer(path)
        if shot_scene == 's026001Iceberg':
            self.zmIcebergAutolayer(path)
        if shot_scene == 's010001VanKrookBoat':
            self.zmIVanKrookBoatAutolayer(path)
        if shot_scene == 's010002VanKrookBoatIntExt':
            self.zmVanKrookBoatIntExtAutolayer(path)
        if shot_scene == 's003029MaotuGamesCompetition':
            self.zmMaotuGamesCompetitionAutolayer(path)
        if shot_scene == 's039002ForbiddenCaveBlocked':
            self.zmForbiddenCaveBlockedAutolayer(path)  
        if shot_scene == 's038001Jungle':
            self.zmJungleAutolayer(path) 
        if shot_scene == 's037002MotuBuildingRuined':
            self.zmMotuBuildingRuinedAutolayer(path) 
        if shot_scene == 's037001MotuBuilding':
            self.zmMotuBuildingAutolayer(path)  
        if shot_scene == 's027001PomareBeach':
            self.zmPomareBeachAutolayer(path)  
        if shot_scene == 's027003PomareBeachHole':
            self.zmPomareBeachHoleAutolayer(path)
        if shot_scene == 's023003MotuDinnerDevastatedw':
            self.zmMotuDinnerDevastatedwAutolayer(path) 
        if shot_scene == 's023002MotuDinner':
            self.zmMotuDinnerAutolayer(path)
        if shot_scene == 's013002SeabedTugboat':
            self.zmSeabedTugboatAutolayer(path)
        if shot_scene == 's005001Mangrove':
            self.zmMangroveAutolayer(path)
        if shot_scene == 's003028MaotuVillageCoconut':
            self.zmMaotuVillageCoconutAutolayer(path)
        if shot_scene == 's003025ChiefHouseIntExt':
            self.zmChiefHouseIntExtAutolayer(path)  
        if shot_scene == 's039001ForbiddenCave':
            self.zmForbiddenCaveAutolayer(path)           
        if shot_scene == 's001004GalionBeachRedSea':
            self.zmGalionBeachRedSeaAutolayer(path)  
        if shot_scene == 's001004GalionBeachRedSea':
            self.zmGalionBeachRedSeaAutolayer(path) 
        if shot_scene == 's003032MaotuRedSea':
            self.zmMaotuRedSeaAutolayer(path)       
        if shot_scene == 's027005PomareMonster':
            self.zmPomareMonsterAutolayer(path)
        if shot_scene == 's001003GalionBeachParty':
            self.zmGalionBeachPartyAutolayer(path)                                                                                                                                                                 