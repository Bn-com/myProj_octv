...
class sk_hbExportCam(object):
    ...
    def HbExceptSelectReCam(self, projectInfo, all=0, scrFrame=1001 , batchUpadate = 0 ,camSelect = 0,shotInfo = []):
        ...
        if not all:
            getCams = mc.ls(sl=1,l=1)
        else:
            getCams = mc.listCameras(p=True)
        if not getCams:
        ...
        for Cam in getCams:
            ...
            ThePath = sk_infoConfig.sk_infoConfig().checkShotDataInfoPath(server = 0,infoMode = 11,shotInfos = shotInfo)
            ThePath = ThePath[:-1]
            ThePath2 = '%s/images'%ThePath
            print u'READY TO: HUANGBI script to CREATE dir %s========================================'%(time.ctime())
            if not os.path.exists(ThePath):
                #os.makedirs(ThePath)
                mc.sysFile(ThePath,makeDir = 1)
            if not os.path.exists(ThePath2):
                #os.makedirs(ThePath2)
                mc.sysFile(ThePath2,makeDir = 1)
            print u'create path DONE %s'%(time.ctime())
            if camSelect:
                CamName_validCharactor = 'cam_' + str(shotInfo[1]) + '_' + str(shotInfo[2]) + '_baked'
                if shotType in [3]:
                    CamName_validCharactor = 'cam_' + str(shotInfo[1]) + '_' + str(shotInfo[2]) + '_' + str(shotInfo[3]) + '_baked'
            else:
                CamName_validCharactor = CamName.split(":")[-1]
            if ThePath[-1] in ['/']:
                finalPath = ThePath  + CamName_validCharactor + ".ma"
            else:
                finalPath = ThePath + '/' + CamName_validCharactor + ".ma"
            exportPath.append(finalPath)
            print '-----finalPath'
            print finalPath
            print ThePath
            print ThePath2
            if os.path.exists(finalPath):
                mc.sysFile(finalPath,delete = 1)
            mc.file(finalPath, type="mayaAscii", pr=1, es=1,force = 1,options="v=0;")
            ...
            if batchUpadate == 0:
                ...
                print u'READY TO: convertImageFormat %s'%(time.ctime())
                self.convertImageFormat(ThePath + "/" + CamName_validCharactor + ".iff", "bmp", 1)
                print u'convertImageFormat DONE %s'%(time.ctime())
    ...
    # 补充：新增处理相机到Z盘路径
    def sceneCamPublish(self,shotType = 2):
        import sk_infoConfig
    ...
    def camServerReference(self,info = 2):
        ...
        reload(sk_infoConfig)
        shotInfo = sk_infoConfig.sk_infoConfig().checkShotInfo()
        info = sk_infoConfig.sk_infoConfig().checkShotType()
