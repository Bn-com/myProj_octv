...
class hh_RenderArnold(object):
    ...
    # 窗口
        ...
        mc.button(
            label=u'删除', bgc=[0, 0, 0.0], width=110, height=30, command='from idmt.maya.Hh_common import hh_RenderArnoldLayer\nreload(hh_RenderArnoldLayer)\nhh_RenderArnoldLayer.hh_RenderArnold().ArnoldAOVDelete(AOVtype="Normal")')
        ...
        mc.button(
            label=u'删除', bgc=[0, 0, 0.0], width=110, height=30, command='from idmt.maya.Hh_common import hh_RenderArnoldLayer\nreload(hh_RenderArnoldLayer)\nhh_RenderArnoldLayer.hh_RenderArnold().ArnoldAOVDelete(AOVtype="Fre")')
...
#        collectionzdp = mc.radioCollection()
        ...
        mc.button(
            label=u'删除', bgc=[0, 0, 0.0], width=110, height=30, command='from idmt.maya.Hh_common import hh_RenderArnoldLayer\nreload(hh_RenderArnoldLayer)\nhh_RenderArnoldLayer.hh_RenderArnold().ArnoldAOVZdepDelete(AOVtype="sss")')
        ...
        mc.button(
            label=u'删除', bgc=[0, 0, 0.0], width=110, height=30, command='from idmt.maya.Hh_common import hh_RenderArnoldLayer\nreload(hh_RenderArnoldLayer)\nhh_RenderArnoldLayer.hh_RenderArnold().ArnoldAOVDelete(AOVtype="Shadow")')
        collectionzdp = mc.radioCollection()
        # zdepth
        ...
        mc.button(
            label=u'删除', bgc=[0, 0, 0.0], width=110, height=30, command='from idmt.maya.Hh_common import hh_RenderArnoldLayer\nreload(hh_RenderArnoldLayer)\nhh_RenderArnoldLayer.hh_RenderArnold().ArnoldAOVDelete(AOVtype="Zdp")')
        # motionvector
        ...
        mc.button(
            label=u'删除', bgc=[0, 0, 0.0], width=110, height=30, command='from idmt.maya.Hh_common import hh_RenderArnoldLayer\nreload(hh_RenderArnoldLayer)\nhh_RenderArnoldLayer.hh_RenderArnold().ArnoldAOVZdepDelete(AOVtype="motionvector")')
        ...
        mc.button(label=u'Arnold渲染设置', width=180, height=30,
                  bgc=[0.13, 0.6, 0.25], command='from idmt.maya.Hh_common import hh_RenderArnoldLayer\nreload(hh_RenderArnoldLayer)\nhh_RenderArnoldLayer.hh_RenderArnold().ArnoldRendererSettings("03")')         
        mc.button(label=u'color', width=180, height=30,
                  bgc=[0, 0, 0.0], command='from idmt.maya.Hh_common import hh_RenderArnoldLayer\nreload(hh_RenderArnoldLayer)\nhh_RenderArnoldLayer.hh_RenderArnold().csl_ArnoldRenderLayerCreat(layername="color")')  
        mc.button(label=u'motionblur', width=180, height=30,
                  bgc=[0.13, 0.15, 0.25], command='from idmt.maya.Hh_common import hh_RenderArnoldLayer\nreload(hh_RenderArnoldLayer)\nhh_RenderArnoldLayer.hh_RenderArnold().csl_ArnoldRenderLayerCreat(layername="motionblur")') 
        mc.button(label=u'id', width=180, height=30,
                  bgc=[0, 0, 0.0], command='from idmt.maya.Hh_common import hh_RenderArnoldLayer\nreload(hh_RenderArnoldLayer)\nhh_RenderArnoldLayer.hh_RenderArnold().csl_ArnoldRenderLayerCreat(layername="id")')                     
        mc.setParent('..')
        mc.setParent('..')
        mc.frameLayout(label=u'常用工具', bgc=[0, 0, 0.0], borderStyle='in',cll=1,cl=1)
        mc.rowColumnLayout(numberOfColumns=2)
        mc.button(label=u'smooth', width=180, height=30,
                  bgc=[0, 0, 0.0], command='from idmt.maya.Hh_common import hh_RenderArnoldLayer\nreload(hh_RenderArnoldLayer)\nhh_RenderArnoldLayer.hh_RenderArnold().csl_SmoothSet()')             
...
##                  bgc=[0.6, 0.15, 0.25], command='from idmt.maya.ShunLiu_common import csl_checkin\nreload(csl_checkin)\ncsl_checkin.csl_checkin().csl_ImageSizeCover(type="quarter",server=1)\ncsl_checkin.csl_checkin().csl_ImageSizeCover(type="half",server=1)')                                     
        ...
        mc.button(label=u'输出角色id01',width=90,height=30,bgc=[0.13, 0.15, 0.25],command='from idmt.maya.Hh_common import hh_RenderArnoldLayer\nreload(hh_RenderArnoldLayer)\nhh_RenderArnoldLayer.hh_RenderArnold().csl_RGBInfoExport(id=\'id01\')')
        mc.button(label=u'输出角色id02',width=90,height=30,bgc=[0.13, 0.15, 0.25],command='from idmt.maya.Hh_common import hh_RenderArnoldLayer\nreload(hh_RenderArnoldLayer)\nhh_RenderArnoldLayer.hh_RenderArnold().csl_RGBInfoExport(id=\'id02\')')
        mc.button(label=u'输出角色id03',width=90,height=30,bgc=[0.13, 0.15, 0.25],command='from idmt.maya.Hh_common import hh_RenderArnoldLayer\nreload(hh_RenderArnoldLayer)\nhh_RenderArnoldLayer.hh_RenderArnold().csl_RGBInfoExport(id=\'id03\')') 
        mc.button(label=u'输出角色id04',width=90,height=30,bgc=[0.13, 0.15, 0.25],command='from idmt.maya.Hh_common import hh_RenderArnoldLayer\nreload(hh_RenderArnoldLayer)\nhh_RenderArnoldLayer.hh_RenderArnold().csl_RGBInfoExport(id=\'id04\')')                 
        mc.button(label=u'输出场景id11',width=90,height=30,bgc=[0.0, 0.0, 0.0],command='from idmt.maya.Hh_common import hh_RenderArnoldLayer\nreload(hh_RenderArnoldLayer)\nhh_RenderArnoldLayer.hh_RenderArnold().csl_RGBInfoExport(id=\'id11\')')
        mc.button(label=u'输出场景id12',width=90,height=30,bgc=[0.0, 0.0, 0.0],command='from idmt.maya.Hh_common import hh_RenderArnoldLayer\nreload(hh_RenderArnoldLayer)\nhh_RenderArnoldLayer.hh_RenderArnold().csl_RGBInfoExport(id=\'id12\')')
        mc.button(label=u'输出场景id13',width=90,height=30,bgc=[0.0, 0.0, 0.0],command='from idmt.maya.Hh_common import hh_RenderArnoldLayer\nreload(hh_RenderArnoldLayer)\nhh_RenderArnoldLayer.hh_RenderArnold().csl_RGBInfoExport(id=\'id13\')')        
        mc.button(label=u'输出场景id14',width=90,height=30,bgc=[0.0, 0.0, 0.0],command='from idmt.maya.Hh_common import hh_RenderArnoldLayer\nreload(hh_RenderArnoldLayer)\nhh_RenderArnoldLayer.hh_RenderArnold().csl_RGBInfoExport(id=\'id14\')')         
        mc.button(label=u'ID材质输出前检测',width=90,height=30,bgc=[0.1, 0.6, 0.25],command='from idmt.maya.Hh_common import hh_RenderArnoldLayer\nreload(hh_RenderArnoldLayer)\nhh_RenderArnoldLayer.hh_RenderArnold().csl_RGBInfoExportTest()')
        mc.setParent('..')
        mc.setParent('..')        
        #idp材质工具    
        mc.frameLayout(label=u'IDP材质工具', bgc=[0, 0, 0.0], borderStyle='in', cll=1,cl=1)
        mc.rowColumnLayout(numberOfColumns=4) 
        mc.button(label=u'R',width=90,height=30,bgc=[1,0,0],command='from idmt.maya.Hh_common import hh_RenderArnoldLayer\nreload(hh_RenderArnoldLayer)\nhh_RenderArnoldLayer.hh_RenderArnold().ArnoldRendererSettings("03")\nhh_RenderArnoldLayer.hh_RenderArnold().ArnoldIDCreat(idpShader="ArnoldIdpR")')
        mc.button(label=u'G',width=90,height=30,bgc=[0,1,0],command='from idmt.maya.Hh_common import hh_RenderArnoldLayer\nreload(hh_RenderArnoldLayer)\nhh_RenderArnoldLayer.hh_RenderArnold().ArnoldRendererSettings("03")\nhh_RenderArnoldLayer.hh_RenderArnold().ArnoldIDCreat(idpShader="ArnoldIdpG")')
        mc.button(label=u'B',width=90,height=30,bgc=[0,0,1],command='from idmt.maya.Hh_common import hh_RenderArnoldLayer\nreload(hh_RenderArnoldLayer)\nhh_RenderArnoldLayer.hh_RenderArnold().ArnoldRendererSettings("03")\nhh_RenderArnoldLayer.hh_RenderArnold().ArnoldIDCreat(idpShader="ArnoldIdpB")')
        mc.button(label=u'M',width=90,height=30,bgc=[0,0,0],command='from idmt.maya.Hh_common import hh_RenderArnoldLayer\nreload(hh_RenderArnoldLayer)\nhh_RenderArnoldLayer.hh_RenderArnold().ArnoldRendererSettings("03")\nhh_RenderArnoldLayer.hh_RenderArnold().ArnoldIDCreat(idpShader="ArnoldIdpM")')
        mc.button(label=u'Y',width=90,height=30,bgc=[1,1,0],command='from idmt.maya.Hh_common import hh_RenderArnoldLayer\nreload(hh_RenderArnoldLayer)\nhh_RenderArnoldLayer.hh_RenderArnold().ArnoldRendererSettings("03")\nhh_RenderArnoldLayer.hh_RenderArnold().ArnoldIDCreat(idpShader="ArnoldIdpY")') 
        mc.button(label=u'C',width=90,height=30,bgc=[0,1,1],command='from idmt.maya.Hh_common import hh_RenderArnoldLayer\nreload(hh_RenderArnoldLayer)\nhh_RenderArnoldLayer.hh_RenderArnold().ArnoldRendererSettings("03")\nhh_RenderArnoldLayer.hh_RenderArnold().ArnoldIDCreat(idpShader="ArnoldIdpC")') 
        mc.button(label=u'K',width=90,height=30,bgc=[1,0,1],command='from idmt.maya.Hh_common import hh_RenderArnoldLayer\nreload(hh_RenderArnoldLayer)\nhh_RenderArnoldLayer.hh_RenderArnold().ArnoldRendererSettings("03")\nhh_RenderArnoldLayer.hh_RenderArnold().ArnoldIDCreat(idpShader="ArnoldIdpK")')                 
        mc.setParent('..')
        mc.setParent('..')        
        ...
        mc.rowColumnLayout(numberOfColumns=4) 
        mc.button(label=u'R',width=90,height=30,bgc=[1,0,0],command='from idmt.maya.Hh_common import hh_RenderArnoldLayer\nreload(hh_RenderArnoldLayer)\nhh_RenderArnoldLayer.hh_RenderArnold().P5_ArnoldDisMatCreat(idpShader="ArnoldIdpR01")')
        mc.button(label=u'G',width=90,height=30,bgc=[0,1,0],command='from idmt.maya.Hh_common import hh_RenderArnoldLayer\nreload(hh_RenderArnoldLayer)\nhh_RenderArnoldLayer.hh_RenderArnold().P5_ArnoldDisMatCreat(idpShader="ArnoldIdpG")')
        mc.button(label=u'B',width=90,height=30,bgc=[0,0,1],command='from idmt.maya.Hh_common import hh_RenderArnoldLayer\nreload(hh_RenderArnoldLayer)\nhh_RenderArnoldLayer.hh_RenderArnold().P5_ArnoldDisMatCreat(idpShader="ArnoldIdpB")')
        mc.button(label=u'M',width=90,height=30,bgc=[0,0,0],command='from idmt.maya.Hh_common import hh_RenderArnoldLayer\nreload(hh_RenderArnoldLayer)\nhh_RenderArnoldLayer.hh_RenderArnold().P5_ArnoldDisMatCreat(idpShader="ArnoldIdpM")')   
        mc.button(label=u'Y',width=90,height=30,bgc=[1,1,0],command='from idmt.maya.Hh_common import hh_RenderArnoldLayer\nreload(hh_RenderArnoldLayer)\nhh_RenderArnoldLayer.hh_RenderArnold().P5_ArnoldDisMatCreat(idpShader="ArnoldIdpY")') 
        mc.button(label=u'C',width=90,height=30,bgc=[0,1,1],command='from idmt.maya.Hh_common import hh_RenderArnoldLayer\nreload(hh_RenderArnoldLayer)\nhh_RenderArnoldLayer.hh_RenderArnold().P5_ArnoldDisMatCreat(idpShader="ArnoldIdpC")') 
        mc.button(label=u'K',width=90,height=30,bgc=[1,0,1],command='from idmt.maya.Hh_common import hh_RenderArnoldLayer\nreload(hh_RenderArnoldLayer)\nhh_RenderArnoldLayer.hh_RenderArnold().P5_ArnoldDisMatCreat(idpShader="ArnoldIdpK")')                              
        mc.button(label=u'Lam',width=90,height=30,bgc=[0.5,0.5,0.5],command='from idmt.maya.Hh_common import hh_RenderArnoldLayer\nreload(hh_RenderArnoldLayer)\nhh_RenderArnoldLayer.hh_RenderArnold().P5_ArnoldDisMatCreat(idpShader="ArnoldLambert")')
        mc.button(label=u'Occ',width=90,height=30,bgc=[1,1,1],command='from idmt.maya.Hh_common import hh_RenderArnoldLayer\nreload(hh_RenderArnoldLayer)\nhh_RenderArnoldLayer.hh_RenderArnold().P5_ArnoldDisMatCreat(idpShader="ArnoldOcc")')   
        mc.button(label=u'Normal',width=90,height=30,bgc=[0.4,0.55,0.9],command='from idmt.maya.Hh_common import hh_RenderArnoldLayer\nreload(hh_RenderArnoldLayer)\nhh_RenderArnoldLayer.hh_RenderArnold().P5_ArnoldDisMatCreat(idpShader="ArnoldNormal")')                       
        mc.setParent('..')        
...
#            mc.connectAttr((idpShader+'.outColor'), ( SG + '.surfaceShader'), f=True) 
    ...
    def ArnoldIDCreat(self,idpShader):
        ...
        #from idmt.maya.Hh_common import hh_RenderArnoldLayer
        #reload(hh_RenderArnoldLayer)
        #hh_RenderArnoldLayer.hh_RenderArnold().ArnoldRendererSettings('02')
        if mc.objExists(idpShader)==0:
            mc.shadingNode('aiUtility', asShader=True,n=idpShader)
        idpSG=idpShader+'SG'
...
#材质球创建
    ...
    def csl_IDRenderCreat(self,objname,layername):
        ...
        for ns in refNamespace:
            if '_' not in ns:
                continue
            if ns.split('_')[1][specialType] in ['c', 'C']:
...
#        return result         
    def csl_IDRenderLayerCreatAll(self,type='chr',ref=1):
        ...
        print '=' * 50
        print '[Test the Idp Info File exist]'
        print '=' * 50
        hasIdpInfo = False
        for ref in allRef:
            

            splitPath =  os.path.splitext(os.path.basename(ref.path))[0].split('_')
            

            idInfoPath = serverPath+'data/RLayerInfo/RGB/id'
            
            if type == 'chr':
              idInfoPath = idInfoPath + str(0)

            if type == 'set':
              idInfoPath = idInfoPath + str(1)


            if len(splitPath) > 1:
              for i in range(6):
                  if os.path.exists(idInfoPath + str(i) + '/' + splitPath[1]):
                    hasIdpInfo = True
...
#删除id信息
    ...
    def csl_fixNamespace(self):
        refFiles = pm.system.listReferences()

        for ref in refFiles:
            ns = ref.namespace
            nsSplit = os.path.basename(ref.path).split('_')
            fns = nsSplit[0] + '_' + nsSplit[1]
            com = re.compile('^' + fns)
            charComp = re.compile('^(c|p|s)', re.IGNORECASE)
            if charComp.match(nsSplit[1]):

                if not com.match(ns):
                    print ref.namespaceExists
                    if pm.namespace(exists = fns):
                        fns = fns + '_999'
                    ref.namespace = fns        
