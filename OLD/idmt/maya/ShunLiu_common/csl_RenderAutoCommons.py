...
class csl_RenderAutoCommons(object):
    ...
    def csl_RenderLayerAuto(self,shotType=3,server=1):
        ...
        tempath='E:/Info_Temp/temp/RenderLayer/'+shotInfo[1]+'/'+shotInfo[2]+'/'+shotInfo[3]+'/'
...
# 场景文件处理：
        ...
        if refconinfo[1]==1:
            ...
            hh_RenderArnoldLayer.hh_RenderArnold().ArnoldRendererSettings('03')
            lightgroups=self.csl_lightInfoList(Type='chr',lightType='group')
            for lig in lightgroups:
                if mc.objExists(lig+'.v'):
                    mc.setAttr((lig+'.v'),0)
...
#场景角色互动文件处理
    ...
    #        setmesh=self.csl_meshInfo(meshtype='s')       
            setmesh=self.csl_Attrlist(attrtype='GD')+self.csl_objInfo(meshtype='s',objtype='aiStandIn')
            ...
            if AistandIns:
                for ais in AistandIns:
                    if 'SET_GRP' in ais and mc.objExists(ais+'.castsShadows'):
                        try:
                            mc.setAttr((ais+'.overrideCastsShadows'),1)
                            mc.setAttr((ais+'.castsShadows'),0)
                        except:
                            pass
    ...
    #设置属性
            ...
            if setshpes:
                if setshpes[0]:
                    for shape in setshpes[0]:
                        ...
                        try:
                            mc.setAttr((shape+'.overrideCastsShadows'),1)
                        except:
                            pass
                        try:
            ...
            '''
            try:
                for proxy  in self.csl_objInfo(meshtype='s',objtype='aiStandIn'):
                    mc.setAttr(proxy + '.visibility', 1)
            except:
                pass
            '''
            hh_RenderArnoldLayer.hh_RenderArnold().ArnoldAOVCreat(AOVtype='AO')
    ...
    #互动idp31文件 
            print u'\n'                                  
            if shotInfo[0]=='csl' and  shotInfo[1] not in ['246']:
    ...
    def csl_setRGBPerLayer(self):
        ...
        for cc in charsLayer:
            ...
            if mGrp:
                ...
                hh_RenderArnoldLayer.hh_RenderArnold().ArnoldIDCreat('ArnoldIdpM')       
...
#角色,道具，场景idp
    def csl_id31Creat(self):
        ...
        setmeshInfo=self.csl_meshInfo(meshtype='s')+self.csl_objInfo(meshtype='s',objtype='aiStandIn') + pm.ls(type = 'aiStandIn')
    ...
    def csl_lightInfoList(self,Type='set',lightType='group'):
        ...
        if lights:
            for light in lights:
                lightP=light.split('|')
                for lig in lightP:
                    if Type=='chr' and lightType=='group' and 'msh_chr_light' in lig.lower() and lig not in lightInfoList:
                        lightInfoList.append(lig)
                    if Type=='set' and lightType=='group' and 'msh_set_light' in lig.lower() and lig not in lightInfoList:
                        lightInfoList.append(lig)
                if Type=='chr' and lightType=='key' and 'msh_chr_light' in light.lower() and 'keylight' in light.lower() and mc.listRelatives(light,p=1,f=1)[0] not in lightInfoList:
                    ligt=mc.listRelatives(light,p=1,f=1)
                    lightInfoList.append(ligt[0])
                if Type=='chr' and lightType=='fill' and 'msh_chr_light' in light.lower() and  'filllight' in light.lower() and mc.listRelatives(light,p=1,f=1)[0] not in lightInfoList:
                    ligt=mc.listRelatives(light,p=1,f=1)
                    lightInfoList.append(ligt[0])  
                if Type=='chr' and lightType=='rim' and 'msh_chr_light' in light.lower() and  'rimlight' in light.lower() and mc.listRelatives(light,p=1,f=1)[0] not in lightInfoList:
                    ligt=mc.listRelatives(light,p=1,f=1)
                    lightInfoList.append(ligt[0]) 
                if Type=='set' and lightType=='key' and 'msh_set_light' in light.lower() and 'keylight' in light.lower() and mc.listRelatives(light,p=1,f=1)[0] not in lightInfoList:
                    ligt=mc.listRelatives(light,p=1,f=1)
                    lightInfoList.append(ligt[0])
                if Type=='set' and lightType=='fill' and 'msh_set_light' in light.lower() and  'filllight' in light.lower() and mc.listRelatives(light,p=1,f=1)[0] not in lightInfoList:
                    ligt=mc.listRelatives(light,p=1,f=1)
                    lightInfoList.append(ligt[0])  
                if Type=='set' and lightType=='rim' and 'msh_set_light' in light.lower() and  'rimlight' in light.lower() and mc.listRelatives(light,p=1,f=1)[0] not in lightInfoList:
                    ligt=mc.listRelatives(light,p=1,f=1)
                if Type=='set' and lightType=='all' and 'msh_set_light' in light.lower() and mc.listRelatives(light,p=1,f=1)[0] not in lightInfoList :
                    ligt=mc.listRelatives(light,p=1,f=1)                    
                    lightInfoList.append(ligt[0])                                                                                                  
...
#时间记录
    ...
    def csl_camRenderSet(self,shortType=3,serve=0):
        ...
        if serve==1:
            ...
            # checkIn
            mel.eval('idmtProject -checkin -description \" ' + description + '\"')  
            print (u'===============!!!End【%s】!!!===============' % (u'%s_已上传文件' % shortName))                            
        return camname                                               
    ...
    #   可渲染相机设置
     ...
     #场景 motion文件               
        if refconinfo[1]==1: 
            ...
            hh_RenderArnoldLayer.hh_RenderArnold().csl_RefIm()
            mel.eval('source "zzjUtilityTools.mel";lighting_DeleteUnusedNode()') 
            meshs=self.csl_meshInfo(meshtype='s')+self.csl_objInfo(meshtype='s',objtype='aiStandIn')
            try:
                mc.select(meshs)
            except:
                pass
...
#CHR SET 组读取
    ...
    def ydRLReadEXcle(self):
        ...
        serverPath = 'Z:/Projects/' + projFullName + '/' + projFullName + '_Scratch/Render/ExcelInfo/'
        ...
        if ';' in shotData[6]:
            shotDIct['setID'] = shotData[6].split(';')[0]
        else:
