...
#        self.button = QPixmap("1.png")
#        self.win.verticalLayout_2.insertWidget(0, self.button)
        ...
        self.tableView.horizontalHeader().resizeSection(0, 135)
        self.tableView.horizontalHeader().resizeSection(1, 30)
        
        
        
        self.splitter = QtGui.QSplitter()
#        self.splitter.setStyleSheet("QSplitter{background-color:transparent;}")
        self.splitter.setStyleSheet("background-color:transparent;")

        
        
#        self.splitter.setStyleSheet("background: url(:/images/StarWar.jpg);")
...
#    horizontalHeaderView选择整列后，自动选择物体
        self.tableView.horizontalHeader().sectionClicked.connect(self.Z_HeaderViewSelected)
        
        
        
#   每隔一秒刷新一次表格        
#        self.connect(self.timer,    SIGNAL("timeout()"), self.Z_OnTimer)
#        self.timer.start( 1000 )

#    发射激光        
        self.connect(self.win.Shoot_Gun, SIGNAL("clicked()"), self.PlutoGun_Shoots)


    def PlutoGun_RefreshAircraft(self):                                          #当改变comboBox的时候，此函数，用来刷新所有控件
#    求comboBox中，选中的摄像机
...
#    设置当前的视图中的摄像机
    ...
    def PlutoGun_Shoot(self):                                           #开枪的主函数
        ...
        Bullets = cmds.listRelatives('____PlutoGun____|'+current_Lasers , c=1 , pa=1)
        try:
            len(Bullets)
        except:
            Bullets = []
    ...
    def PlutoGun_Shoot_cmd(self, frame_time,move):          #开枪函数中用到的命令
        ...
        if laser_Color == 'Red':
            ...
            if not cmds.objExists('GRP_laser_lightInder_green_001'):
                cmds.file( "Z:/Resource/Support/Maya/projects/YODA/laser_lightInder_green.mb",
                          i = 1 ,type = "mayaBinary" , rpr = "laser_lightInder" , pr = 1  )
                laser_GRP = 'GRP_laser_lightInder_green_001'
            else:
                laser_GRP = cmds.duplicate( 'GRP_laser_lightInder_green_001' )[0]                
                
        elif laser_Color == 'Blue':
            if not cmds.objExists('GRP_laser_lightInder_blue_001'):
                cmds.file( "Z:/Resource/Support/Maya/projects/YODA/laser_lightInder_blue.mb",
                          i = 1 ,type = "mayaBinary" , rpr = "laser_lightInder" , pr = 1  )
...
#   =====================ZZJ 写的函数=====================
    ...
    def Z_InitialGroup(self):                              #   如果没有____PlutoGun____这个组，就创建一个
...
#        如果self.Namespace不为空，则根据self.Namespace创建一个组
    ...
    def Z_OnTimer(self):                                   #   每隔一秒刷新一次表格，选中上次所选的cell
        indexes = self.tableView.selectedIndexes ()
#        self.Z_InitialLoad()
        for i in indexes:
            self.tableView.selectionModel().select( i, QItemSelectionModel.Select) 

    def Z_DeleteLaser(self, ):                            #   当按下delete键的时候，执行此函数
        indexes = self.tableView.selectedIndexes()
        list = []
        ...
        for a in list:
            ...
            name = '____PlutoGun____|'+self.Namespace + "_Lasers|"+str(self.model.data(name_index).toString())
            try:
                cmds.delete(name)
            except:
                pass
    ...
    def Z_SomeDataChanged(self, x, y):            #   当改变单元格内容后，显示/隐藏模型
        if x.column() == VIS:
            row = x.row()
            ...
            vis_index = self.model.index( row ,  VIS)
            vis = self.model.data(vis_index).toInt()[0]
        ...
        if x.column() == SPEED:
            ...
            vis_index = self.model.index( row ,  SPEED)
            speed = self.model.data(vis_index).toFloat()[0] *20
            
            laser_GRP_laser = cmds.listRelatives(name, c=1 , path = 1)[0]
            cmds.keyframe(laser_GRP_laser , index =(1,1) , at='tz', valueChange=speed  )
...
#----------------------------------------新的数据--------------------------------------------    
        current_Lasers = self.Namespace+"_Lasers"
        if cmds.objExists('____PlutoGun____|'+current_Lasers)==0:  return

        Lasers = cmds.listRelatives('____PlutoGun____|'+current_Lasers , c=1 , pa=1)
        try:
            len(Lasers)
        except:
            Lasers = []
...
#----------------------------------------Name-------------------------------------------              
            name = name
#----------------------------------------Vis---------------------------------------------           
            vis = cmds.getAttr(name+".v")
            if vis: vis ='1'
            else: vis ='0'
#----------------------------------------Speed------------------------------------------
#            speed = 1
            laser_GRP_laser = cmds.listRelatives(name, c=1 , path = 1)[0]
            speed = cmds.keyframe(laser_GRP_laser ,at='tz', ev=1 ,index =(1,1), q =1 )[0]
            speed = speed/20

            
#----------------------------------------数据填入model------------------------------------------
            ship = PlutoGunShip(name, vis, speed)
    ...
    def Z_init_PlutoGun_Group(self):                 #  如果没有____PlutoGun____这个组，就创建一个
        if cmds.objExists('____PlutoGun____')==0:
            cmds.group( em=True, n = '____PlutoGun____' )
...
def Z_selectOBJ(PGW, tableView):                            #   选择maya里的模型
    Namespace = PGW.Namespace
    list = []
    for i in tableView.selectedIndexes():
        if i.column() ==NAME:
            name = '____PlutoGun____|'+Namespace + "_Lasers|"+str(i.data().toString())
            list.append(name)
#   选择物体      
    try:
#        cmds.select(cl =1)
