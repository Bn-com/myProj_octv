...
class GA_RenderAutoCom(object):
    ...
    def GA_RenderAutoCom(self,shotType=2,server=1,fix=1,exr=0):
        unKonwNode=mc.ls('_UNKNOWN_REF_NODE_1')
        if unKonwNode:
            mc.delete(unKonwNode)
        ...
        if exr==1:
            ...
            if refconinfo[0]==1 and refconinfo[1]==1:
                self.GA_timeRecord()
                chrSetFile=self.GA_FileExr('chrset',2)
                self.GA_timeRecord()
                print (u'===============!!!End 【%s】!!!===============' % (u'%s_导出角色场景文件' % chrSetFile))
            #05--导出场景角色互动文件
            if refconinfo[0]==1 and refconinfo[1]==1:
                self.GA_timeRecord()
            ...
            if refconinfo[0]==1:
                ...
                GA_RedShiftRender.GA_RedShiftRender().GA_RefIm()
                print u'=====shadow属性已经设置 #Arnold 设置====='
                GA_RedShiftRender.GA_RedShiftRender().GA_RSShaderAssign(objs,'1',0)
            ...
            if refconinfo[0]==1 and refconinfo[1]==1:
                ...
                GA_RedShiftRender.GA_RedShiftRender().GA_RefIm()
                GA_RedShiftRender.GA_RedShiftRender().GA_RSShaderAssign(objs,'Lambert',0)
                setFile=self.GA_FileExr('ao',2)
                self.GA_timeRecord()
                print (u'===============!!!End 【%s】!!!===============' % (u'%s_导出场景角色AO文件' % aoFile))
            if refconinfo[0]==1:
                #角色文件
                mc.file(chrFile,options='v=0',type=fileTypeAll,f=1,o=1)
                chalightPath=lightingFile
                if refconinfo[1]==0:
                    chalightPath='//file-cluster/gdc/Projects/DouDiZhu/Doudizhu_Scratch/MODEL/AI2(Light)/cha_light.ma'
            ...
            #chr set文件
            if refconinfo[0]==1 and refconinfo[1]==1:
                mc.file(chrSetFile,options='v=0',type=fileTypeAll,f=1,o=1)
                GA_RedShiftRender.GA_RedShiftRender().GA_RSRenderLayerCreat("co","chrset",0,1,1,1,2,1,1)
                print (u'===============!!!End 【%s】!!!===============' % (u'%s_已完成创建角色场景文件' % chrSetFile))
            #IDP31文件
            if refconinfo[0]==1 and refconinfo[1]==1:
            ...
            #AO文件
            if refconinfo[0]==1 and refconinfo[1]==1:
                mc.file(aoFile,options='v=0',type=fileTypeAll,f=1,o=1)
                GA_RedShiftRender.GA_RedShiftRender().GA_RSRenderLayerCreat("co","ao",0,1,1,1,2,1,1)
                print (u'===============!!!End 【%s】!!!===============' % (u'%s_已完成创建AO文件' % aoFile))
        else:
            if refconinfo[0]==1 and refconinfo[1]==0:
                if refconinfo[1]==0:
                    chalightPath='Z:/Projects/DouDiZhu/Doudizhu_Scratch/MODEL/AI2(Light)/cha_light.ma'
                mc.file((chalightPath),i=1,pr=1)
                ...
                aoFile=GA_RedShiftRender.GA_RedShiftRender().GA_RSRenderLayerCreat("co","ao",0,1,1,1,2,1,1)
                ...
                chrSetFile=GA_RedShiftRender.GA_RedShiftRender().GA_RSRenderLayerCreat("co","chrset",0,1,1,1,2,1,1)
                self.GA_DdzLightSet('chr',0)
                chrFile=GA_RedShiftRender.GA_RedShiftRender().GA_RSRenderLayerCreat("co","chr",0,1,1,1,2,1,1)
                chalightFile=GA_RedShiftRender.GA_RedShiftRender().GA_RSRenderLayerCreat("light","chr",0,1,1,1,2,1,1)
                id31File=GA_RedShiftRender.GA_RedShiftRender().GA_RSRenderLayerCreat("id31","",0,1,1,1,2,1,1)
...
#删除不需要文件
        if server!=1:
            DeFile=[chrFile,setFile,chrSetFile,id31File,chalightFile,aoFile]
            if DeFile:
                for fil in DeFile:
...
#def csl
    ...
    def GA_FileExr(self,ftype='chr',shotType=2):
        ...
        elif ftype in ['set','id31','shadow','ao','chrset']:
