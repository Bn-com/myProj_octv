...
class GA_RedShiftRender(object):
    ...
    def GA_RSShaderAssign(self,objs,shaderType='AO',transparency=0):
        if transparency==0:
            ...
            ##创建新材质球和SG节点
            if shaderType=='AO':
                mc.shadingNode('RedshiftAmbientOcclusion', asShader=True,n=Shade)
    ...
    #AO材质
            if shaderType=='AO':
                mc.setAttr ((Shade+'.numSamples'),512)
    #Shadow材质
    ...
    def GA_MeshSet(self,renderLayer='shadow'):
        ...
        if chr:
            for ch in chr:
                ...
                if renderLayer in ['id31','chalight','chr']:
    ...
    def GA_RSAOSet(self,set=1):
        ...
        if matList:
            for mat in matList:
                ...
                if set == 1:
                    mc.setAttr((mat+'.ao_samples'),512)
                    mc.setAttr((mat+'.ao_distance'),1)
