...
#----------------------------------------------------------------------------------------------------------#     
#【通用】相机工具（渲当前帧，路径，项目，集数，可以填在_init_
#@author: hanhong
#Data：2015/9/17
...
class nj_HbExceptCam(object):
    def __init__(self,pro='nj'):
        ...
        self.pro=pro
        if pro=='nj':
            self.camPath='Z:/Projects/Ninjago/Ninjago_scratch/Animation/Cam/'
            ...
            if Cam!="frontShape" and Cam!="perspShape" and Cam!="sideShape" and Cam!="topShape":
                if Cam!="frontShape" and Cam!="perspShape" and Cam!="sideShape" and Cam!="topShape":
                        if Cam.find("cam")!=-1:
                            ...
                            ThePath=self.camPath+questEP+"/"+TheSc+'/'
                            ThePath2=self.camPath+questEP+"/"+TheSc+"/images/"
...
#
        ...
        if TheSc != optionMenu("EP", q = True, v = True):
            optionMenu( "EPNUM",label='SQ',e=1,v=TheSc )
            self.nj_ChangeReSets()
...
#
    ...
    def nj_OpenReCamPath(self):
        ...
        if questSets != '' and questEP != '':
            path="explorer"+' '+ self.camPath+questEP+"/"+questSets+"/"
            os.system(path.replace('/','\\'))
            
        else:
            path="explorer "+self.camPath
            os.system(path.replace('/','\\'))
    ...
    def nj_ImportReCam(self,Name):
        ...
        AllObj=ls(sl=1)
        path=self.camPath+questEP+"/"+questSets+"/"
    ...
    def getTheFileName(self):
        ...
        for TheFiles in TheFileName:
            if TheFiles.split(".")[-1]=="mb":
                try:
                    TheFile=TheFiles.split("/")[-1].split("_")[2]
    ...
    def nj_ChangeReSets(self):
        ...
        TheFileList=getFileList( folder=self.camPath+questEP+"/" )
        TheFileList.sort()
        if TheFileList!=None:
        ...
        sysFile(self.camPath+questEP+"/"+questSets+"/images/", makeDir=True )
    
        
        TheDateLib=[]
        mm.eval("string $GetName=python(\"optionMenu('EPNUM', q = True, v = True)\");")
        mm.eval("string $GetTheName=\"sk_\"+$GetName+\"_h\";")
        ...
        intField("TheNumber",e=1, value=len(TheDateLib),cc="from idmt.maya.py_common import GDC_BlockingCamerTools\nreload(GDC_BlockingCamerTools)\nGDC_BlockingCamerTools.nj_HbExceptCam('nj').nj_ChangeReSets4()" )    
        ...
        try:
            ...
            TheUpDateSc=os.listdir(self.camPath+questEP+"/"+questSets+"/images/")
            TheUpDateSc.sort()
            Thejpg=[]
            ...
            for j in range(len(Thejpg)):
                    theCommand='from idmt.maya.py_common import GDC_BlockingCamerTools\nreload(GDC_BlockingCamerTools)\nGDC_BlockingCamerTools.nj_HbExceptCam("nj").nj_ImportReCam(\"'+Thejpg[j]+'\")'
                    theCommand2='from idmt.maya.py_common import GDC_BlockingCamerTools\nreload(GDC_BlockingCamerTools)\nGDC_BlockingCamerTools.nj_HbExceptCam("nj").nj_ImageOk(\"'+Thejpg[j]+'\")'
                    theCommand3='from idmt.maya.py_common import GDC_BlockingCamerTools\nreload(GDC_BlockingCamerTools)\nGDC_BlockingCamerTools.nj_HbExceptCam("nj").nj_CancleHook(\"'+Thejpg[j]+'\")'
                    iconTextButton("Temp"+str(j) ,e=1,visible=1,bgc=(1,1,1),style='iconAndTextVertical', image1=self.camPath+questEP+"/"+questSets+"/images/"+Thejpg[j], label=Thejpg[j].split(".")[0],c=theCommand )
            ...
            try:
                    TheOKDateSc=os.listdir(self.camPath+questEP+"/"+questSets+"/images/old/")
    ...
    def nj_ChangeReSets2(self):
        ...
        try:
            ...
            questSets=TheSc
            sysFile(self.camPath+questEP+"/"+questSets+"/images/", makeDir=True )
            optionMenu( "EPNUM",label='SQ',e=1,v=questSets )
    ...
    def nj_ImageOk(self,Name):
        ...
        if user in  self.user:
            questSets = optionMenu("EPNUM", q = True, v = True)
            questEP = optionMenu("EP", q = True, v = True)
            path=self.camPath+questEP+"/"+questSets+"/images/"+Name
            path02=self.camPath+questEP+"/"+questSets+"/images/old/"+Name
    ...
    def nj_CancleHook(self,Name):
        ...
        if user in self.user:
            try:
                questSets = optionMenu("EPNUM", q = True, v = True)
                questEP = optionMenu("EP", q = True, v = True)
                path01=self.camPath+questEP+"/"+questSets+"/images/"+Name
                path02=self.camPath+questEP+"/"+questSets+"/images/old/"+Name
                questFile=os.path.exists(path02)
                if questFile!=0:
                    os.remove(path01) 
    ...
    def nj_ChangeColor(self):
        ...
        questSets = optionMenu("EPNUM", q = True, v = True)
        questEP = optionMenu("EP", q = True, v = True)
        path=self.camPath+questEP+"/"+questSets+"/images/"
        ...
        for j in range(len(Thejpg)):
                ...
                if Thejpg[j].find(getTheSelect[0])==-1:
                    iconTextButton("Temp"+str(j) ,e=1,visible=1,bgc=(1,1,1),style='iconAndTextVertical', image1=self.camPath+questEP+"/"+questSets+"/images/"+Thejpg[j], label=Thejpg[j].split(".")[0],c=theCommand )
                if Thejpg[j].find(getTheSelect[0])!=-1:
                    iconTextButton("Temp"+str(j) ,e=1,visible=1,bgc=(1,0,0),style='iconAndTextVertical', image1=self.camPath+questEP+"/"+questSets+"/images/"+Thejpg[j], label=Thejpg[j].split(".")[0],c=theCommand )
    
                menuItem("TheObMenu"+str(j),e=1,l="This Camera is ok",c=theCommand2 )
                menuItem("Cancle"+str(j),e=1,l="cancle the hook",c=theCommand3 )
    
    def nj_SkReCamToolsUI(self):
        shotInfo = sk_infoConfig.sk_infoConfig().checkShotInfo()
        serverPath = sk_infoConfig.sk_infoConfig().checkProjectTDPath()
        projectInfo = sk_infoConfig.sk_infoConfig().checkProjectNameSimple2Full(shotInfo[0])         
        EPNum=self.EPNum
        ...
        optionMenu( "EP",label='EP',cc="from idmt.maya.py_common import GDC_BlockingCamerTools\nreload(GDC_BlockingCamerTools)\nGDC_BlockingCamerTools.nj_HbExceptCam('nj').nj_ChangeReSets3()" )
        if self.pro=='nj':
            for i in range(len(self.EPNum)):
                menuItem( i,label=self.EPNum[i])
        else:       
    ...
    #        menuItem( 'C'+i,label='C'+i)
        ...
        optionMenu( "EPNUM",label='SQ',cc="from idmt.maya.py_common import GDC_BlockingCamerTools\nreload(GDC_BlockingCamerTools)\nGDC_BlockingCamerTools.nj_HbExceptCam('nj').nj_ChangeReSets()" )
        TheFileList=getFileList( folder=self.camPath+questEP+"/" )
        for m in range(1000):
        ...
        button("OKBUTTON",l = "export all cam", c = "from idmt.maya.py_common import GDC_BlockingCamerTools\nreload(GDC_BlockingCamerTools)\nGDC_BlockingCamerTools.nj_HbExceptCam('nj').nj_ExceptReCam()")
    
        button("OKBUTTON02",l = "exp selected cam", c = "from idmt.maya.py_common import GDC_BlockingCamerTools\nreload(GDC_BlockingCamerTools)\nGDC_BlockingCamerTools.nj_HbExceptCam('nj').nj_ExceptSelectReCam()")
        button("OKBUTTON03",l = "open path", c = "from idmt.maya.py_common import GDC_BlockingCamerTools\nreload(GDC_BlockingCamerTools)\nGDC_BlockingCamerTools.nj_HbExceptCam('nj').nj_OpenReCamPath()")
        intField("TheNumber", value=100,cc="from idmt.maya.py_common import GDC_BlockingCamerTools\nreload(GDC_BlockingCamerTools)\nGDC_BlockingCamerTools.nj_HbExceptCam('nj').nj_ChangeReSets4()" )
        ...
        textScrollList("TheDateLibSc",w=128,h=600,allowMultiSelection=True,numberOfRows=8,selectCommand="from idmt.maya.py_common import GDC_BlockingCamerTools\nreload(GDC_BlockingCamerTools)\nGDC_BlockingCamerTools.nj_HbExceptCam('nj').nj_ChangeColor()")
        setParent("..")
        ...
        TheUpDateSc = []
        if os.path.isdir(self.camPath+questEP+"/"+questSets+"/images/"):
            TheUpDateSc=os.listdir(self.camPath+questEP+"/"+questSets+"/images/")
        TheOKDateSc = [];
        if os.path.isdir(self.camPath+questEP+"/"+questSets+"/images/"):
            TheOKDateSc=os.listdir(self.camPath+questEP+"/"+questSets+"/images/")
