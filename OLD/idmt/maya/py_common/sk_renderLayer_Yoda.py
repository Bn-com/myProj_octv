...
class sk_renderLayer_Yoda(object):
    ...
    def ydRLAutoCreate(self, createMode = 0 ,refClean = 1 , RGB = 0):
        print ('=================================================================')
        ...
        for key in keyInfos:
            ...
            # Ambient Occlusion
            self.ydRLAOCreate(key)
            # Normal
            self.ydRLNMCreate(key)
            # Light
            self.ydRLFNCreate(key)
    ...
    def ydRLSpeficCreate(self, renderLayer, saveMode='', selectMode=0, arnoldType=0):
        ...
        if layerType == 'PREVIEW':
            ...
            self.ydRLPVCreate(BGType, selectObjType=selectMode)

        # COLOR
        if layerType == 'CO':
            if mc.ls(BGType + '_' + layerType):
                mc.delete(BGType + '_' + layerType)
            self.ydRLCOCreate(BGType, selectObjType=selectMode)
        ...
        if layerType == 'AO':
            if mc.ls(BGType + '_' + layerType):
                mc.delete(BGType + '_' + layerType)
            if arnoldType == 0:
                self.ydRLAOCreate(BGType, selectObjType=selectMode)
            if arnoldType == 1:
                self.ydRLAOArnoldCreate(BGType, selectObjType=selectMode)
        # Normal
        if layerType == 'NM':
            if mc.ls(BGType + '_' + layerType):
                mc.delete(BGType + '_' + layerType)
            if arnoldType == 0:
                self.ydRLNMCreate(BGType, selectObjType=selectMode)
            if arnoldType == 1:
                self.ydRLNMArnoldCreate(BGType, selectObjType=selectMode)
        # Frenel
        if layerType == 'FN':
            if mc.ls(BGType + '_' + layerType):
                mc.delete(BGType + '_' + layerType)
            self.ydRLFNCreate(BGType, selectObjType=selectMode)
        # Zdepth
        if layerType == 'ZDP':
            if mc.ls(BGType + '_' + layerType):
                mc.delete(BGType + '_' + layerType)
            self.ydRLZDEPTHCreate(BGType, selectObjType=selectMode)
        # SPE
        if layerType == 'SPEC':
            if mc.ls(BGType + '_' + layerType):
                mc.delete(BGType + '_' + layerType)
            self.ydRLSPECCreate(BGType, selectObjType=selectMode)
        # RIM
        if layerType == 'RIM':
            if mc.ls(BGType + '_' + layerType):
                mc.delete(BGType + '_' + layerType)
            self.ydRLRIMLIGHTCreate(BGType, selectObjType=selectMode)
        # Light
        if layerType == 'LIGHT':
            if mc.ls(BGType + '_' + layerType):
                mc.delete(BGType + '_' + layerType)
            self.ydRLLIGHTCreate(BGType, selectObjType=selectMode)
        # Light_New
        if layerType == 'LGTSYS':
            if mc.ls(BGType + '_' + layerType):
                mc.delete(BGType + '_' + layerType)
            self.ydRLLIGHTNewCreate(BGType, selectObjType=selectMode)

        if layerType == 'LTSYS':
            if mc.ls(BGType + '_' + layerType):
                mc.delete(BGType + '_' + layerType)
            self.ydRLLIGHTCHCreate(BGType, selectObjType=selectMode)
        # SHDW
        if layerType == 'SHDW':
            if mc.ls(BGType + '_' + layerType):
                mc.delete(BGType + '_' + layerType)
            self.ydRLSDCreate(BGType, selectObjType=selectMode)
        # LINE
        if layerType == 'LINE':
            if mc.ls(BGType + '_' + layerType):
                mc.delete(BGType + '_' + layerType)
            self.ydRLLineCreate(BGType, selectObjType=selectMode)
        # CAUSTICS
        if layerType == 'CAUSTICS':
            if mc.ls(BGType + '_' + layerType):
                mc.delete(BGType + '_' + layerType)
            self.ydRLCausticsCreate(BGType, selectObjType=selectMode)
        # MASK_01
        if layerType == 'MASK01':
            if mc.ls(BGType + '_' + layerType):
                mc.delete(BGType + '_' + layerType)
            self.ydRLMSK01reate(BGType, selectObjType=selectMode)

        if layerType == 'MASK02':
            if mc.ls(BGType + '_' + layerType):
                mc.delete(BGType + '_' + layerType)
            self.ydRLMSK02reate(BGType, selectObjType=selectMode)
        # MASK_01
        if layerType == 'MSK':
            if mc.ls(BGType + '_' + layerType):
                mc.delete(BGType + '_' + layerType)
            self.ydRLSeaMSKreate()

        if layerType == 'MASK':
            if mc.ls(BGType + '_' + layerType):
                mc.delete(BGType + '_' + layerType)
            self.ydRLMskCreate(BGType, selectObjType=selectMode)
    ...
    #----------------------------------------------------------#
    # 渲染标准设置
    def ydRLCommonConfig(self , layerName = '' ,shotType = 2):
    ...
    # ZIP code
    #----------------------------------------------------------#
    def ydRLFramebuffer(self, fileType = 'exr',datatype=4):    
        mc.setAttr('defaultRenderGlobals.animation', 1)
        mc.setAttr('defaultRenderGlobals.putFrameBeforeExt', 1)
        mc.setAttr('defaultRenderGlobals.periodInExt', 1)
        mc.setAttr('defaultRenderGlobals.extensionPadding', 4)
    ...
    def ydRLObjsInfo(self,layerType,renderLayer):
        ...
        if layerType == 'CHRM':
            ...
            if renderLayer in ['BSR']:
                MaskObjs = [(refPRP + refSET + refCHR_F + refCHR_B),[],[]]
                if layerObjs:
                    layerObjs= refCHR_M + refPRP + refSET + refCHR_F + refCHR_B
            if renderLayer in ['FBR']:
        ...
        if tempMeshes:
            for mesh in tempMeshes:
                ...
                if '.f[' not in mesh:
                    ...
                    obj = mesh.split('.f[')[0]
                if outObjs:
                    if obj in inObjs and obj not in outObjs:
    ...
    def ydRLObjsTypesInfos(self):
        ...
        shotInfo = sk_infoConfig.sk_infoConfig().checkShotInfo()
        serverPath = sk_infoConfig.sk_infoConfig().checkProjectServerPath()
        serverTransPath = serverPath + 'data/renderLayerInfos/' + shotInfo[0] + '/' + str(shotInfo[1]) + '/' + str(shotInfo[2]) + '/'
    ...
    #----------------------------------------------------------#
    # 获取有透明贴图的物体 | 文件读取
    def ydRLTransparencyObjsOld(self, valueCheck = 0):
        '''
        transparencySG = []
        ...
        for SGNode in SGNodes:
            ...
            if doNot == 0:
                ...
                if ':' in SGNode:
                    ...
                    needMeshes = mc.ls(meshes,l = 1)
                    # 记录信息
                    transparencySG.append([SGNode, needMeshes, transpancyNode[0]])
                else:
                    meshes = mc.sets(SGNode, q=1)
                    needMeshes = mc.ls(meshes,l = 1)
                    # 记录信息
        ...
        for j in range(len(transparencySG)):
            ...
            recordState = 1
        transparencySG = [needSGNodes,needSGMeshes,needTransNodes]
        return transparencySG
    
    #----------------------------------------------------------#
    # 按类别获取该层有的透明信息
    def ydRLTransparencyObjsByType(self,rlObjs,layerType):
        transpancyObjs = self.ydRLTransparencyObjsOld()
        transpancySGNodes = []
        transpancyMeshes = []
        transpancyNode = []
        ...
        return [transpancySGNodes,transpancyMeshes,transpancyNode]
    ...
    def ydRLTransShaderConnection(self, transpancyNode, transShader, transShaderAttr):
        if transpancyNode[:6] == '[food]':
            ...
            if mc.nodeType(transpancyNode.split('.')[0]) in['layeredShader','layeredTexture', 'surfaceShader', 'ramp', 'file', 'reverse']:
    ...
    #--------------------------------#
    # 选取模式
    def ydRLSelectMode(self,selectObjType):
        # 选取模式
        rlObjs = []
        # 选取模式
    ...
    def ydRLReadEXcle(self):
        ...
        shotData = shotAllData.row_values(int(shotInfo[1]) + 1)  
    ...
    #--------------------------------#
    # Color层
    def ydRLCOCreate(self, layerType, selectObjType=0):
    ...
    # 需要开启FG渲染
    # No Lights
    def ydRLAOCreate(self, layerType, selectObjType=0):
    ...
    def ydRLNMCreate(self, layerType, selectObjType=0):
    ...
    # ZDEPTH层
    # No Lights
    def ydRLZDEPTHCreate(self, layerType, selectObjType=0, distance=14000):
    ...
    # Fresnel层
    # No Lights
    def ydRLFNCreate(self, layerType, selectObjType=0):
    ...
    #-------------------------------------------------#
    # RGB自动创建函数
    def ydRLRGBAutoCreate(self,layerType):     
        # BSR
        self.ydRLRGBCreate(layerType + '_' + 'BSR')
        # Back To MasterLayer
        mel.eval('editRenderLayerGlobals -currentRenderLayer "defaultRenderLayer"')
        # FBR
        self.ydRLRGBCreate(layerType + '_' + 'FBR')
        # Back To MasterLayer
        mel.eval('editRenderLayerGlobals -currentRenderLayer "defaultRenderLayer"')
        # SPR
        self.ydRLRGBCreate(layerType + '_' + 'SPR')
        # Back To MasterLayer
        mel.eval('editRenderLayerGlobals -currentRenderLayer "defaultRenderLayer"')
        
    #-------------------------------------------------#
    # 核心创建层函数
    def ydRLRGBCreate(self, layerType):
        print (u'===============!!!Start 【%s】!!!===============' % (u'%s_%s层' % (layerType.split('_')[0],layerType.split('_')[1])))
        print 'Working...'

        chekInfo = layerType.split('_')
        # 缺少物体
        # 物体信息
        needInfo = self.ydRLObjsInfo(chekInfo[0],chekInfo[1])
        rlObjs    = needInfo[0]
        MaskObjs  = needInfo[1]
        layerName = needInfo[2]
        rlSGNodes = needInfo[3]
        ...
        if layerObjs:
                ...
                for obj in objs:
                    ...
                    try:
                        ...
                        if '.f[' not in mesh:
                            ...
                            try:
                                mc.sets((obj + u'.f[0:' + str(faceNum - 1) + u']'), e=1, forceElement = shaderSG)
                            except:
                                print '---[Check]---'
                                print obj
                                # 该死的maybug！！！再次分解
                                mc.sets((obj + u'.f[0:' + str(int(faceNum - 1) / 2) + u']'), e=1, forceElement = shaderSG)
                                mc.sets((obj + u'.f[' + str(int(faceNum - 1) / 2 + 1) + ':' + str(faceNum - 1) + u']'), e=1, forceElement = shaderSG)
                        else:
                            # print u'>-------------->'
                            # print mesh
                            # 获取面数
                            ...
                            if ':' in faceInfo:
                                ...
                                if faceNumPos == faceNumPre + 1:
                                    mc.sets((obj + u'.f[' + str(faceNumPre) + u']'), e=1, forceElement = shaderSG)
                                    mc.sets((obj + u'.f[' + str(faceNumPos) + u']'), e=1, forceElement = shaderSG)
                                else:
                                    mc.sets((obj + u'.f[' + str(faceNumPre) + ':' + str(int(faceNumPre + (faceNumPos - faceNumPre) / 2)) + u']'), e=1, forceElement = shaderSG)
                                    mc.sets((obj + u'.f[' + str(int(faceNumPre + 1 + (faceNumPos - faceNumPre) / 2)) + ':' + str(faceNumPos) + u']'), e=1, forceElement = shaderSG)
                            else:
            ...
            if matteObjs:
                for obj in matteObjs:
                    try:
                        mc.sets(obj , e = 1 , forceElement = MatteSG)
                    except:
                        print '--------'
                        print obj
        ...
        for mesh in meshes:
            ...
            if mc.nodeType(mesh) == 'mesh':
                if '.f[' in mesh:
                    ...
                    grp = mc.listRelatives(mesh,p=1,type = 'transform')
                if not grp:
                    continue
                ...
                for info in allInfos:
                    ...
                    for k in range(len(info)):
                        ...
                        if '|' not in info[k]:
                            ...
                            needMeshes.append(needMeshFull)
                    #needMeshes = mc.listRelatives(needMeshes , f = 1)
    ...
    def ydRLayerZdepthDistanceConfigUI(self):
        ...
        mc.rowLayout()
        mc.button(label=u'[变更设置]', width=320, c='sk_renderLayer_Yoda.ydRLConfig().ydRLayerZdepthDistanceConfig()')
        mc.setParent("..")
