...
class sk_sceneConfig(object):
    ...
    def sk_sceneCameraUpdate(self,batchUpadate = 0 , info = 2):
        import sk_infoConfig
        reload(sk_infoConfig)
        ...
        # bakeCame
        elif shotInfo[0] == u'do5':#=====DO5 检查摄像机interaxialSeparation 和 zeroParallax 属性值 是否是默认的======================
            interSep_v = PyNode(camSourceName).getShape().interaxialSeparation.get()
            zeroPar_v =  PyNode(camSourceName).getShape().zeroParallax.get()  
            if interSep_v == 6.350 or zeroPar_v == 254.000:
                error(u'摄像机属性 : +++interaxialSeparation++++zeroParallax+++不应该为6.350 和 254.000')
        if mc.ls(camBakeName):
            mc.delete(camBakeName)
        if needCam:
            exp=mc.listConnections(needCam,d=1,type='expression')
            if exp:
                mc.delete(exp)
#删除残留表达式
            mc.select(needCam)
    ...
    def sk_sceneProxySetAdd(self):
        ...
        proxyGrps = mc.ls('*MSH_c_hi_proxy_*', type='transform',l=1) + mc.ls('*:*MSH_c_hi_proxy_*', type='transform',l=1)
        if proxyGrps:
            # 创建
                            ...
                            if masterNO1:
                                ...
                                if meshes:
                                    ...
                                    for mesh in meshes:
                                        ...
                                        NO1Objs.append(mc.listRelatives(mesh,p = 1 ,type = 'transform')[0])       
                                    # 记录好shaderNOInfo后开始强制赋予材质
                                    rootGrp = mc.listRelatives(newMaster, p = 1 , type = 'transform',f=1)[0]
                                    print '--------'
                                    print rootGrp
                                    meshes = mc.listRelatives(rootGrp , ad = 1 ,type = 'mesh',f = 1)
                                    print meshes
                                        ...
                                        for key in shaderSGNO1info:
                                            if objSimple != 'MSH_c_hi_proxy_':
                                                for meshGrp in shaderNO1Info[key]:
                                                    ...
                                                    # 传UV
                                                    for No1Obj in NO1Objs:
                                                        if No1Obj.split('_food')[0] == obj.split('|')[-1].split('_food')[0] and '_proxy_' not in No1Obj:
                                                            print u'>>>>>>>>'
                                                            print No1Obj
                                                            print obj.split('|')[-1]
                                                            mc.transferAttributes( No1Obj, obj, transferUVs=2, transferColors=2 ,sampleSpace = 4  )  
    ...
    def sk_sceneAutoReplace(self,modelType=1, animFile=0 ,save = 0):
        ...
        if proxySel:
            ...
            if emptyProxy[0] == []:
                ...
                print(u'==================有【空】的proxy，请清理后再替换==================')
                mc.error(u'==================有【空】的proxy，请清理后再替换==================')
        else:
            print(u'==================未发现proxy，请检查==================')
    ...
    def sk_sceneVegetationCacheTypeExport(self, server = 1):
        ...
        if shotInfo[1][0] != 'p':
            ...
            # 执行缓存创建
            if server == 0:
                sk_checkCommon.sk_checkTools().checkCacheDoCreate((fileName + '_0'), cacheObjs, localPathCache,0,0)
                # 输出分段物体信息
            ...
            if sceneVegetationTypeInfo:
                ...
                for objGrp in sceneVegetationTypeInfo:
                    ...
                    if objGrp[0] in usefulList:
                        ...
                        # 纠错
                        if not objGrp[1]:
                            print u'=============有空的proxy，请清理掉再操作============='
                            mc.error(u'=============有空的proxy，请清理掉再操作=============')
                        ...
                        if len(objGrp[1]) > 1:
                            ...
                            for cacheObj in cacheServerObjs:
                                ...
                                for k in range(len(objGrp[1])):
                                    # 打补丁。。。
                                    objNameTemp = objName
                                    if '_ca_' in objNameTemp:
                                        objNameTemp = objNameTemp.replace('_ca_','_')
                                    checkTemp = objGrp[1][k]
                                    if '_ca_' in checkTemp:
                                        checkTemp = checkTemp.replace('_ca_','_')
                                    print '*****'
                                    print objNameTemp
                                    print checkTemp
                                    if objNameTemp in checkTemp:
    ...
    def sk_sceneImportAudio(self):
        ...
        if shotInfo[0] in audioIList:
            ...
            if camProjectInfos == "LION":
                audioPath += audioProjectInfos + shotInfo[2] + '.wav'
            else:
            ...
            if finalLayout == 2:
                if root:
                    if root not in keepRoot:
                        if 'cam' not in root:        #梁宇：把动画相机（非参考的bake相机）不加到OTC组中，避免导出OTC文件后随带相机而相机与动画层有连接关系，丢失连接
    ...
    def sk_sceneAssetNamespaceConfig(self):
        ...
        for refPath in allRefPaths:
            ...
            # 多个asset时
                ...
                allIndex = sk_checkCommon.sk_checkTools().checkListSameAllIndex(refPaths,refPath)
    ...
    def sk_sceneGRPDelete(self, fileGRP='OTC'):
        ...
        fileFomat = '.ma'
        renderFilePathServer = sk_infoConfig.sk_infoConfig().checkCacheServerPath(2)
    ...
    def sk_sceneSETRefShaderReset(self , info , serverModify = 1):
        ...
        if serverModify == 1:
            needFilePath = sk_infoConfig.sk_infoConfig().checkCacheServerPath(2)
    ...
    def sk_sceneGRPImport(self, fileGRP='OTC'):
        ...
        if os.path.exists(otcFileServer):
            ...
            if fileGRP == 'SET':
                ...
                # ma模式处理
                animRefInfos = sk_infoConfig.sk_infoConfig().checkFileRead(otcFileServer)
                renderRefInfos = []
                for line in animRefInfos:
                    if '_ms_anim.m' in line:
                        line = line.replace('_ms_anim.m','_ms_render.m')
                    renderRefInfos.append(line)
                sk_infoConfig.sk_infoConfig().checkFileWrite(otcFileServer,renderRefInfos)
                '''
                # 导入,必须这里清理
                print '---'
                print otcFileServer
    ...
    def sk_sceneReferenceInfoCheck(self):
        ...
        if shotInfo[3] == 'an' and localRef:
            # 动画文件里本地参考警告
            print(u'===============【错误】有【%s】个本地参考===============' % (localRef))
            mc.error(u'===============【错误】有【%s】个本地参考===============' % (localRef))
            
        if shotInfo[3] == 'an' and notAnimRef:
            # 白海豚报错
            if shotInfo[0] == 'zm':
                print(u'===============【错误】有【%s】个非anim参考===============' % (notAnimRef))
    ...
    def sk_skinAndCopyA2B(self,sourceObj = '',targetObj= '', selectMode = 0 ):
        ...
        if joints:
            if mc.nodeType(joints[0])== 'joint':
                ...
                print(u'=============选取源【%s】没有蒙皮骨骼============='%sourceObj)
    ...
    def sk_sceneTreeplaneAimCam(self,setType = 1):
        ...
        # cali处理
        if shotInfo[0] == 'cl':
            planeGrps = mc.ls("*:c_BIL*",type="transform")
            nameCAM = 'CAMERA'
        # 清理animC
    ...
    def sk_sceneGlobalAllAssetMasterCheck(self):
        import sk_infoConfig
        reload(sk_infoConfig)
        import os
        # 获取项目所有asset ID
                        ...
                        if assetFiles:
                            for j in range(3):
                                ...
                                for fileName in assetFiles:
                                    if (HMLType + '_ms_anim.m') in fileName and '_ng_' not in fileName:
                                        animNum = animNum + 1
                                    if (HMLType + '_ms_render.m') in fileName and '_ng_' not in fileName:
    ...
    # 全局扫描面材质|角色道具
    def sk_sceneGlobalAllAssetFaceShaderCheck(self):
        import sk_infoConfig
        reload(sk_infoConfig)
        import os
        # 获取项目所有asset ID
                ...
                for asset in assetInfo:
                    ...
                    if os.path.exists(assetServerPath):
                        ...
                        if assetFiles:
                            for fileName in assetFiles:
                                # 不允许'_ng_'在文件名里
                                if '_ms_render.m' in fileName and shotInfo[0] in fileName and '_ng_' not in fileName:
    ...
    # 全局扫描SmoothSet            
    def sk_sceneGlobalAllAssetSmoothSetCheck(self):  
        import sk_infoConfig
        reload(sk_infoConfig)
        import sk_checkCommon
        reload(sk_checkCommon)
    ...
    # 全局扫描renderState
    def sk_sceneGlobalModelRenderStateCheck(self):  
        import sk_infoConfig
        reload(sk_infoConfig)
        import sk_checkCommon
        reload(sk_checkCommon)
    ...
    # 全局扫描anim和render版本差异
    def sk_sceneGlobalAnimRenderDifChcek(self):  
        import sk_infoConfig
        reload(sk_infoConfig)
        import sk_checkCommon
        reload(sk_checkCommon)
        ...
        for asset in allAsset:
            ...
            # 全局路径
            needFolder = '/master/'
            fileKeyAnimInfo = 'ms_anim.m'
            fileKeyRenderInfo = 'ms_render.m'
            assetServerPath = serverPath + 'scenes/' + assetType + '/' + asset + needFolder
            # 处理不同类型的高中低模
            for assetHMLType in modelHML:
                if os.path.exists(assetServerPath):
                    # 获取文件名
                    assetFiles = mc.getFileList(folder = assetServerPath)
                    if assetFiles:
                        ...
                        # 不允许'_ng_'在文件名里
                        for fileName in assetFiles:
                            if (assetHMLType + fileKeyAnimInfo) in fileName and assetType in fileName and shotInfo[0] in fileName and '_ng_' not in fileName:
                                animFile = fileName
                            if (assetHMLType + fileKeyRenderInfo) in fileName and assetType in fileName and shotInfo[0] in fileName and '_ng_' not in fileName:
                                renderFile = fileName
    ...
    def sk_sceneGlobalAnim2RenderSetInfo(self,checkType = 0 , modelType = 2):
        # 获取项目信息
        import sk_infoConfig
        reload(sk_infoConfig)
        import os
            ...
            if assetInfo[0]:
                ...
                if checkType == 4:
                    ...
                    if assetName in assetInfo[0]:
                        ...
                        print(u'==============================[%s]不在本项目asset中=============================='%assetName)
    ...
    # 全局扫描 rg tx cachelist animlist
    def sk_sceneGlobalCacheAnimListChcek(self):  
        import sk_infoConfig
        reload(sk_infoConfig)
        import sk_checkCommon
        reload(sk_checkCommon)
        ...
        keyDonotNeed = '_ng_'
        # serverInfoPath
        localInfoPath = localPath + 'cacheAnimListInfo/'
        mc.sysFile(localInfoPath, makeDir=True)
        serverInfoPath = serverPath + 'data/AssetInfos/cacheAnimListInfo/'
        makeDirCMD = 'zwSysFile(\"MD\",\"' + serverInfoPath + '\",\"\",1)'
        mel.eval(makeDirCMD)
        # 清理掉历史文件
        localOldFiles = mc.getFileList(folder = localInfoPath)
                ...
                for assetHMLType in modelHML:
                    if os.path.exists(assetServerPath):
                        ...
                        if assetFiles:
                            for fileName in assetFiles:
                                # 不允许'_ng_'在文件名里
                                if (assetHMLType + fileKeyInfo) in fileName and shotInfo[0] in fileName and '_ng_' not in fileName:
                                    # 打开文件
                                    mc.file((assetServerPath + fileName ) , open = 1, f = 1)
                                    
                                    # cacheList检测
                                    caObjs = mc.ls('*_ca_*',type = 'transform',l = 1)
                                    if caObjs:
                                        need = []
                                        for obj in caObjs:
                                            if '|MODEL|' in obj and obj[-1] == '_':
                                                objSimple = obj.split('|')[-1]
                                                if  '_ct_an' not in objSimple and '_nr_' not in objSimple and '_si_' not in objSimple and '_proxy_' not in objSimple:
                                                    need.append( objSimple )
                                        caObjs = need
                                    objsCaList = []
                                    if mc.ls('MESHES'):
                                        objsCaList = mc.sets('MESHES', q = 1)
                                    ...
                                    if objsCaList == None:
                                        objsCaList = []
                                    if caObjs == [] and objsCaList == []:
                                        pass
                                    else:
                                        # 只要有一个为空，则出错
                                        if (caObjs + objsCaList) == caObjs or  (caObjs + objsCaList) == objsCaList :
                                            cacheListState = 0
                                    if caObjs and objsCaList:
                                        # 数量不对，也错
                                        if len(caObjs) != len(objsCaList):
                                            cacheListState = 0
                                    
                                    # animList检测
                                    ctObjs = mc.ls('*_ct_an*',type = 'transform',l = 1)
                                    if ctObjs:
                                        need = []
                                        for grp in ctObjs:
                                            if '|MODEL|' in grp and grp[-1] != '_':
                                                grpSimple = grp.split('|')[-1]
                                                if  '_ca_' not in grpSimple and '_nr_' not in grpSimple and '_si_' not in grpSimple and '_proxy_' not in grpSimple:
                                                    need.append( grpSimple )
                                        ctObjs = need
                                    grpsCtList = []
                                    if mc.ls('CTRLS'):
                                        grpsCtList = mc.sets('CTRLS', q = 1)
                                    ...
                                    if grpsCtList == None:
                                        grpsCtList = []
                                    if ctObjs == [] and grpsCtList == []:
                                        pass
                                    else:
                                        # 只要有一个为空，则出错
                                        if (ctObjs + grpsCtList) == ctObjs or  (ctObjs + grpsCtList) == grpsCtList :
                                            animListState = 0
                                    if ctObjs and grpsCtList:
                                        # 数量不对，也错
                                        if len(ctObjs) != len(grpsCtList):
                                            animListState = 0
                                    
                                    errorInfo = []
                                    if cacheListState == 0:
                                        errorInfo.append(fileKeyInfo[:2] + ' file')
                                        errorInfo.append(u'=====Cache List Error=====')
                                    if animListState == 0:
                                        errorInfo.append(fileKeyInfo[:2] + ' file')
                                        errorInfo.append(u'=====Anim  List Error=====')
                                    
                                    # 输出信息
                                    if errorInfo:
                                        sk_infoConfig.sk_infoConfig().checkFileWrite((localInfoPath + asset + assetHMLType + '_cacheAnimListError.txt'), errorInfo)
                                        updateAnimCMD = 'zwSysFile "copy" ' + '"' + (localInfoPath + asset + assetHMLType + '_cacheAnimListError.txt' ) + '"' + ' ' + '"' + (serverInfoPath + asset + assetHMLType + '_cacheAnimListError.txt') + '"' + ' true'
                                        mel.eval(updateAnimCMD)
    ...
    def sk_sceneGlobalAssetTextureInfoExport(self):   
        # 获取asset信息
        import sk_infoConfig
        reload(sk_infoConfig)
        import sk_checkCommon
        ...
        # 输出路径
        print u'/n=====请到下面路径查询[全asset]透明信息====='
        print (serverPath + 'data/AssetInfos/transShaderInfo/')
        
    # 全局扫描
    # 将所有asset的tx文件输出置换贴图信息
    def sk_sceneGlobalAssetDisplacementInfoExport(self):   
        # 获取asset信息
        import sk_infoConfig
        reload(sk_infoConfig)
        import sk_checkCommon
                    ...
                    if assetFiles:
                        for fileName in assetFiles:
                            ...
                            if fileKeyInfo in fileName  and shotInfo[0] in fileName and '_ng_' not in fileName:
                                ...
                                mc.file(checkFile ,open = 1, force = 1 , loadReferenceDepth = 'all' )
                                # 输出置换信息
                                sk_checkCommon.sk_checkTools().checkDisplacementObjsInfoExport()
        # 输出路径
        print u'/n=====请到下面路径查询[全asset]置换信息====='
        print (serverPath + 'data/AssetInfos/displacementShaderInfo/')
