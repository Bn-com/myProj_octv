...
class sk_ProjectTools_Yoda(object):
    ...
    #anim =0,不弹出动画提醒
    #-------------#
    def yd_rebuildClean(self , serverClean = 0 , norenderConfig = 0,Constraints=1,anim=0,camcheckin=1):
        # 开始处理
        ...
        for i in range(len(refNodes)):
            if 'CAMRN' in refNodes[i]:
                mc.file(refPaths[i] ,ir=1)
            if 'Z:/Projects/Ninjago/Project' in refPaths[i]:
                refPathN=refPaths[i].replace('Z:/Projects/Ninjago/Project','//file-cluster/GDC/Projects/Ninjago/Project')
                mc.file(refPathN,loadReference=refNodes[i],type='mayaBinary',options='v=0')
        #
        refInfo = sk_referenceConfig.sk_referenceConfig().checkReferenceListInfo()
        refNodes = refInfo[0][0]
        refPaths = refInfo[1][0]
        ...
        if 'EW' not in shotInfo[1] and  'IW' not in shotInfo[1] and shotInfo[1] not in ['EW010','E7065','E7066','E7067','E7068','E7069','E7070','E7071','E7072','E7073','E7074','I7066']:
