...
# low=1 针对需要低模的项目，low=0 针对不需要低模的项目
    ...
    def gdc_ArnoldProxyCreat(self,meshH,meshL,low):
        ...
        #创建代
        Arnold=mtoa.core.createStandIn(path=(proxypath+ass)).name()
        ...
        if low!=0:
            ...
            'aiVisibleInDiffuse','aiVisibleInGlossy']
            if meshpShape:
                for Att in Attr:
...
#@author:韩虹
#data：2017-05-25
    def GDC_proxyIDSQL(self):
        shotInfo=sk_infoConfig.sk_infoConfig().checkShotInfo()
        proj=sk_infoConfig.sk_infoConfig().checkProjectNameSimple2Full(shotInfo[0])
        s = idmt.pipeline.service.Service()
