...
'''
def sk_lockObjs(typeNum):
    objs = mc.ls(sl = 1)
    doNotLock = ['drawOverride']
    for obj in objs:
        objChildren = mc.listRelatives(obj,ad =1,type ='transform',f = 1)
        if objChildren:
            objChildren.append(obj)
        else:
            objChildren = [obj]
        for objchild in objChildren:
            attrs = mc.listAttr(objchild)
            for attr in attrs:
                if attr not in doNotLock:
                    try:
                        mc.setAttr((objchild+'.'+attr),l=int(typeNum))
                    except:
                        pass
...
Data    :2013_04_07
'''
def checkNamespaceCleanEmpty(configType = 1):
...
Data    :2013_09_23
'''
def sk_deleteNamespace(namespace):
    ns = namespace
...
Data    :2013_06_23
'''
def sk_getNamespaceObjs(namespace):
...
Data    :2013_07_08
'''
def sk_animFrameVChange(pro = 'Calimero'):
...
# 临时改变模式
def sk_constraintsOnOffConfig(on = 0,fixType = 0 ):
    # 获取约束
    allConstraints = []
    if fixType == 0:
