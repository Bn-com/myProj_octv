...
class sk_smoothSet_Strawberry(object):
    ...
    def UI_setSmooth(self):
        ...
        UI_add_b = mc.button(l=(unicode('添加所选','utf8')), c = 'sk_smoothSet_Strawberry.sk_smoothSet_Strawberry().smoothSetAdd()' )
        UI_rm_b = mc.button(l=(unicode('移除所选','utf8')), c = 'sk_smoothSet_Strawberry.sk_smoothSet_Strawberry().smoothSetRemove(0)')
        UI_rmAll_b = mc.button(l=(unicode('移除该级下所有','utf8')), c = 'sk_smoothSet_Strawberry.sk_smoothSet_Strawberry().smoothSetRemove()')
        mc.setParent('..')
    ...
    def smoothSetAdd(self,projType = 1, smoothLevel = -1):
        ...
        if objs:
            ...
            if needObjs:
                ...
                for obj in objs:
                    shape = mc.listRelatives(obj,ni = 1, s=1, f=1)
                    checkSmooth = mc.getAttr(shape[0] + '.smoothLevel')
                    print checkSmooth
                    #if len(checkSmooth) != 1:
                    #    smoothLevel = [smoothLevel,smoothLevel,smoothLevel,smoothLevel]
                    mc.setAttr((shape[0] + '.smoothLevel'),smoothLevel)
                    mc.setAttr((shape[0] + '.renderSmoothLevel'),smoothLevel)
        # 说明
        print '\n'
        print u'-------------SmoothSet说明-------------'
        print u'[有效物体]：无_si_；无_nr_；无_proxy_；'
        print u'-------------------------------------'
