...
class sk_clProjectTools(object):
    ...
    def sk_sceneUICLTempTools(self):
        ...
        # 【anim】转【render】
        mc.rowLayout()
        mc.button(w = 150 , h = 30 ,bgc = [0,0.1,0.45],label = (unicode('【anim】转【render】', 'utf8')),c = 'from idmt.maya.commonCore.core_mayaCommon import sk_referenceConfig\nreload(sk_referenceConfig)\nsk_referenceConfig.sk_referenceConfig().checkLayout2Anim2Render(1)')
        mc.setParent("..")
    ...
    def sk_clAnim2Render(self):
        ...
        for i in range(len(refPaths)):
            ...
            # 替换参考
            #mc.file(refNodes[i], loadReference = newPath)
            try:
                mc.file(newPath, loadReference = refNodes[i])
            except:
                print u'===以下文件后台无法正常读取，请联系前期==='
                print newPath
                mc.error(u'===以下文件后台无法正常读取，请联系前期===')
    ...
    def sk_rebuildClean(self , serverClean = 0 , norenderConfig = 0):
        ...
        for layer in layers:
            ...
            for obj in objs:
                ...
                if mc.ls(obj + '.tt_visibility'):
                    if mc.getAttr((obj + '.tt_visibility'),settable = 1):
                        attr = '.tt_visibility'
                        attrV = 1
        ...
        if shotInfo[0] != "cl":
            if norenderConfig == 0 :
                errorDelete = sk_checkCommon.sk_checkTools().checkCleanDisplayLayers([],['norender'],0)
                if errorDelete:
                ...
                errorDelete = sk_checkCommon.sk_checkTools().checkCleanDisplayLayers([],['norender'],1)
        else :
            sk_animFileCheck.sk_animFileCheck().shotDisplayLayerCheck()
    ...
    def orient_billboard_to_camera(self,camera_name="CAMERA", set_key=False ):
        ...
        for controller_bil in list_c_bil:
            ...
            if animation_curve_list is None or set_key == True:
                ...
                list_mesh = mc.listRelatives(controller_bil, allDescendents=True, type="mesh",f = 1)
                x = 0
                y = 0
                z = 0
                compt = 1.0
                ...
                if mc.ls('cam_' + shotInfo[1] + '_' + shotInfo[2]):
                    posistion_cam = mc.getAttr('cam_' + shotInfo[1] + '_' + shotInfo[2] + ".translate")
                if mc.ls('CAMERA'):
    ...
    # Calimero 头发系统bake
    #-----------------------------------------#
    def calimeroHairBake(self):
        bkSet = mc.ls('*PRIS*:*dynCtrls*',type = 'objectSet')
        io = (mc.playbackOptions(q=1, minTime=1)-5, mc.playbackOptions(q=1, maxTime=1)+5)
        bkObjs = []
        if bkSet:
            for set in bkSet:
                ...
                bkObjs = bkObjs + objs
        if bkObjs:
            TTCtrl = mc.ls('*PRIS*:*c_ribbon_Ctrl',type = 'transform')
            if not TTCtrl:
                return 0
            TTCtrl = TTCtrl[0]
            # TT模式下运行
            TTMode = mc.getAttr(TTCtrl + '.switch')
            if TTMode != 1:
                return 0
            # 显示模式下运行
            if mc.ls(TTCtrl + 'Ribbon_Vis'):
                vis = mc.getAttr(TTCtrl + '.switch')
                if not vis:
                    return 0
            dnCtrl = mc.ls('*PRIS*:*c_DynamicTT_',type = 'transform')
            if dnCtrl :
                # 动力学开关开启
                mc.setAttr((dnCtrl[0] + '.dynamic'),1)
                mc.bakeResults(bkObjs,  t=io,
                    simulation=1,
                    sampleBy=1,
    ...
    def sk_clAnimFileReClean(self):   
        ...
        assetType = 'Animation'
        assetServerPath = serverPath + 'scenes/' + assetType + '/' 
        
        # 集数
        epNums = ['episode_139','episode_140','episode_143']
        ...
        for epNum in epNums:
            # 镜头文件夹
            shotFolders = mc.getFileList(folder = (assetServerPath + epNum + '/'))
            if not shotFolders:
                continue
            for folder in shotFolders:
                needFolders = []
                errorFolder = []
                
                if folder in ['episode_camera','episode_avi'] or 'scene_' not in folder:
                    continue
                
                #if folder not in ['scene_098']:
                #    continue
                
                # 镜头文件名
                fileNames = mc.getFileList(folder = (assetServerPath + epNum + '/' + folder + '/anim/' ))
                if not fileNames:
                    continue
                mayaFile = ''
                for fileName in fileNames:
                    if '.ma' in fileName:
                        mayaFile = fileName
                # 备份文件名
                historyMayaFile = assetServerPath + epNum + '/' + folder + '/anim/history/' + mayaFile
                print historyMayaFile
                # 覆盖文件名
                performMayaFile = assetServerPath + epNum + '/' + folder + '/anim/' + mayaFile
                # 处理文件
                if not os.path.exists(historyMayaFile):
                    continue
                print '-------'
                print historyMayaFile
                mc.file(historyMayaFile,open = 1, force = 1)

                layers = mc.listConnections('layerManager.displayLayerId')
                if layers:
                    for layer in layers:
                        if layer.lower() == 'norender':
                            needFolders = [(epNum + '_' + folder)]

                try:
                    print '------------'
                    print historyMayaFile
                    print needFolders
                    self.sk_rebuildClean(1,1)
                    print '-----Done-----'
                    # 保存
                    mc.file(rename = performMayaFile)
                    mc.file(save = 1,force = 1)
                except:
                    errorFolder = [(epNum + '_' + folder)]
                    print '------------'
                    print historyMayaFile
                    print '-----Error-----'
                ...
                if needFolders:
                    if checkNorender:
                        sk_infoConfig.sk_infoConfig().checkFileWrite((localPath +  'helloSunChao.txt'), needFolders, 1)
                    else:
                        sk_infoConfig.sk_infoConfig().checkFileWrite((localPath +  'helloSunChao.txt'), needFolders)
                    checkNorender = 1
                
                if errorFolder:
                    if checkError:
                        sk_infoConfig.sk_infoConfig().checkFileWrite((localPath +  'helloError.txt'), errorFolder, 1)
                    else:
                        sk_infoConfig.sk_infoConfig().checkFileWrite((localPath +  'helloError.txt'), errorFolder)
                    checkError = 1
                
    # 临时用
    def cl_renderSettingTempConfig(self):
