...
#modify between horse and company
#*** feed in path
#*** 

def yyEAGetProjectRepository():
...
def yyEAExportWrapper():
    ...
    if optionMode == "yyEAHi":
        yyAutoSelectWithACInHi(isExportJnt)
    ...
    if selObj != []:
        ...
        if isInfinity == 1:
            ...
            startTime = -5000000
            endTime = 5000000
        dict = yyEAExport(startTime, endTime)
...
def yyEAExport(tStart = 0, tEnd = 8):
...
def yyEAImportWrapper():
    ...
    if os.access(filePath, 0):
        yyEAImport(namespace, filePath, timeOffset, timeScale, isReplace)
    
def yyEATransferWrapper():
    selObj = mc.ls(sl = True)
    try:
    ...
    yyAutoSelectWithNS(isJnt)
    isReplace = mc.checkBox("yyEATransferIsReplace", q = True, v = True)
    
    contents = yyEAExport(tStart = -500000, tEnd = 510000)
    ...
    #yyAutoSelectWithNS(??)
    yyEAImport(ns2 + ":", tempDir + "/yyEACurveFile.yyea", 0, 1, isReplace)
    
    
    
def yyEAImport(namespace, filePath, timeOffset, timeScale, isReplace):
    #print namespace
    #print filePath
    #print timeOffset
    #print timeScale
...
def yyEAGUI():
    ...
    mc.menu( label='Edit')
    mc.menuItem( label='Refresh', c = "yyEARefresh()")
    mc.menuItem( divider=True )

    mc.menuItem( label='Exit', c = "yyEAExit()")

    mc.menu( label='About')
    mc.menuItem( label='Help', c = "yyEAHelp()")
    mc.menuItem( divider=True )

    mc.menuItem( label='About' , c = "yyEAAbout()")
    ...
    mc.setParent("..")
    ##--------------- play back range --------------##
    mc.button(l = "animCurve Export>>>>>", h = 30, c = "yyEAExportWrapper()")
    mc.separator(h = 10)
    ##--------------- import options --------------##
    ...
    ##--------------- import  --------------##
    mc.button(l = ">>>>>animCurve Import", h = 30, c = "yyEAImportWrapper()")
    mc.separator(h = 10)
    ...
    mc.checkBox("yyEATransferJnt", l = "Enclude Joint Animation", v = 0, align  = "left", onc = "yyEASureTransferJoint()")
    mc.checkBox("yyEATransferIsReplace", l = "Is Replace?", v = 1, align = "left")
    mc.button(l = ">>>animCurve Transfer>>>", h = 30, c = "yyEATransferWrapper()")
    ...
    try:
        ...
        mc.separator(h = 10)
        repositoryPath = yyEAGetProjectRepository()
        mc.text(l = "Creature Name:", align = "left")
        try:
            creatureNodeList = os.listdir(repositoryPath)
        except:
            creatureNodeList = []
        mc.optionMenu("yyEA_om01", cc = "yyEAChangeCreature()",w =120)
        try:
        ...
        creaturePath = yyEAGetOptionItemFP01()
        try:
        ...
        mc.optionMenu("yyEA_om02", cc = "yyEAChangeGenre()")
        try:
        ...
        genrePath = yyEAGetOptionItemFP02()
        try:
    ...
    mc.setParent("..")
    mc.button(l = "Easy Anim Feed In", h = 30, c = "yyEAFeedInWrapper()")
    mc.button(l = "Watch Example Video", h = 30, c = "yyEAWatchExampleVideoClip()")
    ...
    mc.button(l = "Auto Select Objects with Anim (Whole Scene)", h = 30, c = "yyAutoSelectWithAC(1)")
    mc.button(l = "Auto Select Objects with Anim (in Hierarchy)", h = 30, c = "yyAutoSelectWithACInHi(1)")
    mc.button(l = "Auto Select Objects with Anim (by Namespace)", h = 30, c = "yyAutoSelectWithNS(1)")
    mc.separator(h = 10)
    ##--------------- animation cleanser --------------##
    ...
    mc.setParent("..")
    mc.button(l = "Delete Animation", h = 30, c = "yyEADeleteAnimCurveWrapper()")
    mc.separator(h = 10)
    
    ##------------- non-animation channels ------------##
    mc.text(l = "Non-animation Channel Channels:", w = 240, align = "left", fn = "smallBoldLabelFont")
    mc.button(l = "Check Non-animation Channels", h = 30, c = "yyNonAnimChannelChecker()")
...
def yyEAChangeCreature():
    selection = yyEAGetOptionItemFP01()
    #print selection
    for i in range(100):
        stri = str(i)
        try:
...
def yyEAChangeGenre():
    selection = yyEAGetOptionItemFP02()
    #print selection
    for i in range(100):
        stri = str(i)
        try:
...
def yyEAGetFeedInFilePath():
    repository = yyEAGetProjectRepository()
    creature = yyEAGetOptionItemFP01()
    genre = yyEAGetOptionItemFP02()
    curve = yyEAGetOptionItemFP03()
    feedinPath = repository + "/" + creature + "/" + genre + "/" + curve + ".yyea"
    
    return feedinPath

def yyEAWatchExampleVideoClip():
    repository = yyEAGetProjectRepository()
    creature = yyEAGetOptionItemFP01()
    genre = yyEAGetOptionItemFP02()
    curve = yyEAGetOptionItemFP03()
...
def yyEAFeedInWrapper():
    
    feedinFile = yyEAGetFeedInFilePath()
    ...
    if os.access(filePath, 0):
        ...
        for i in range(times):
            ...
            if jump == 0:
                jump = yyEAFeedIn(namespace, filePath, timeOffset, timeScale, isReplace, isStop, stopTime)
            else:
                currentKeyTime = timeOffset + (jump + jumpJump) * i
                yyEAFeedIn(namespace, filePath, currentKeyTime, timeScale, isReplace, isStop, stopTime)
...
def yyAutoSelectWithACInHi(withJnt = 0):
    ...
    if access == 1:
        ...
        for i in range(len(selObj)):
            for j in range(len(animObj)):
                if isThatInDagHi(selObj[i], animObj[j]) == 1:
...
def yyAutoSelectWithNS(withJnt = 0):
    try:
...
def yyAutoSelectWithAC(withJnt = 0):
...
def yyEADeleteAnimCurveWrapper():
    ...
    if stat == "Yes":
        ...
        if option == "yyEADASl":
            yyDeleteSelectAnimCurve()
        else:
            yyDeleteSelectAllAnimCurve()


def yyDeleteSelectAnimCurve():
    deleted = 0
    try:
        selObj = mc.ls(sl = True)
        for i in range(len(selObj)):
...
def yyDeleteSelectAllAnimCurve():
...
def isThisHaveConstraintOutConns():
