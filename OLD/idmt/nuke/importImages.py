...
def importDO(proj=u'DiveollyDive4', proj_abb=u'do4'):
    ep = ""
    sc = ""
    path = nuke.root().knob('name').value()
    buff1 = os.path.split(path)
...
def commonimportImages():
    ...
    if proj.projectId!= 0:
        ...
        buff2 = re.compile('_').split(buff1[1])
        if buff2[0] in ['csl']:
            filepathf=filepathf.replace('//file-cluster/GDC/','L:/')
        ep = buff2[1]
        seq = buff2[2]
        sc = buff2[3]
        parity= getParity(ep)
        if re.search(r'^\d+$', sc) != None:
            folder=filepathf+"/"+parity + "/ep_" + ep + "/seq_" + seq + "/sc_" + sc + "/"
        else:
            folder=filepathf+"/"+parity + "/ep_" + ep + "/sc_" + seq + "/"
        importImages(folder)
    else:
        nuke.message('can not catch the shot number form filename')