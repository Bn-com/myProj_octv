...
import sip
import mi_get_multicam_callui_QT;reload(mi_get_multicam_callui_QT)

from PyQt4.QtGui import *
from PyQt4.QtCore import *

import mi_get_multiCam
def getMayaWindow():
    ptr = apiUI.MQtUtil.mainWindow()
    return sip.wrapinstance(long(ptr), QObject)


class Maya_Main(mi_get_multicam_callui_QT.QT_MainUI):
    def __init__(self, parent=getMayaWindow()):
        super(Maya_Main, self).__init__(parent)
        self.ui = mi_get_multiCam.Ui_MainWindow()
        self.ui.setupUi(self)
        self.ui.QPB_OK.clicked.connect(self.xxx)

    def xxx(self):
        user_cam_category = self.get_cam_category()
        user_op = self.get_operate()
        op_dic = {u'add':u'add',u'rep':u'replace'}
        print u'You want %s camera: %s' %(op_dic[user_op],u';'.join(user_cam_category))
        #print u'user_operation: %s' % (user_op)
