...
#        fileType=['cha','set','cs','chrlight','setlight']
        ...
        if refconinfo[1]==1: 
            ...
            setG=self.csl_GroupSelect()[1]
            if len(setG)>2 :
                for i in (1,(len(setG)-1)):
                    mc.select([setG[i]]+self.csl_GroupSelect()[3])
                    setName=SetbaseName.replace('_set_','_set0'+str(i)+'_')
                    mc.file((tempath+setName),options='v=0',f=1,type=fileTypeAll,preserveReferences=1,es=1)
    ...
    #        sk_sceneTools.sk_sceneTools().sk_sceneNoRefNamespaceClean()
            ...
            lightgroups=''
            if chrLgtGrp!='':
                lightgroups=chrLgtGrp
            else:
                mc.error(u"没有角色灯光组或角色灯光组命名不正确，请与环节负责人联系")
            if lightgroups:
                mc.select(lightgroups)
                mc.file((tempath+chrlightName),options='v=0',f=1,type=fileTypeAll,es=1)               
                print (u'===============!!!End 【%s】!!!===============' % (u'%s_已导出角色灯光文件' % chrlightName))                
            '''    
    #        sk_sceneTools.sk_sceneTools().sk_sceneNoRefNamespaceClean()
...
# 场景文件处理：
        ...
        if refconinfo[1]==1:
            ...
            if len(setG)<=2: 
                ...
                lightgroup=self.csl_lightInfoList(Type='chr',lightType='group')
                if lightgroup:
                    for chrlight in lightgroup:
                        mc.setAttr(chrlight+'.v',0)
        ...
        #   动画帧数，渲染尺寸设置
                ...
                for i in (1,(len(setG)-1)):
                    setAOVNameN=setAOVName.replace('l6setAOV_','l6set0'+str(i)+'AOV_')
                    setAOVlayerNameN=setAOVlayerName.replace('set','set0'+str(i))
                    setName=SetbaseName.replace('_set_','_set0'+str(i)+'_')
                    mc.file((tempath+setName),options='v=0',type=fileTypeAll,f=1,o=1) 
                    GDC_ArnoldCommon.GDCRenderArnold().ArnoldRendererSettings('03')
                    mc.file(rename=(tempath+setAOVNameN))
                    mc.file(save=1,type = fileTypeAll,f = 1)
                    lightgroup=self.csl_lightInfoList(Type='chr',lightType='group')
                    if lightgroup:
                        for chrlight in lightgroup:
                    ...
                    meshs=self.csl_GroupSelect()[1][1]
                    mc.createRenderLayer(meshs,name=setAOVlayerNameN, noRecurse=1, makeCurrent=1)
                    mc.setAttr("defaultRenderLayer.renderable", 0)
                    GDC_ArnoldCommon.GDCRenderArnold().ArnoldAOVCreat(AOVtype='AO')
                    GDC_ArnoldCommon.GDCRenderArnold().ArnoldAOVCreat(AOVtype='Normal')
                    GDC_ArnoldCommon.GDCRenderArnold().ArnoldAOVCreat(AOVtype='Zdp')
            ...
            #   动画帧数，渲染尺寸设置
                    ...
                    if  server==1  :
                        ...
                        mel.eval('idmtProject -checkin -description \" ' + description + '\"')  
                    print u'\n'
                    print (u'===============!!!End 【%s】!!!===============' % (u'%s_已完成场景AOV文件' % setAOVNameN))                                                   
...
#选择相关mesh
    def csl_meshInfo(self,meshtype='s'):
        ...
        for i in range(len(meshs)):
            for j in range(len(namespaces)) :
                if namespaces[j] in meshs[i] and namespaces[j].split('_')[1][0].lower()==meshtype.lower() and ':MODEL' in meshs[i]:
                    meshP=mc.listRelatives(meshs[i],p=1,f=1)
                    if meshP:
                        meshList.append(meshP[0])
        return meshList 
