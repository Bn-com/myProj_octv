...
messageRadio = mc.radioButtonGrp('messagRadioBG', label=u'查看Deadline服务器信息', labelArray2=[u'是', u'否'], sl=2, cl3=['left', 'left', 'left'],
                                 cw3=[150, 100, 100], numberOfRadioButtons=2, p=oneC)
mc.radioButtonGrp('AutoSubmit', label=u'是否全自动提交Deadline', labelArray2=[u'是', u'否'], sl=2, cl3=['left', 'left', 'left'], cw3=[150, 100, 100],
                  numberOfRadioButtons=2, p=oneC)
mc.radioButtonGrp('ArnoldProxy', label=u'aiStandIn', labelArray2=[u'是', u'否'], sl=1, cl3=['left', 'left', 'left'], cw3=[150, 100, 100],
                  numberOfRadioButtons=2, p=oneC)
...
# mc.radioButton('one', l=u'#1 (211池)', onc=selectServer, p='row2')
mc.radioButton('one', l=u'#1 (206池)', onc=selectServer, p='row2')
mc.radioButton('two', l=u'#2 (222池)', onc=selectServer, p='row2')
mc.radioButton('three', l=u'#3 (223池)', onc=selectServer, p='row2')
mc.radioButton('four', l=u'#4 (224池)', onc=selectServer, p='row2')
# mc.radioButton('five', l=u'#5 (163池)', onc=selectServer, p='row2')
mc.radioButton('six', l=u'#4 (221池)', onc=selectServer, p='row2')
mc.radioButton('seven', l=u'#6 (100池)', onc=selectServer, backgroundColor=(0.3, 0.1, 0), p='row2')
mc.radioButton('eight', l=u'#7 (102池)', onc=selectServer, backgroundColor=(0.3, 0.1, 0), p='row2')
...
three_one = mc.button('slButton', l='Submint', c=copyFile, p=form, en=False, bgc=[0, 0.7, 0], w=100)
three_two = mc.button('cancelButton', l='Cancel', c=close, p=form, w=100)

mc.formLayout(form, edit=True,
              attachForm=[(one, 'left', 5), (two_one, 'right', 5), (two_two, 'right', 5), (one, 'top', 84), (two_one, 'top', 84),
                          (three_one, 'left', 5), (three_one, 'right', 5), (three_two, 'left', 5), (three_two, 'right', 5),
                          (three_two, 'bottom', 5)],
              attachControl=[(two_one, 'left', 1, one), (two_two, 'left', 1, one), (two_two, 'top', 1, two_one),
                             (one, 'bottom', 1, three_one), (two_two, 'bottom', 1, three_one), (three_one, 'bottom', 1, three_two)],
              attachNone=[(three_one, 'top')],
              )

windowName = win
messageRadio = messageRadio
assetRadio = selectRadio
outputFiled = two_two
okButton = three_one
...
selectOption = mc.radioCollection(assetRadio, q=True, sl=True)
if selectOption == 'one':
    deadlineSereveIp = r'//192.168.80.206'
    SERVE_PATH = r'\\192.168.80.206'
if selectOption == 'two':
    deadlineSereveIp = r'//192.168.80.222'
    SERVE_PATH = r'\\192.168.80.222'
elif selectOption == 'three':
    deadlineSereveIp = r'//192.168.80.223'
    SERVE_PATH = r'\\192.168.80.223'
elif selectOption == 'four':
    deadlineSereveIp = r'//192.168.80.224'
    SERVE_PATH = r'\\192.168.80.224'
# elif selectOption == 'five':
#    deadlineSereveIp = r'//192.168.90.163'
#    SERVE_PATH = r'\\192.168.90.163'
elif selectOption == 'six':
    deadlineSereveIp = r'//192.168.80.221'
    SERVE_PATH = r'\\192.168.80.221'
elif selectOption == 'seven':
    deadlineSereveIp = r'//192.168.90.100'
    SERVE_PATH = r'\\192.168.80.205'
elif selectOption == 'eight':
    deadlineSereveIp = r'//192.168.80.102'
    SERVE_PATH = r'\\192.168.80.225'

import pymel.core as pm

cur_proj = pm.workspace.getPath()

SERVE_PATH = cur_proj.strip()
...
if addr == '':
    ...
    try:
        str = os.popen(r'"%s/deadlinecommand.exe" -ChangeRepository %s/DeadlineRepository' % (addr, deadlineSereveIp)).read()
    except:
        sys.stderr.write(u'设定Deadline服务器时出错,请检查网络连接或权限.')
        mc.button(okButton, e=True, en=False)
    else:
        # 是否加载服务器信息
        ...
        if mValue == 1:
            try:
                ...
                mc.scrollField(outputFiled, e=True, tx=str)
        if deadlineSereveIp == r'//192.168.90.163':
...
if os.path.isdir(serFilePath):
    if mc.file(q=True, amf=True):
        if copyType == 2 or copyType == 4:
            saveFlag = mc.confirmDialog(title=u'温馨提示', message=u'文件已被修改过，请问是否先保存再继续提交？', button=['Yes', 'SaveAs', 'No'],
...
myUseRender = checkRender()
OtherLayers = myUseRender[0]
MrLayers = myUseRender[1]
ArLayers = myUseRender[2]
AllUsedRender = myUseRender[4]
print AllUsedRender
if len(set(AllUsedRender)) > 1:
    mc.confirmDialog(title=u'警告', message=u'提交的文件仅能使用一种渲染器！\n----------请修改文件---------', button=['OK'], defaultButton='Yes',
                     dismissString='No')
    return False
...
if allTexFiles:
    for texFile in allTexFiles:
        try:
            ...
            if texFileName:
                texFileName = myChangeNetPath(texFileName)
...
if allCacheFiles:
    for mycacheFile in allCacheFiles:
        try:
            ...
            if cachePath:
                cachePath = myChangeNetPath(cachePath)
                ...
                # if LocaPaths:
                #     LocaPaths = myChangeNetPath(LocaPaths)
...
if allYetiCacheFiles:
    for myYetiCacheFile in allYetiCacheFiles:
        ...
        try:
            ...
            if YetiTexPath:
                YetiTexPath = myChangeNetPath(YetiTexPath)
...
if mc.pluginInfo('vrayformaya.mll', query=True, loaded=True):
    ...
    noVrCausticsFiles = myCheck_VrSetFilesModel(VrCaustics_typeOn, VrCaustics_typeModel, VrCaustics_typeFuleAttr, VrCaustics_numModel,
                                                VrCausticsSets)
    # 统计错误
...
def copyFile( *args):
    NonePlane()
    if not mc.radioCollection(assetRadio, q=True, sl=True):
...
def __init__( parent=None):
    super(CopyProject, ).__init__(parent)
    fileSName = mc.file(q=True, sn=True, shn=True)
    CopyLocalPath = getCopyPath()
    CpauLocalPath = getCPauPath()
    worker1 = CopyJobThread()
    worker2 = CopyJobThread()
    percent.connect(EditProgressWindow)
    stateMsg = ''
    # copyType分三种模式
    # 1 单纯拷贝模式
    # 2 拷贝提交模式
...
def EditProgressWindow( i):
    mc.progressWindow(edit=True, progress=i)

# 双线程拷贝
def CopyDataJob( myDict, OpenDoubleCopyFlag):
    i = 0
    for key in myDict.keys():
        if mc.progressWindow(q=True, isCancelled=True):
            while True:
                if worker1.wait() and worker2.wait():
                    break
            return False
        mySourceFile = key
        ...
        if OpenDoubleCopyFlag:
            if i == 0:
                worker1.ready(CpauLocalPath, CopyLocalPath, mySourceFile, myDestFile)
            elif i == 1:
                worker2.ready(CpauLocalPath, CopyLocalPath, mySourceFile, myDestFile)
            else:
                while True:
                    if worker1.isFinished():
                        worker1.ready(CpauLocalPath, CopyLocalPath, mySourceFile, myDestFile)
                        break
                    elif worker2.isFinished():
                        worker2.ready(CpauLocalPath, CopyLocalPath, mySourceFile, myDestFile)
                        break
        else:
            worker1.ready(CpauLocalPath, CopyLocalPath, mySourceFile, myDestFile)
...
def getCopyPath():
    ...
    if os.path.isdir(FCOPY_LPATH_DN):
        numSpath = fileCountIn(FCOPY_SPATH_DN)
        numLpath = fileCountIn(FCOPY_LPATH_DN)
...
def getCPauPath():
    ...
    if not os.path.isfile(CPAU_LPATH_FP):
        cmd = r'%s /cmd=diff /force_close /error_stop=FALSE /no_confirm_del /force_start=FALSE "%s" /to="%s"' % (
        CopyLocalPath, CPAY_SPATH, CPAU_LPATH_DN)
        p = subprocess.Popen(cmd, stdin=subprocess.PIPE, stdout=subprocess.PIPE, shell=True)
        while True:
            if not p.poll() is None:
                del p
...
def myCreateFolder( address):
    try:
        ...
        cmd = r'%s -u %s -p %s -hide -wait -c -nowarn -ex "md %s"' % (CpauLocalPath, REMOTE_USER, REMOTE_PWD, address)
        p = subprocess.Popen(cmd, stdin=subprocess.PIPE, stdout=subprocess.PIPE, shell=True)
        while True:
            if not p.poll() is None:
                del p
...
def myCreateDeepFolder( address):
    try:
        ...
        print address
        cmd = r'%s -u %s -p %s -hide -wait -c -nowarn -ex "md %s"' % (CpauLocalPath, r'octvision\rd', r'rd1234', address)
        print cmd
        p = subprocess.Popen(cmd, stdin=subprocess.PIPE, stdout=subprocess.PIPE, shell=True)
        while True:
            if not p.poll() is None:
...
def myJudeArnoldTxFile( texFileName):
    PathSplitT = os.path.splitext(texFileName)
    if len(PathSplitT) > 1:
        LowerPathType = PathSplitT[1].lower()
    ...
    if allfiles:
        for eachfile in allfiles:
            ...
            try:
                ...
                texFirstFileName = myChangeNetPath(texFirstFileName)
                # 判断贴图是否开启了序列帧模式
                # 序列标识：
        ...
        if copyData:
            ...
            # 拷贝文件
            if not CopyDataJob(copyData, True):
                return False
        # 设置路径
        ...
        #         #拷贝文件
        #         if not CopyDataJob(ArnoldProxyCopyData, True):
        #             return False
    return True
...
def myCopy_Data():
    ...
    if allfiles:
        for eachfile in allfiles:
            try:
                ...
                if indexType:
                    ...
                    copyFinalcachePath = os.path.join(serveProject, serLastcachePath)
                else:
                    copyFinalcachePath = os.path.join(serveProject, types, other_file)
                # try:
        ...
        if copyData:
            ...
            # 拷贝文件
            if not CopyDataJob(copyData, False):
                return False
        # 设置路径
        for key in setData.keys():
            try:
...
def myCopy_YetiCache():
    ...
    serTexFileName = os.path.join(serveProject, type_TexFile)
    serFileName = os.path.normpath(serFileName)
    allYetiCacheFiles = mc.ls(type='pgYetiMaya')
    setTexData = {}
    # 进行
...
def myCopy_AbcData():
    tmpCopyFlag = True
    type_file = 'alembic'
    serFileName = os.path.join(serveProject, 'cache\\' + type_file)
    ...
    if allfiles:
        for eachfile in allfiles:
            try:
                ...
                abcCachePath = myChangeNetPath(abcCachePath)
...
def myCopy_Shavedata():
    ...
    if allShaveShapes and myshaveGlobals:
        ...
        if shavePath:
            shavePath = myChangeNetPath(shavePath)
...
# 拷贝粒子缓存
def myCopy_Particles():
    tmpCopyFlag = True
    type_file = 'particles'
    type_cache = 'cache'
    serFileName = os.path.join(serveProject, type_cache, type_file)
    AllIsParFlag = False
    ...
    if mydynGlobals and AllIsParFlag:
        fileShortName = os.path.splitext(fileSName)[0]
...
def myCopy_Proxy_OImagesModel( myType, mtAttr):
    tmpCopyFlag = True
        ...
        if allTypeShapes:
            ...
            for shapeEach in allTypeShapes:
                try:
                    ...
                    if myFilepath:
                        myFilepath = myChangeNetPath(myFilepath)
                        # 获取文件名
                        FileOkFlag = False
            ...
            for shapeEach in allTypeShapes:
                try:
                    ...
                    if myFilepath:
                        ...
                        if copyType == 1 and not FileOkFlag:
                            # myFilepath = myFilepath.replace('/','\\')
                            # texFileNameS = myFilepath.split('\\')
                            # indexType = texFileNameS.index(type_file)
                            # myFileBaseName = '\\'.join(texFileNameS[indexType+1::])
...
# 拷贝Vr光子贴图、焦散贴图
def myCopy_VrSetFilesModel( typeOn, typeModel, typeFuleAttr, numModel):
    tmpCopyFlag = True
    copyData = {}
    typeName = ''
    type_file = 'sourceimages'
    ...
    try:
        if mc.getAttr("vraySettings.%s" % typeOn):
            if mc.getAttr('vraySettings.%s' % typeModel) == numModel:
                localFile = mc.getAttr('vraySettings.%s' % typeFuleAttr)
                if localFile:
                    localFile = myChangeNetPath(localFile)
...
def myCopy_Mr():
    ...
    mrIbAttr = 'texture'
    if not myCopy_Proxy_OImagesModel(mrIbType, mrIbAttr):
        return False
    # 拷贝mentalrayTexture节点的贴图
    mrTxType = 'mentalrayTexture'
    mrTxAttr = 'fileTextureName'
    if not myCopy_Proxy_OImagesModel(mrTxType, mrTxAttr):
        return False
    return True
...
def myCopy_Vr():
    ...
    VrType = 'VRayMesh'
    VrAttr = 'fileName'
    if not myCopy_Proxy_OImagesModel(VrType, VrAttr):
        return False
    # 检查Vray的VRayLightIESShape灯光贴图
    VrIesLType = 'VRayLightIESShape'
    VrIesLAttr = 'iesFile'
    if not myCopy_Proxy_OImagesModel(VrIesLType, VrIesLAttr):
        return False
    # Irradiance map光子贴图
    IrrMap_typeOn = 'giOn'
    ...
    IrrMap_numModel = 2
    if not myCopy_VrSetFilesModel(IrrMap_typeOn, IrrMap_typeModel, IrrMap_typeFuleAttr, IrrMap_numModel):
        return False
    # Light cache map光子贴图
    LightC_typeOn = 'giOn'
    ...
    LightC_numModel = 2
    if not myCopy_VrSetFilesModel(LightC_typeOn, LightC_typeModel, LightC_typeFuleAttr, LightC_numModel):
        return False
    # Caustics的焦散贴图
    ...
    Caustics_numModel = 1
    if not myCopy_VrSetFilesModel(Caustics_typeOn, Caustics_typeModel, Caustics_typeFuleAttr, Caustics_numModel):
        return False
    return True

def myCopy_Ar():
    # 拷贝Arnold代理与代理贴图
    if not myCopy_Ar_Proxy():
        return False
    # ArType = 'aiStandIn'
    # ArAttr = 'dso'
    # if not myCopy_Proxy_OImagesModel(ArType, ArAttr):
    #     return False
    ...
    ArIesLType = 'aiPhotometricLight'
    ArIesLAttr = 'aiFilename'
    if not myCopy_Proxy_OImagesModel(ArIesLType, ArIesLAttr):
        return False
    return True

# 拷贝mrIb贴图、mrTex节点的贴图路径、摄像机投影贴图、检查Vray的VRayLightIESShape灯光贴图
def myCopy_OtherImages():
    # 检查摄像机投影贴图
    camImType = 'imagePlane'
    camImAttr = 'imageName'
    if not myCopy_Proxy_OImagesModel(camImType, camImAttr):
        return False
    return True

# 拷贝Realflow缓存的模块
def myCopy_rfCacheModel( myType, mtAttr):
    tmpCopyFlag = True
    try:
        ...
        serFileName = os.path.join(serveProject, type_file, type_data)
        if allRfNodes:
            for RfNode in allRfNodes:
                myFileFullpath = mc.getAttr('%s.%s' % (RfNode, mtAttr))
                if myFileFullpath:
                    myFileFullpath = myChangeNetPath(myFileFullpath)
...
def myCopy_rfCache():
    ...
    rfParType = 'RealflowEmitter'
    rfParAttr = 'Paths[0]'
    if not myCopy_rfCacheModel(rfParType, rfParAttr):
        return False
    # 拷贝meshs缓存
    rfMeshType = 'RealflowMesh'
    rfMeshAttr = 'Path'
    if not myCopy_rfCacheModel(rfMeshType, rfMeshAttr):
        return False
    return True
...
def mySaveFile():
    ...
    if driveFlag:
        ...
        locaoFileName = os.path.join(localTempPath, fileSName)
        myTyprName = 'mayaBinary'
        if fileSName.lower().find('mb') >= 0:
            myTyprName = 'mayaBinary'
        else:
            myTyprName = 'mayaAscii'
        serFileName = os.path.join(serveProject, type_file)
        fileserName = os.path.join(serFileName, fileSName)
        ...
        if VrayFlag and VrayAnimationFlag:
            try:
                ...
                ChangeFlag = True
                time.sleep(1)
        if not worker1.myLocalFlag or copyType == 5:
            ...
            CopyDataJob(copyData, True)
            # myProjectAddress = serveProject.replace('\\', '/')
...
def writeInfo():
        ...
        if pattern.match(eachLayer):
            if not eachLayer == 'defaultRenderLayer':
                try:
                    ...
                    stateMsg += u'注意,%s节点无法删除.\n'
                    om.MGlobal.displayWarning(u'注意,%s节点无法删除.\n')
    del pattern

    imgFormat = {23: 'avi', 11: 'cin', 35: 'dds', 9: 'eps', 0: 'gif', 8: 'jpg', 7: 'iff', 10: 'iff', 31: 'psd', 36: 'psd', 32: 'png', 12: 'yuv',
                 2: 'rla', 5: 'sgi', 13: 'sgi', 1: 'pic', 19: 'tga', 3: 'tif', 4: 'tif', 20: 'bmp', \
    ...
    key = ['Version', 'Build', 'Name', 'StartFrames', 'EndFrames', 'FrameStep', 'OutputDirectories0', 'SceneFile', 'Renderer', 'RenderLayer',
           'ImageWidth',
    ...
    hostFile = ['%s\\scenes\\%s' % (serveProject, fileNameWithExt)]
    outputDir = [serveImages]
    projPath = [serveProject]
...
def main( *args):
    ...
    if copyType == 2 or copyType == 3:
        serveImages = myCreateImagesFolder()
    # 设置deep层输出的路径
    elif copyType == 4:
        serveImages = myCreateDeepImagesFolder()

    # 当模式为1和2时工程目录，3时为读取当前工程目录
    if copyType == 1 or copyType == 2 or copyType == 4 or copyType == 5:
        serveProject = myCreateScenes()
    elif copyType == 3:
        serveProject = mc.workspace(q=True, rd=True)
        myFileFullName = mc.file(q=True, sn=True)
    if serveProject.find('z:') >= 0:
        serveProject = serveProject.replace('z:', OCT_DRIVE)
    elif serveProject.find('Z:') >= 0:
        serveProject = serveProject.replace('Z:', OCT_DRIVE)
    ...
    try:
        ...
        worker1.myLocalFlag = False
        worker2.myLocalFlag = False
    else:
        worker1.myLocalFlag = True
        worker2.myLocalFlag = True
    ...
    if (copyType == 2 or copyType == 4) and ArnoldFlag:
        if mc.objExists("defaultArnoldRenderOptions"):
            ...
            if myArnoldLocalPath:
                if myArnoldLocalPath[-1] != ';':
                    mc.setAttr("defaultArnoldRenderOptions.texture_searchpath", myArnoldLocalPath + ';' + serveProject + '\\sourceimages;',
                               type="string")
                else:
                    mc.setAttr("defaultArnoldRenderOptions.texture_searchpath", myArnoldLocalPath + serveProject + '\\sourceimages;',
                               type="string")
            else:
                mc.setAttr("defaultArnoldRenderOptions.texture_searchpath", serveProject + '\\sourceimages;', type="string")
            # if myArnoldProxyPath:
            #     if myArnoldProxyPath[-1] != ';':
            #          mc.setAttr("defaultArnoldRenderOptions.procedural_searchpath", myArnoldProxyPath+';'+serveProject+'sourceimages;', type="string")
            #     else:
            #         mc.setAttr("defaultArnoldRenderOptions.procedural_searchpath", myArnoldProxyPath+serveProject+'sourceimages;', type="string")
            # else:
            #     mc.setAttr("defaultArnoldRenderOptions.procedural_searchpath", serveProject+'sourceimages;', type="string")
    # 运行主程序
    if serveProject:
                            ...
                            if myCopy_Particles():
                                ...
                                if myCopy_rfCache():
                                    ...
                                    if myCopy_YetiCache():
                                        ...
                                        if myCopy_OtherImages():
                                            if mrFlag:
                                                j += 1
                                                mc.progressWindow(edit=True, title=u'共%s步, 第 %s 步,正在上传Mr渲染器的相关文件' % (k, j))
                                                if not myCopy_Mr():
                                                    mc.confirmDialog(title=u'警告：', message=u'上传文件被中断！', button=[u'确认'], icn='critical',
                                            ...
                                            if arFlag:
                                                ...
                                                if not myCopy_Ar():
                                                    ...
                                                    return False
                                            j += 1
                                            mc.progressWindow(edit=True, progress=1, min=0, max=1, status=u"正在保存文件!",
                                                              title=u'共%s步, 第 %s 步,正在保存文件' % (k, j))
                                            delDefaultRenderLayer()
                                            Cancelsimulation()
                                            myFileFullName = mySaveFile()
                                            if myFileFullName:
        ...
        if copyType == 2 or copyType == 3 or copyType == 4:
            if copyType == 3:
                delDefaultRenderLayer()
            changeMrLayer()
            myfileBaseName = os.path.splitext(fileSName)[0]
            mm.eval(r'global string $myFileName = "%s"' % myfileBaseName)
            mm.eval(r'global string $myDeadlineImagesPath = "%s"' % serveImages.replace('\\', '/'))
            mm.eval(r'global string $myDeadlineImagesPath = "%s"' % serveImages.replace('\\', '/'))
            mm.eval(r'global string $myDeadlineSceneFile = "%s"' % myFileFullName.replace('\\', '/'))
            mm.eval(r'global string $myDeadlineProjectPath= "%s"' % serveProject.replace('\\', '/'))
