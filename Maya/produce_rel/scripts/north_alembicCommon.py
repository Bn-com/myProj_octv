...
class GDCAlembicCommon(object):
    ...
    #------------------------------#
    def GDC_FinaloutABC(self,server = 1,cachePre = -10,shave = 1,abcBy_ns = 2,justCache = 0,bkMode = 1,assetFrameDict = {},printMode = 1,shapeMode = 1,eulerFilter = 1):
        ...
        # 清理服务器端旧的SET和OTC文件
        self.sk_sceneGRPDelete('SET',server)
        self.sk_sceneGRPDelete('OTC',server)
        #---------------------------#
        # Setup 009 文件内部大组归类
        #---------------------------#
        ...
        #---------------------------#
        unDisplayLayerObjs = self.sk_FL_RefHideObjsRecord(server=server,shotType=shotType)
        # 记录cacheDict
        cacheDict = {}
        ...
        #---------------------------#
        # 输出显示隐藏动画信息
        self.checkCacheVStateExport([],shotType,server = server )
        print u'\n'
        ...
        if cacheObjs:
            ...
            if abcBy_ns:
                self.checkAbcCacheExport(cacheObjs,server=server,exportType='mesh',cachePre = cachePre,step=1,assetFrameDict = assetFrameDict,abcBy_ns = abcBy_ns,eulerFilter = eulerFilter)
            else:
        ...
        if shave and curveObjs:
            if  abcBy_ns:
                self.checkAbcCacheExport(curveObjs,server=server,exportType='curve',cachePre = cachePre,step=1,assetFrameDict = assetFrameDict,abcBy_ns = abcBy_ns,eulerFilter = eulerFilter)
            else:
        ...
        self.sk_sceneSETRefShaderReset(shotInfos,serverModify = server , shotType =shotType)
        ...
        # 导回SET_GRP和OTC_GRP
        self.sk_sceneGRPImport('SET',server,shotType)
        self.sk_sceneGRPImport('OTC',server,shotType)
        print u'\n'
        ...
        if cacheObjs:
            ...
            print '\n' + time.strftime('%Y-%m-%d %H:%M:%S') + '\n'
            if abcBy_ns:
                self.assetAbcRebuild(checkType = 'mesh',shotName = shotID,server = server,abcBy_ns = abcBy_ns,shapeMode = shapeMode)
            else:
        ...
        if shave==1:
            ...
            if curveObjs:
                ...
                if abcBy_ns:
                    self.assetAbcRebuild(checkType = 'curve',shotName = shotID,server = server,abcBy_ns = abcBy_ns)
                else:
        ...
        self.checkCacheVStateImport(shotType,server)
    ...
    # #选中版
    #-----------------------------#
    def checkAbcCacheExport(self,objsCache, server = 1,exportType = 'mesh' ,cachePre = 0 ,step = 1,assetFrameDict = {} ,abcBy_ns = 1,eulerFilter = 1):
        if not objsCache:
            return
        nsObjs = {}
        ...
        localPath  = sk_infoConfig.sk_infoConfig().checkShotDataInfoPath(server = 0,infoMode = 2.5)
        serverPath = sk_infoConfig.sk_infoConfig().checkShotDataInfoPath(server = 1,infoMode = 2.5)
        fileList = []
    ...
    # nsList指定输出
    def checkAbcCacheExportByNs(self,needTypes = ['c','p'],checkNsList = [],server=1,exportType = 'mesh',st = 0,et = 0,step = 1,bkMode = 1,eulerFilter = 1):
        # 处理大组
        sk_sceneTools.sk_sceneTools().sk_sceneReorganize(0)
        # 约束
        if bkMode:
    ...
    # 手动选物体，转nslist输出
    def checkAbcCacheExportByNsSel(self,needTypes = ['c','p'],server=1,exportType = 'mesh',st = 0,et = 0,step = 1,bkMode = 1):
        ...
        if checkNsList:
            self.checkAbcCacheExportByNs(needTypes = needTypes,checkNsList = checkNsList,server=server,exportType = exportType,st = st,et = et,step = step,bkMode = bkMode)
    ...
    def assetAbcRebuild(self,checkType = 'mesh',shotName = '',server = 1,abcBy_ns = 2,clean = 1,printMode = 0,shapeMode = 0):
        ...
        for ns in nsList:
            ...
            if abcBy_ns in [2]:
                errorList = errorList + self.replaceRebuildPerformV2(checkAbcNode=abcNode,checkType=checkType,checkNs=ns,shapeMode=shapeMode)
    ...
    #---------------------------------------------------------#
    # 渲染文件更新cache
    def sk_shotCacheUpdate(self,server = 1, abcBy_ns = 2):
        shotID = sk_infoConfig.sk_infoConfig().checkShotID()
        shotType = sk_infoConfig.sk_infoConfig().checkShotType()
        # 清理
        self.checkCacheCleanBlendShapes()
    ...
    #--------------------------------------#
    # 重连模式
    def replaceRebuildPerformV2(self,checkAbcNode = '',checkType= 'mesh',checkNs = '',shapeMode=0):
        #if checkType not in ['mesh'] and not mc.ls(checkAbcNode):
        #    print mc.ls(checkAbcNode)
        #    return []
        print '---------abcRebuild'
        ...
        for checkAbcObj in nsAbcObjs:
            ...
            if checkType not in ['trans']:
                # blend
                self.singleFrameCacheBlendConfig(checkAbcObj,sourceObj,blendPre = self.tempBlend,abcByNs = 2,shapeMode=shapeMode)
            # transform
            for checkAttr in attrList:
    ...
    def singleFrameCacheBlendConfig(self,source,target,delHistory = 0,blendPre = '',abcByNs = 1,shapeMode=0):
        mc.select(cl=1)
        mc.select(source)
        ...
        if nodeType in ['mesh'] or abcByNs in [2]:
            try:
                ...
                if shapeMode in [0]:
                    blendNode = mc.blendShape(envelope=1,origin = 'local',weight = [0,1])
                if shapeMode in [1]:
                    blendNode = mc.blendShape(frontOfChain = 1,envelope=1,origin = 'local',weight = [0,1])
            except:
                print '\n----------BlendGetError'
                print source
                print target
    ...
    def checkAbcConnectInfoExport(self,server = 1):
        ...
        infoKey = 'abcInfos'
        shotType = sk_infoConfig.sk_infoConfig().checkShotType()
        localSetAnimPath = sk_infoConfig.sk_infoConfig().alembicLocalPath(shotType)
        ...
        # 更新
        if server:
            serverSetAnimPath = sk_infoConfig.sk_infoConfig().alembicServerPath(shotType)
            ...
            for shape in abcshapes:
                for obj in nodeNeeds:
                    if (mc.listRelatives(shape, p=1, type='transform', f=1)[0]).split('|')[-1] == obj.split('|')[-1]:
                        ...
                        if mc.isConnected(g,(needInfo+'.inMesh'))!= True:
...
################################################################
   ...
   ############################################################################################################################
    ...
    #2015/03/24
    ###########################################################################################################################
    def GDC_alembicImp(self,line='pre',server = 1,shotType = 1,UI=0,replaceMode = 0,abcBy_ns = 0,checkType = 'mesh'):
        if not mc.pluginInfo('AbcImport',loaded = 1,q = 1):
            mc.loadPlugin('AbcImport')
        from idmt.maya.commonCore.core_mayaCommon import sk_infoConfig
        reload(sk_infoConfig)
        ...
        if line in ['fs']:
            ...
            if server:
                ...
                basePath = sk_infoConfig.sk_infoConfig().checkShotDataInfoPath(server = server,infoMode = 2.5)
            if replaceMode:
                # 整体模式
                if not abcBy_ns:
    ...
    #2015/03/24
    #---------------------------------------------------------#
    def GDC_curvealembicImp(self,server = 1,UI = 0,abcBy_ns=0):
        if not mc.pluginInfo('AbcImport',loaded = 1,q = 1):
            mc.loadPlugin('AbcImport')
        from idmt.maya.commonCore.core_mayaCommon import sk_infoConfig
        reload(sk_infoConfig)
...
#######Cache物体（from）
   ...
   #----------------------------------------------------------------------------------------------#
    ...
    def csl_checkCacheReturnMaterial(self, MatLists = {} ,finalLayout = 0,shotType=3 ,forceMode = 1):
        if finalLayout:
...
#读取场景并导出场景
    ...
    def csl_checkCacheRecordMaterial(self, checkObjs = [] , finalLayout = 0 ,cacheMode = 1 ,shotType = 3,faceMode = 1):
  ...
  #----------------------------------------------------------------------------------------------#
    ...
    def sk_sceneGRPDelete(self, fileGRP='OTC' ,server = 1):
        shotInfo = sk_infoConfig.sk_infoConfig().checkShotInfo()
        shotType = sk_infoConfig.sk_infoConfig().checkShotType()
    ...
    def sk_sceneGRPExport(self, fileGRP='OTC' , server=1 , shotType = 2 ,needNsList = [] ):
        ...
        if fileGRP == 'SET':
            ...
            for num in range(len(animRefInfos)):
                ...
                checkKey = '_ms_gpu.m'
                if checkKey in lineInfo and shotInfo[0] == u'mi':#=========add by zhangben for minitiger  set gpu.mb file ==== 2016.3.8===========
                    lineInfo = lineInfo.replace(checkKey,u'_ms_render.m')
                checkKey = '_gpu_'
                if checkKey in lineInfo and shotInfo[0] == u'mi':#=========add by shenkang for mi set gpu 2017.2.27===========
...
#            mel.eval(cmd)
    ...
    #------------------------------#
    # OTC结构处理：导入
    def sk_sceneGRPImport(self, fileGRP='OTC' ,server = 1, shotType = 2):
        renderFilePathServer = sk_infoConfig.sk_infoConfig().checkCacheLocalPath(shotType)
        if server:
            renderFilePathServer = sk_infoConfig.sk_infoConfig().checkCacheServerPath(shotType)
        # ma文件利于文本读取改参考
    ...
    #------------------------------#
    # v信息导入
    def checkCacheVStateImport(self , shotType = 2,server = 1):
        vData = self.checkObjsVData(shotType,server)
        if not vData:
            return
    ...
    def checkCacheRecordMaterialImport(self,shotType,server = 1):
        serverDataPath = sk_infoConfig.sk_infoConfig().checkShotDataInfoPath(server=server,infoMode=1)
        fileInfo = 'ShotShaderInfo.txt'
        allInfo = self.checkFileRead(serverDataPath + fileInfo)
        if not allInfo:
            return {}
    ...
    #------------------------------#
    # vData数据结构处理
    def checkObjsVData(self, shotType = 2,server = 1):
